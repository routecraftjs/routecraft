...
**Configuration refuses the collision at boot.** `sessions: { store }` and `suspension: { store }` resolving to the same file now fails while a person is still reading startup output, naming both settings and the shared path. Claims are held per context, and `":memory:"` is exempt because each in-memory database is private to the connection that opened it. A setting holds one path: claiming a second releases the first, so the path an unconfigured default gives up when a `sessions` block replaces it does not go on blocking a store that legitimately wants it.
