...
**Security posture, stated both ways.** The guard resolves every hostname and classifies every resulting address before connecting, refusing loopback, private, link-local (including the `169.254.169.254` metadata address), and other non-public ranges with the new `AI2001`; it re-runs on every redirect hop. Cross-host redirects are not followed, so a permitted host cannot bounce a fetch somewhere it could not reach directly; the target comes back to the model as a URL to consider. Requests carry no caller headers, cookies, or authorization, and URLs with embedded credentials are refused. `allowedDomains` bounds reachable hosts by name.
...
New error codes `AI2001` (refused URL), `AI2002` (request failed), `AI2003` (unreadable content). `ipaddr.js` becomes a dependency of `@routecraft/ai` rather than an optional peer, so the egress guard can never be degraded by an absent install.
