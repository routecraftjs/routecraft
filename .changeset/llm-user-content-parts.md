...
Two limits worth knowing, both documented on the reference page. A part carrying a URL is downloaded by the SDK from your process unless the provider declares it can fetch the URL itself. And a deferral boundary carries JSON data only, so an agent that parks refuses a `Uint8Array` or a `URL` instance in a part at park time, naming the exact part: on a route that can park, pass a base64 string, or the URL as a plain string, which the SDK still reads as a URL.
