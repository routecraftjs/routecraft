...
- **Guard before you validate:** `validateAgainst` assumes a schema and dereferences `~standard.validate` unguarded, so a value arriving from user configuration goes through `isStandardSchema(value)` first. Throw your own refusal on failure: the error code and message differ per boundary on purpose, and only the structural test is shared. Seven sites hand-rolled that test before the guard existed (#575).
- **Never gate on `instanceof Promise`.** The spec says `validate()` returns a result *or a promise of one*, and "promise" is the thenable contract, not the `Promise` class. A hand-rolled `instanceof Promise` check misses a thenable, then reads the thenable object itself as the result record, where the absent `issues` reads as success: a rejecting schema reports that the value passed (#545). `validateAgainst` tests for `then`; a synchronous seam that cannot await (the AI SDK `jsonSchema({ validate })` bridge) refuses on the same test. The class also misses a genuine native `Promise` from another realm, which carries a different intrinsic. This one is enforced: `.eslint-rules/no-instanceof-promise.mjs` bans the operator across `packages/*/src`.
