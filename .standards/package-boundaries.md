...
- The tier shares **only the environment variables the route node declares it needs**, over a documented baseline (`PATH`, `HOME`, `LANG`, `TZ`) that carries no credentials. The route grants environment variables and network egress, and a command reaches neither unless it was granted. Filesystem reads are the stated exception: the `unshare` tier does not contain them, so a command can read any file the calling user can.
