...
- The tier shares **only the environment variables the route node declares it needs**, over a documented baseline (`PATH`, `HOME`, `LANG`, `TZ`) that carries no credentials. A tool can do only what the route granted it, network egress included.
