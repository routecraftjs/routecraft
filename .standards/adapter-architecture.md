...
4. **Key-presence overloads are permitted** (`http({ url })` client vs `http({ path })` server; `mail()` send vs `mail({ action })` operation vs `mail({ folder })` read) because the operation keyword is the category enforcer: `.from(http({ url }))` fails to compile because that shape has no `subscribe`. Key presence has no widening hazard (unlike boolean literals). Key-sniffing over OPTIONAL keys remains forbidden (see naming-policy.md).

   "Presence" means the key was SUPPLIED, never that it holds something truthy. A supplied-but-empty value (`path: ""`, `path: undefined` from programmatically built options) is refused with `RC5003`, never demoted to the other shape: `undefined` on the absence axis is the twin of a widened boolean on the literal axis, and silently switching roles would hand back an adapter that ignores every option passed alongside it. Only an OMITTED key selects the other shape. Shared guard: `selectsFileRole` in `adapters/shared/file-role-guards.ts`, which every `path`-discriminated factory (`json`, `html`, `csv`, `jsonl`, `xml`) routes through. A new one that hand-rolls the check instead makes this law false rather than merely untested, so the guard is the only sanctioned way to ask.

4b. **ARITY is not a discriminant.** Argument count must never select a role. A factory reached with the same intent through a shorthand and a long form returns the same roles either way: `mail('INBOX')` and `mail('INBOX', { markSeen: true })` both carry `subscribe` and `fetch`, and the keyword picks. Positional overloads that change the returned shape are law-2 violations wearing a different hat, and they force the reader to memorise a table. Shorthands may only fill in options.
