...
4. **Key-presence overloads are permitted** (`http({ url })` client vs `http({ path })` server; `mail()` send vs `mail({ action })` operation vs `mail({ folder })` read) because the operation keyword is the category enforcer: `.from(http({ url }))` fails to compile because that shape has no `subscribe`. Key presence has no widening hazard (unlike boolean literals). Key-sniffing over OPTIONAL keys remains forbidden (see naming-policy.md).
   ...
   "Presence" means the key was SUPPLIED, never that it holds something truthy. A supplied-but-empty value (`path: ""`, `path: undefined` from programmatically built options) is refused with `RC5003`, never demoted to the other shape: `undefined` on the absence axis is the twin of a widened boolean on the literal axis, and silently switching roles would hand back an adapter that ignores every option passed alongside it. Only an OMITTED key selects the other shape. Shared guard: `selectsFileRole` in `adapters/shared/file-role-guards.ts`.
