...
- **`agent({ principal: true })` is the only sanctioned path that puts principal data into an LLM prompt.** It appends a `## Caller` section to the system prompt (`formatCallerSection` in `packages/ai/src/agent/destination.ts`). The flag is opt-in; the default omits the section so an agent never leaks identity into a prompt by accident.
