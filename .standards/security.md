...
- **Ops health details are withheld by default** (`packages/routecraft/src/plugins/ops/report.ts`). `ops.health.details` defaults to `when-authenticated`, so per-component `details` maps reach only a caller the server's validator admits; `always` is the named opt-out and `never` the tightening. Statuses themselves are always served, because a probe carrying no credential is the only way an orchestrator can use the surface, and a health endpoint that answers 401 to the kubelet is a health endpoint that restarts the pod. On a server with no validator configured `when-authenticated` collapses to `always`: there is nothing to authenticate against, and a gate with nothing behind it must not silently withhold. That collapse is why the docs say to put the mount on a server the public cannot reach when the details matter, and it mirrors the collapse the http plugin already applies to `/ready`'s `requireAuth`. The gate covers diagnostics only and never topology: component names are the keys of the always-served status maps.
