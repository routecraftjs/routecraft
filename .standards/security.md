...
- **A declared answerer policy fails closed, on both halves** (`packages/routecraft/src/suspension/answerer.ts`). `.suspend({ answer, authorize })` says who may answer a parked question, and every ambiguity resolves to a refusal: an ingress that resolves no principal cannot satisfy a policy and is refused (`RC5056`) rather than passing; `sub: "same"` and `sub: "different"` require a present, non-empty subject on BOTH sides, so two principals that merely lack one are never treated as the same person; a per-call credential presented against a record with no binding, and an unbound credential presented against a record with one, both refuse (`RC5055`), because an advisory binding is not one. The permissive arms that remain are named rather than accidental: `sub: "any"` is the default because a different answerer is the point of an approval, and a `.resume()` door declaring no `keys` serves every channel because that is the single-door case; both mismatches warn at startup.
- **Suspension policy is enforced from the record, and the pre-claim window is ordered** (`packages/routecraft/src/suspension/revive.ts`). The declarative half of the policy persists at park and is read back from the record, never re-read from the live site: editing a site governs future parks only, so one deploy cannot silently weaken every outstanding four-eyes approval. The predicate half cannot persist, so its verbatim source joins the continuation hash and an edit takes the `RC5048` re-ask. Enforcement runs in three bands, and the ordering is the security property, not a style choice: the record-only checks run before the settled-state disclosure and before BOTH pre-claim transitions that settle a record and drive the suspended route's error channel (the deadline arm and the continuation arm). Without that, a holder the checks exist to reject could deny the record, burn the rightful answerer's single-use claim, and drive an approver notification with a credential that was never theirs. The hash is compared non-destructively for the same reason. Do not move a check between bands without re-deriving that argument.
