...
- **The agent `## Caller` section is the only sanctioned path that puts principal data into an LLM prompt.** The agent destination appends it to the system prompt (`formatCallerSection` in `packages/ai/src/agent/destination.ts`). It is **on by default** (`agent({ principal })` defaults to `true`); set `principal: false` to suppress it. The field is the single, documented control point.
- **Default-on is a deliberate trade-off, and it sends identity to the model provider.** Because the section is injected by default, an agent forwards `name` / `email` / `subject` / `roles` to the LLM provider unless the author opts out. This favours DX (agents understand the caller out of the box) over the strict reading of the § 6a production-safe-default principle. Deployments with PII-egress constraints (the identity must not leave the trust boundary for a third-party model) MUST set `principal: false`; treat that as the safe configuration when the model provider is outside your data-handling perimeter. Any future per-field expansion of the default set re-opens this trade-off and needs security sign-off.
- **Only a deliberate subset is surfaced: `name`, `email`, `subject`, and `roles`.** `claims`, `userinfoClaims`, and the bearer token are NEVER injected (a prompt is logged, cached by providers, and echoed back in completions, so it must be treated as an exfiltration surface). `scopes` are loggable per § 3 but are deliberately excluded from the prompt as well, to avoid nudging the model to treat itself as the authorization gate; do not add them without product sign-off.
- **The block is verified, trusted context and belongs in the system prompt, not the user prompt.** The exchange body (user prompt) is attacker-controlled input; the principal is verified at the source boundary. Keeping identity in the system channel stops it from being mistaken for, or smuggled in via, the body.
