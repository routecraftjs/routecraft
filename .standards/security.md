...
- **`reason` is a bounded enum, not free text.** On the MCP paths it is one of `expired`, `infrastructure`, `invalid_token`, `missing_header`, `unsupported_scheme`, `missing_expires_at`; the HTTP plugin uses its own fixed literals. Never derive `reason` from `err.message` (see section 4).
