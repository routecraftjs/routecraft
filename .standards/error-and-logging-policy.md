...
| **Agent delta listener** | An `onDelta` listener threw, on a streamed delta (`stream-llm.ts`) or on the final text a session turn hands it when the provider streamed none (`enricher.ts`). The listener is the consumer's; its failure does not fail the turn, and the delta is dropped | warn | `{ err }` |
