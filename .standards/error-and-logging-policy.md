...
| **Context error handler** | A handler registered at the `error` point with `ctx.registerHandler()` threw. Handled rather than propagated on purpose: the chain continues to the next handler, and the handler's own throw never replaces the error that reaches the failure path, so this log is the only place it survives | error | `{ operation, err, context: "context error handler", handlerIndex }` |
