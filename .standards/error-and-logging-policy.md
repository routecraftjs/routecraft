...
| **Deferral deny-on-cancellation** | Store failure while denying a deferral parked by a cancelled run (best effort: the caller's RC5054 must land whatever the store does) | error | `{ deferralId, routeId, expiresAt, err }` |
