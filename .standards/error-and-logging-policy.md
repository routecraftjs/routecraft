...
| **Agent session follow-up turn** | The boundary turn that consumes a session's inbox failed on an exchange with no route to track it (a synthetic exchange). With a route, `route.trackTask` is the boundary | error | `{ agent, session, err }` |
