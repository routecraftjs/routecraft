...
| Key | At ingress | Why |
|---|---|---|
| `routecraft.id` | minted fresh | Ingress is always a new exchange. Inheriting the caller's id collides in every store keyed by it: telemetry spans (`${exchangeId}:${contextId}`), the `exchanges` / `exchange_snapshots` tables, and deferral ids (`${exchangeId}~${sequence}`). |
...
Durable defer and resume is the first live consumer of this contract: `.defer()` persists exactly these two slots, and `.resume()` rebuilds a `DefaultExchange` around them on whichever process holds the answer. The deferral's own state follows the same rule rather than earning a bag of its own: the park counter, the validated result, and the resume receipt are `routecraft.deferral.*` headers, and `ex.deferral` is the derivation over them.
