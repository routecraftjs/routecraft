...
| `scaffolder-smoke` | `log-flags.bun.test.ts` and `relaunch.bun.test.ts` against the built CLI (`CRAFT_CLI_ENTRY=packages/cli/dist/index.js`), then `bun run test:integration` twice (`TEST_PACKAGE_MANAGER=bun`, then `=npm`) | The built CLI honours `--log-level` and `--log-file` (the bundle can load core ahead of the flags with no source change; 0.7.0 shipped that way), and `craft run` on a file outside any project runs on the CLI's own core (the lone-file cases need core's `dist/`, which the `test` job never builds, so they skip there). End-to-end scaffolder flow: `create-routecraft` -> install -> `bunx tsc --noEmit` -> `bunx craft run` (npm arm skips the run since `craft` is Bun-only). Catches CLI binary regressions, scaffolder template drift, package-manager-specific install failures. |
...
**`loadOptionalPeer` is not adapter-only: `shared/sqlite/driver.ts` calls it on behalf of the deferral store and the telemetry SQLite sink, and neither is an adapter. A new SQLite consumer goes through that shared driver rather than calling `loadOptionalPeer` itself. Pass `consumer` (who is asking, used verbatim in the RC5017 message, so it carries its own noun: `"cron adapter"`, `"deferral store (sqlite)"`) and `packageName`. New code MUST use `loadOptionalPeer`.** The cron source (`packages/routecraft/src/adapters/cron/source.ts`) and the html adapter (`packages/routecraft/src/adapters/html/shared.ts`) are the canonical references; copy the shape (lazy import via the thunk, RC5017 message, type-only `import type` at the top of the file).
...
| Merging the "Version Packages" PR | `release` | `bun run release` (= build + `changeset publish`) publishes to npm with provenance and tags each package (`@routecraft/routecraft@0.7.0`), then pushes a `v<core-version>` tag, then creates one GitHub Release per package with `scripts/create-github-releases.mjs`. A changelog section over GitHub's 125000-character body limit is cut at the last whole entry and ends in a link to the full changelog. A release that still fails is a warning, never a job failure, so `build-and-deploy-docs` always freezes the docs to the fresh tag in the same workflow run. |
