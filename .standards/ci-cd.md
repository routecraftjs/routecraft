...
| Push to `main` touching packages (after smokes pass) | `ci.yml` `publish-snapshot` | Publishes a lockstep canary of EVERY public package (`0.6.1-canary-<datetime>`, all sharing one timestamp) under the npm `canary` dist-tag, no git tags. A synthetic all-package changeset is generated in the job, so canaries flow on every merge whether or not the PR carried a changeset, and downstream projects tracking `@canary` across packages always get a compatible set. |
