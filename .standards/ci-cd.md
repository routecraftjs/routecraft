...
| `adapter-cross-runtime (node)` | `node node_modules/vitest/vitest.mjs run --include '**/test/cross-runtime/**/*.test.ts' --passWithNoTests` | Same suite as above, run under Node. New adapters with a runtime-specific code path (`Bun.sql` vs `pg`, `Bun.s3` vs `@aws-sdk/client-s3`, etc.) drop a sibling test in `packages/<pkg>/test/cross-runtime/*.test.ts` and both arms must pass. |
