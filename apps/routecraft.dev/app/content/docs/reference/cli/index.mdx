...
| --log-level \<level\> | Any pino level (trace, debug, info, warn, error, fatal) or silent. Default: warn. Applied before the logger initializes. |
...
`LOG_LEVEL` and `LOG_FILE` also work from the env files `run` and `start` load, whether [the ones a profile selects](#the-environment-a-profile-selects) or the one `--env` names, and a flag still beats the same setting from a file. A log file that cannot be opened for appending stops the command before it runs, exiting `2` with the path and where it came from; a missing file and missing parent directories are created.
...
| `craft ops deferrals` | One page of what the instance is waiting on, oldest first. Waiting by default; `--state settled` or `--state all` for the rest. A page rather than the whole collection, so the rendering names the cursor for the next |
| `craft ops deferrals <id>` | One deferral: what it waits for, since when, and how it settled |
...
Browse what a context recorded: routes, agents, tool calls, exchanges and live events, read from the SQLite database the [`telemetry()` plugin](/docs/introduction/monitoring#telemetry-plugin) writes. It reads the file rather than a running instance, so it works while the context runs and after it has stopped.
...
| Option | Description |
| --- | --- |
| `--db <path>` | Telemetry database to read. Defaults to `.routecraft/telemetry.db` in the working directory |

Navigation, views and keys are in the [Terminal UI guide](/docs/introduction/tui).
...
A file either escape names is required: if it cannot be read, the command stops before it runs and exits `2` naming the path. The cascade is different, because every file in it is optional.
