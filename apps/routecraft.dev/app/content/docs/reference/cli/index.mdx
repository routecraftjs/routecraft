...
Flags become a flat body: a bare `--flag` is `true`, a repeated key becomes an array, and every other value stays the string the shell passed. Nothing is coerced to a number, because a route that wants one declares it with `.input()` and a CLI guessing types produces bugs that only appear for values which happen to look numeric.
...
| 4 | The door refused: no credential, a rejected one, or a missing scope |
...
Keep `.routecraft/` out of version control. The repository's own `.gitignore` already covers it, and [create-routecraft](https://github.com/routecraftjs/routecraft/issues/588) puts it in the scaffold; check it if you added the directory by hand.
...
Stage one is also bounded by [`shutdown.timeoutMs`](/docs/reference/configuration#shutdown), which matters under an orchestrator: it sends one SIGTERM and then SIGKILLs, so no second Ctrl+C is coming. On that deadline in-flight execution is abandoned and the process exits 1, so a forced shutdown is distinguishable from a clean one.
