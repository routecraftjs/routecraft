...
**The agent travels as a header.** The protocol has no field for which persona a client wants, so `--agent` becomes a `Routecraft-Agent` request header and every conversation the connection opens talks to that agent. A name the instance does not hold is refused by name rather than quietly served by somebody else.
