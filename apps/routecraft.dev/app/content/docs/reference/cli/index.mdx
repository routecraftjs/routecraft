...
deferral all count as terminal alongside a completion, so a broken or parked exchange
...
**A parked exchange is a success.** A route that reaches a durable `.defer()` replies with the standard [`Deferred`](/docs/advanced/durable-agents) acknowledgment instead of its output, and `craft exec` renders it as an outcome at exit `0`, with the deferral id, the resume token and the expiry. It is not an error, and the operator at the terminal is often exactly who the park is waiting for.
