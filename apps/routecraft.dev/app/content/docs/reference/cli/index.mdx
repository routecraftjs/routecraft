...
Standard output belongs to the protocol, so nothing but protocol frames is ever written to it and every diagnosis goes to standard error. It shares [`exec`'s exit codes](#exec): a profile named nowhere exits `2` naming the profiles that do exist, and an address that never answered exits `3` naming it and which setting supplied it. That last one is only for the first connection: an address nothing has ever answered on is configuration to check, where an instance that answered and then went away is an outage to wait out.
