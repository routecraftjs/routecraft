...
Standard output belongs to the protocol, so nothing but protocol frames is ever written to it and every diagnosis goes to standard error. A profile that is named nowhere exits `2` naming the profiles that do exist; an instance that cannot be reached exits `1` naming the address and which setting supplied it.
