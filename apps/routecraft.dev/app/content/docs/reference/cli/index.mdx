...
<Callout type="note" title="A file outside a project">

The file needs no `package.json`. Where `@routecraft/routecraft` is not installed, it runs on the copy that ships with the CLI. The CLI does not bundle adapter dependencies, so an adapter that needs one names it and stops: install it next to the file (`bun add croner`), or next to a globally installed CLI (`bun add -g croner`).
