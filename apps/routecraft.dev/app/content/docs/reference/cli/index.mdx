...
| `--session <id>` | The conversation to continue. A fresh id is minted and printed at the start when omitted |
| `--url <url>` | Ops server base URL. Defaults to the [settings file](#settings-file), else `http://127.0.0.1:8080` |
| `--token <token>` | Bearer credential presented at the management door |
...
A message sent while the session's turn is running is queued rather than answered, and the terminal says so: the turn that consumes it replies into the transcript, which is where a reattached terminal would have to read it. A route failure on one message is printed and the loop goes on, because the conversation is still there; a refused door or an unreachable instance ends the loop with [`exec`'s exit code](#exec) for it.
