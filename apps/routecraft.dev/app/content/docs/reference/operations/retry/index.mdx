...
```ts skip="fragment: source, format are illustrative"
craft()
  .id('resilient-processor')
  .from(source)
  .retry({ maxAttempts: 3, backoff: 1000, factor: 2, jitter: 'full' })
...
- `backoff` - Base wait between attempts (default: 1000ms)
- `factor` - Growth multiplier per attempt: the wait before attempt `n` is `backoff * factor^(n - 1)`. `1` (default) is fixed backoff; `2` doubles each time (`1000, 2000, 4000, ...`); any value `>= 1` is allowed. (Replaces the old `exponential` boolean: `exponential: true` is now `factor: 2`.)
- `maxBackoff` - Upper bound on a single wait so an exponential `factor` cannot grow without limit; the computed wait is clamped to this before jitter (default: the platform timer ceiling, effectively unbounded)
- `jitter` - Randomise each wait to de-sync retry storms: `'none'` (default), `'full'` (uniform in `[0, computed]`), or a number in `[0, 1]` (keep `1 - jitter` to `1` of the wait). Jitter only ever shortens a wait, so it never exceeds `maxBackoff`.
...
```ts skip="fragment: the predicate on its own, not a statement"
...
```ts skip="fragment: source, operation, destination are illustrative"
...
```ts skip="fragment: flakyUpstream, format are illustrative"
craft()
  .id('resilient-pipeline')
  .retry({ maxAttempts: 3, backoff: 2000, factor: 2, maxBackoff: 10_000 })
  .timeout(10_000)
  .from(direct())
