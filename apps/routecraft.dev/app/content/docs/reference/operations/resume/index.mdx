...
Return false or throw to refuse. Both, and a hook that never settles, produce one [`RC5056`](/docs/reference/errors#rc-5056) with one message: a hook whose failures can be told apart from outside is an oracle for what it knows. The log distinguishes them and a thrown cause never reaches the wire. An async hook is bounded by this route's own lifecycle rather than by a framework knob: it is cut short when the route stops, and sooner if the route declares a `.timeout()`, and the suspension's deadline is re-checked once it resolves, so a resume that arrived in time and then sat behind a slow hook reports [`RC5047`](/docs/reference/errors#rc-5047) rather than a refusal.
