...
```ts skip="fragment: userWithWrite is illustrative"
...
A route can opt into the opposite reading at its own gate with [`authorize({ effective: true })`](/docs/reference/operations/authorize#reading-the-actors-scopes), which satisfies a scope check from the subject's ring or the outermost actor's. That is a per-route decision about one capability, made where the caller is refused; the minted principal is unchanged, and every other gate still sees only the intersected scopes.
...
```ts skip="fragment: mailPrincipal, grants are illustrative"
