...
**All three run outside the route's resilience wrappers.** A failure inside a `.retry()`, `.timeout()`, `.circuitBreaker()` or `.concurrency()` segment surfaces to the wrapper first, so the rings are consulted once the attempts are exhausted rather than once per attempt. That is what a route author declaring `.retry()` means by it, and it is why a park lands after the retries rather than on attempt one: nobody should be asked to approve something an automatic retry would have fixed.
