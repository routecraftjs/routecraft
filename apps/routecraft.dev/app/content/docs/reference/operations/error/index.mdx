...
3. **Context level**: a handler registered at the [`error` point](/docs/reference/configuration#handlers), reached only where the two above gave up
...
**The route and context rings run outside the route's resilience wrappers; a step-scope `.error()` does not.** A failure inside a `.retry()`, `.timeout()`, `.circuitBreaker()` or `.concurrency()` segment surfaces to the wrapper first, so those two rings are consulted once the attempts are exhausted rather than once per attempt. That is what a route author declaring `.retry()` means by it, and it is why a park lands after the retries rather than on attempt one: nobody should be asked to approve something an automatic retry would have fixed.

A step-scope wrapper sits in the step list, inside the segment, so it runs on every failed attempt. The two are useful for different jobs: recover a flaky step per attempt with a step-scope `.error()`, and decide what a route's exhausted failure means with the route or context ring.
...
### `recovery.defer(request)` \{#recovery-defer-request}
...
### `notify`: telling someone, in the right order \{#notify}
...
The event is last because it is the claim that an exchange IS parked. A `notify` that throws denies the record and fails the run with [RC5067](/docs/reference/errors#rc-5067), so firing it earlier would give one exchange both `route:exchange:deferred` and `route:exchange:failed`, and a subscriber would be looking at a park that was already dead. An exchange gets exactly one terminal event.
...
ctx.registerHandler('error', (error, exchange, forward, ctx) => {
  const refusal = insufficientAuthorityOf(error)
  if (!refusal || !refusal.effective) return undefined   // not mine, next handler
  // Execution two already carries what a human lent; parking it again would
  // ask for the same scope a second time.
  if (ctx.execution === 2) return undefined
