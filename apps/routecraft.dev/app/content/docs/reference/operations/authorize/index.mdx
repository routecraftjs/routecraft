...
<Callout type="warning" title="The agent becomes the ceiling">

Widening a check to the actor's ring makes the agent's standing scopes the **ceiling of what any caller can reach through that agent**. That is the intended consequence, and it moves where the control lives: keeping an agent's own grant narrow stops being tidiness and becomes the thing bounding every caller who goes through it.

It also means `effective: true` belongs with a named actor matcher rather than `actor: 'any'`. Paired with `'any'`, every agent that can reach the route lends its ring to whoever drives through it, so the ceiling becomes the widest grant among them rather than one agent's.
