...
| `anyScope` | `string[]` | Accepted scopes, any ONE of which admits the principal (OR), where `scopes` requires every one (AND). Given both, they compose as an AND of the two conditions. An empty array is no check, exactly as `scopes: []` is. |
...
- **Missing scope:** throws [`RC5038`](/docs/reference/errors#rc-5038). Recoverable: the cause carries `missing.scopes` so a consent flow can request exactly what is absent. For an `anyScope` refusal that array is the whole accepted set, since any one entry would have opened the door.
