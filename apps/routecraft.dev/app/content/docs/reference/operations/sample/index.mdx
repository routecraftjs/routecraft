...
sample(options: { every: number } | { interval: Duration }): RouteBuilder<Current>
```
...
Pass exactly one of `every` or `interval`; they are mutually exclusive (a sampler is either count-based or time-based). Sampler state (the counter or the window timestamp) is per-route.
...
- `interval` - Time-based: pass the first exchange seen in each `interval` window and drop the rest until the window elapses. Must be at least 1ms.
