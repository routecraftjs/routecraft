...
```ts skip="fragment: source, expensiveOperation, destination are illustrative"
...
```ts skip="fragment: weatherApi, formatForecast are illustrative"
craft()
  .id('weather')
  .cache({ ttl: 60_000 })
  .from(direct())
...
```ts skip="fragment: loadEmployees, loadLeaveForUser are illustrative"
...
**Failure mode:** provider read failures throw `RC5028` (retryable). Key derivation failures throw `RC5029` (not retryable). Provider write failures emit `route:cache:failed phase:"set"` but do NOT fail the exchange (the result was already computed and returned).
