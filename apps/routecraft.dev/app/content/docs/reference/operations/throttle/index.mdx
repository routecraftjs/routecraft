...
throttle(options: {
  rate: number
  per?: 'second' | 'minute' | 'hour' | 'day' | Duration
  mode?: 'delay' | 'reject'
  burst?: number
...
- `per` - the time window. A unit word, one of `'second'` (default), `'minute'`, `'hour'`, `'day'`, or a [`Duration`](/docs/reference/configuration#durations) for a window no unit word names, such as `'90s'`.

  <Callout type="warning">
  A **bare number is milliseconds**, as everywhere else a `Duration` is accepted. `per: 60` is a 60ms window, not a minute. Write `per: '60s'` or `per: 'minute'` for that.
  </Callout>
...
```ts skip="fragment: httpSource, handler are illustrative"
craft()
  .from(httpSource)
...
```ts skip="fragment: source, destination are illustrative"
craft()
  .from(source)
...
```ts skip="fragment: clientIp, mcpTool, destination are illustrative"
craft()
