...
Deferral the exchange durably and exit the pipeline. The run that reaches a `.defer()` ends there and replies immediately; the exchange continues from the next step later, when [`.resume()`](/docs/reference/operations/resume) arrives with the payload.
