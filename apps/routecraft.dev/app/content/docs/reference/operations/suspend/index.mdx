...
| `question` | `string` | -- | No | Human-facing question, surfaced as `Suspended.question` and handed to the answering route's hook. |
| `reason` | `string` | -- | No | Machine-facing reason, surfaced and handed the same way. |
| `meta` | plain JSON | -- | No | Anything the answering route needs to decide who may answer, or an operator needs to read off the record. Persisted verbatim and never interpreted. See [Who may answer](#who-may-answer). |

<Callout type="warning" title="There is no approve or deny">

A suspension is a durable pause with one single-use answer slot of arbitrary shape. The framework has no approve/deny concept, does not read the answer, and attaches no meaning to it: this route's own continuation interprets whatever arrives, which is why the shipped example ends in a `.filter()`. `schema` describes the slot; it does not make an answer an approval, and the framework never reads the answer to decide anything.
...
`question` and `reason` are optional on both suspend surfaces: a route usually notifies through ordinary steps before the park, but a question worth showing an approver is also worth carrying on the record, where the answering route's hook and an operator reading the store can both see it.
...
| Authenticate and authorize the answerer | the resume ingress route: `.authenticate()` for who they are, [`.resume({ authorize })`](/docs/reference/operations/resume#securing-resume) for whether they may answer. `meta` here carries whatever that decision needs |
...
The framework does not define how approvals work. It has no notion of an approver, a role, a four-eyes rule, or an escalation: a suspension is a durable pause with one single-use answer slot, and what makes an answerer legitimate is your design, not ours.

What it does guarantee is the part you cannot build from outside:

- The token addresses exactly one parked record and is single-use.
- The claim is a compare-and-swap, so one answer wins and late or duplicate ones read the settled outcome.
- **Every refusal happens before that claim**, so saying no costs the rightful answerer nothing.
- Both principals and the record are handed to your code at the one moment deciding is free.

So the decision lives on the answering route, in [`.resume({ authorize })`](/docs/reference/operations/resume#securing-resume), and `meta` is how the question carries whatever that decision needs:
...
.suspend({
  schema: Approval,
  ttl: '72h',
  question: `Approve ${payout.amountCents / 100} to ${payout.payee}?`,
  meta: { channel: 'finance', requires: ['payouts:approve'], fourEyes: true },
})
```

`meta` is plain JSON under the same rule as the exchange body ([`RC5042`](/docs/reference/errors#rc-5042) for anything that cannot round-trip), persisted verbatim, and never read by the framework. Because it lives only on the record, a parker that snapshots its policy there gets **policy travels with the question** by construction: the answering hook reads the record, so editing this site changes nothing for questions already asked.

<Callout type="warning" title="`meta`, `question` and `reason` are authored by the suspending step">

On the agent surface that step is the MODEL, and the model has read whatever untrusted tool output is in its thread. Route on them, log them, render them. Do not let one of them be the only thing standing between a caller and an approval.

</Callout>

The continuation always runs as the **parked** principal, restored from storage and [marked restored](/docs/reference/operations/resume#securing-resume). The answering hook gates who may inject the answer and receive the result; it never changes whose authority the rest of the route executes under.
