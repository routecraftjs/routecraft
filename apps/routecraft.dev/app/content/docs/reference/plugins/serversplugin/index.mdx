...
- `teardown(ctx, info)` closes gracefully (stop accepting, reap idle connections, drain in-flight up to `shutdownGraceMs`, then force-close) and emits `server:closed { server }`.
