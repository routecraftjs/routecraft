...
| `auth` | `ValidatorAuthOptions \| ApiKeyAuthOptions \| false` | inherited | No | Single-mount sugar: the wall for a lone `default` mount at `/`. Omit to inherit server auth, `false` to remove the wall (the inherited validator stays reachable for `.authorize()`). Mutually exclusive with `mounts`. |
