...
| `indicators` | `Indicator[]` | `[]` | Indicators to register. See [`defineIndicator`](#defineindicator). |
...
- `ready` is true when no component in the view is `down`.
- The status code is 503 when `ready` is false.
- A view whose components are all `inactive` is `up`, ready, 200: nothing has said this instance cannot serve.

`ready` on `/health` covers every component, so it can be false while `/health/ready` still answers 200. **Nothing should route on the aggregate's `ready`**; that is what `/health/ready` is for.
...
The same shape as `/health`, filtered to the `context` component plus `instance`-domain components.
...
```ts
export const craftConfig = defineConfig({
  ops: { port: 9090, indicators: [mailHealth] },
})
```
...
If both surfaces are configured they cannot share a server, because this mount claims the `/health` prefix and the claim validator refuses the overlap at start. Point one of them at another named server, or disable the http built-ins with `http: { builtins: { health: { enabled: false }, ready: { enabled: false } } }`. When you have both, the ops paths are the ones to point probes at: the http `/ready` cannot go red, including during boot and drain, which is the failure this surface exists to prevent.
