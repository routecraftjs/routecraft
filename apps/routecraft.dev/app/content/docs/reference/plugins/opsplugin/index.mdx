...
Waiting is the default, because the question this is opened with is what is still owed an answer; `?state=settled` asks for the history and `?state=all` for both, and `?route=` narrows to one route. The collection is ordered oldest first, by `(deferredAt, id)` rather than by id: a deferral id is `{exchangeId}#{sequence}` over a uuid, so id order says nothing about time. `limit` and `after` page it, and the cursor is bound to the `state` and `route` filters it was minted under. `GET /ops/deferrals/{id}` answers one row. A store supplied through `deferral: { store }` that does not implement `DeferralStore.list` is refused with [`RC5066`](/docs/reference/errors#rc-5066) naming the member, rather than answering an empty page: an empty listing and a listing the store cannot produce look identical to whoever is reading them.
