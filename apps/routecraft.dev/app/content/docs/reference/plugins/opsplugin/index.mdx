...
Three resources on the same mount, under `/ops`, for driving a running instance: list its routes, describe one, and dispatch work to one. [`craft exec`](/docs/reference/cli#exec) and [`craft ops`](/docs/reference/cli#ops) are clients of exactly this API, and so is anything you point at it with `curl`.
...
It is also the validator the [management tiers](#management-api) identify their caller with. The two uses do not conflict, because a tier's own value decides what that identity must carry while health asks only whether there is one. `auth: false` carries the [server plugin's](/docs/reference/plugins/serversplugin) meaning unchanged: no validator is effective for this mount, so the details gate closes and a scope-gated tier has nothing to check against, which fails the boot rather than admitting everyone.
