...
| `deferred` | 202 | The route parked; `deferral` carries the acknowledgment |
...
A park is an outcome and not an error. The response is the standard [`Deferred`](/docs/advanced/durable-agents) acknowledgment every other surface returns, carrying the deferral id, the single-use resume token and the expiry. There is no resume verb here: who may resume a parked run is the application's policy, expressed with `.resume({ authorize })`, and a management resume door would be the framework building the bypass it deliberately refused to build.
