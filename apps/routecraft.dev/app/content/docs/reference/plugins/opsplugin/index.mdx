...
| `auth` | `HttpAuth` | inherits `servers.<name>.auth` | Validator for the details gate. Not a wall: the surface answers every probe without a credential whatever this says. |
...
`ops.auth` is not a wall. The ingress never authenticates on its own, and this handler calls `authenticate()` only to decide whether to serve `details`, so the surface stays reachable without a credential, which is the only way an orchestrator can use it. `auth: false` is refused as a no-op: there is no wall here to remove. The `/ops` action namespace will define its own admission rule when actions ship.
