...
| `defaultOptions` | `AgentDefaultOptions` | No | Defaults applied to every agent in the context that omits them: `model`, `tools`, `maxTurns`, `principal`, `blocks`, and the sampling block (`temperature`, `maxTokens`, `topP`, the penalties, `reasoning`, `providerOptions`). A per-agent value wins per key. Two installs setting the same single-valued default throw at context init; `blocks` is the one field that composes across installs. |
...
| `user` | `string \| LlmPromptPart[] \| (exchange) => string \| LlmPromptPart[]` | No | User prompt override. Static string, an array of content parts, or a function deriving either from the exchange. Defaults to `exchange.body` (string as-is, JSON for objects) when omitted. See [content parts](/docs/reference/adapters/llm#content-parts) |
...
| `temperature`, `maxTokens`, `topP`, `frequencyPenalty`, `presencePenalty` | `number` | No | Sampling controls for every model call the agent makes. Default to `0` and `1024` for the first two, unset for the rest. Inherit `defaultOptions` when omitted |
| `reasoning` | `'none' \| 'low' \| 'medium' \| 'high'` | No | How much the model should reason per turn, mapped to each provider's own control. Same [mapping table](/docs/reference/adapters/llm#reasoning-effort) as `llm()`. Inherits `defaultOptions.reasoning` when omitted |
...
| `model` | A full `provider:model` reference or one of the aliases `opus`, `sonnet`, `haiku`. `inherit` is valid only as a scalar. A YAML list may hold references and those three aliases; `inherit` inside a list is refused |
...
Only a [surface that has a picker](/docs/advanced/talk-from-your-editor#choosing-the-model-and-thinking-level) uses these; nothing else in a dispatch changes. A choice that is not on the agent's list is refused with [`AI1017`](/docs/reference/errors#ai-1017), so what an agent runs on is still the agent file's decision rather than the caller's.
...
```ts skip="fragment: TenantConfig is illustrative"
...
```ts expect-error="FnOptions.handler is typed (input: unknown), so reading a field off input does not compile and annotating the parameter is not assignable either. The example is the shape we want; the type is what has to change."
import { agentPlugin } from '@routecraft/ai'
import { z } from 'zod'
...
`testFn` validates the input against the `input` schema, calls the handler with a synthetic `{ logger, abortSignal, suspend }` context, and returns the handler's output. Validation failures throw `RC5002`. It works structurally on any `{ input, handler }` shape, so real `FnOptions` values pass without modification.

## Agent tools

> **Status: live.** Tools an agent declares via `tools([...])` are bridged into the Vercel AI SDK's tool-calling loop at dispatch time. The model sees each tool's name, description, and JSON schema; the SDK validates tool-call arguments against the schema, reports validation errors back to the model for self-correction, and otherwise invokes the agent's handler. Durable park and resume through `ctx.suspend()` is live; see [Durable agents](/docs/advanced/durable-agents). Token-level streaming is live via `agent({ stream: true })`; see [Streaming the reply](/docs/reference/adapters/agent#streaming-the-reply).
...
```ts skip="fragment: requireApproval is illustrative"
...
```ts skip="fragment: shows only the tools option"
agent({
  tools: tools([
    'CurrentTime',                                  // fn
...
| Builder | Use |
|---|---|
| `directTool(routeId, overrides?)` | Adapt a registered direct route as a fn. Pulls description, input schema, and tags from the route's discovery bundle by default; `overrides` accepts `description` and `input` to replace either of those (tags pass through unchanged), and `background: true` to return a handle now and deliver the result later (see [Background tools](#background-tools)). |
...
```ts skip="fragment: readFileSync is illustrative"
agentPlugin({
  functions: {
...
```ts skip="fragment: description and system stand in for values defined above"
agentPlugin({
  defaultOptions: {
    model: 'anthropic:claude-opus-4-7',
    tools: tools(['CurrentTime', 'fetchOrder']),
...
```ts expect-error="toolPolicy is all-or-nothing: naming one channel without fn and direct is the mistake this line demonstrates"
...
| Event | Fields | When |
|---|---|---|
| `route:agent:tool:invoked` | `session?`, `toolCallId`, `toolName`, `_snapshot.input` | Agent decided to call a tool. |
| `route:agent:tool:result` | `session?`, `toolCallId`, `toolName`, `_snapshot.output`, `duration` | Tool handler returned successfully. |
| `route:agent:tool:error` | `session?`, `toolCallId`, `toolName`, `errorName`, `_snapshot.error`, `duration` | Tool handler / guard / input validation threw. |
...
**2. `stream: true`** (per-dispatch, opt-in): the dispatch produces the deltas instead of the consolidated `AgentResult`. This is the shortest path from a prompt to a token-by-token reply, because an `AsyncIterable` body is exactly what the [http source frames as SSE](/docs/reference/adapters/http#streaming-responses-sse):

```ts skip="fragment: ChatInput, aria are illustrative"
craft()
  .id("chat-stream")
...
```ts skip="fragment: sse is illustrative"
agent({
  model: "openai:gpt-4o",
  system: "Be helpful.",
  tools: tools(["search"]),
...
- **Per-agent only.** Neither `onDelta` nor `stream` is part of `defaultOptions`: delta sinks are request-scoped, and whether a route's output is a stream is intrinsic to that route.
