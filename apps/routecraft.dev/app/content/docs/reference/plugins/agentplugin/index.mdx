...
| Longer than that | Deferral the run durably with `ctx.defer()`: the loop survives restarts and continues when the resume arrives through any ingress. See [Durable agents](/docs/advanced/durable-agents). |
