...
The trade is the result. `text`, `output`, `usage` and `toolCalls` are consolidated when a run finishes, and a stream is handed over before that. A route that needs both streams to its reader and reads the coarse `route:agent:*` events for the record. Under `.enrich()` the stream merges like any other enrichment value.
