...
| `defaultOptions` | `Partial<LlmOptions>` | No | Default options applied to all `llm()` calls, including the sampling block (`temperature`, `maxTokens`, `topP`, the penalties, [`reasoning`](/docs/reference/adapters/llm#reasoning-effort) and `providerOptions`). A per-call value wins per key |
...
```ts skip="third-party: ai/test is not a dependency of the docs site"
