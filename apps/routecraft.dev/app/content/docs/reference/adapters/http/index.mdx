...
| `maxBodySize` | `number` | `10485760` (10 MB) | No | Maximum response body in bytes. Larger responses fail the exchange with [`RC5061`](/docs/reference/errors#rc-5061). See [Response size limit](#response-size-limit). |
...
Under `'manual'`, a 3xx does not trip `throwOnHttpError`, because it is what the route asked for. Every other non-2xx still does, so opting out of following a redirect is not opting out of noticing a `404`.
