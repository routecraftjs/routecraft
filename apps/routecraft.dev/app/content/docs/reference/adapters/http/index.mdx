...
```ts skip="fragment: loadOrder, createOrderSchema, saveOrder are illustrative"
// routes/orders.ts
...
| `respond` | `HttpResponder` | -- | No | Function deciding when the caller is answered and with what. Returning without awaiting `finished` answers now and detaches the pipeline. Omit it and the framework answers with the pipeline's result. See [Acknowledge, then process](#acknowledge-then-process). |
...
```ts skip="fragment: watchOrder is illustrative"
craft()
  .id("order-events")
...
```ts skip="fragment: nextEvent are illustrative"
...
```ts skip="fragment: '...' is a placeholder, not valid syntax"
defineConfig({
...
```typescript skip="fragment: '...' is a placeholder, not valid syntax"
defineConfig({
...
```typescript skip="fragment: handlePush, handlePaymentEvent, toInboundMessage are illustrative"
...
```typescript skip="fragment: a chain shown without its route"
...
```typescript skip="fragment: isPing is illustrative"
...
```typescript skip="fragment: reportDeliveryFailure, handleDelivery are illustrative"
export const birdHook = craft()
  .id('bird-hook')
...
- A responder on a route that also uses `.batch()` is refused with `RC5003` at subscribe. A batched message waits in the buffer instead of running, so it is not in-flight work and a graceful shutdown discards it, while the responder has already answered.
- Once shutdown has begun, every responder gets the `503` above, including one that would have awaited.
...
| `body` | `HttpRequestPayload \| (exchange) => HttpRequestPayload` | -- | No | Request body (JSON serialized when not string/binary) |
...
| `responseBody` | `'text' \| 'bytes'` | `'text'` | No | How the response body reaches the route. `'bytes'` hands over a `Uint8Array` with no decoding and no JSON parsing. See [Binary responses](#binary-responses). |
...
```ts skip="fragment: revalidateAndFollow is pseudo-code the reader supplies"
