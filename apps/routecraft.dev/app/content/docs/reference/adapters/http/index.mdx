...
- Mount `path`s are static canonical pathname prefixes: no `?`, `#`, `:param` segments, empty segments, or backslashes, and no `.` or `..` segments (raw or encoded), spaces, or anything else URL parsing would rewrite into a different pathname. Anything else is refused at construction with RC5003.
