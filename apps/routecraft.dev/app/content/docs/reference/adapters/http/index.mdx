...
- **Redeliveries inside the tolerance window are yours to deduplicate.** The window bounds replay by age, and the specification's answer to the rest is `webhook-id`, which is stable across retries. Opt into `rawBody: true` and read it from `routecraft.http.rawHeaders['webhook-id']` when a duplicate delivery would do damage.
