...
| `respond` | `"result" \| "accepted"` | `"result"` | No | When the caller is answered. `"accepted"` acknowledges with `202` once the gates pass and runs the pipeline detached. See [Acknowledge, then process](#acknowledge-then-process). |
...
| Yielded | On the wire |
|---|---|
| object with a `data` property (an `SseEvent`) | An event: `event:`, `id:`, `retry:` and `data:` fields read from it. Other keys are not sent. |
...
A streaming response is exempt from the listener's idle reaper, which is
otherwise the only limit on how long a connection may sit open. Two options on
the server definition put a ceiling back:
...
`idleTimeout` takes a [`Duration`](/docs/reference/configuration) and defaults
...
It counts requests that claim a streaming slot, which is how a route's
streaming body is admitted here and how the [ops event
tail](/docs/reference/plugins/opsplugin#tailing-events) is admitted on its
mount. A surface that declares itself long-lived for every request instead,
which today means the MCP mount, opts out at mount level and is not counted.
So the cap bounds the streams this listener admits per request rather than
every long-lived connection it carries: size it against the other surfaces
sharing the same listener rather than treating it as the total.

<Callout type="note">

**This is the listener's bound, not a route's.**
[`.concurrency({ max, mode: "reject" })`](/docs/reference/operations/concurrency)
...
Admission is checked once, when the request arrives. On an ordinary request
that covers the whole request; on a stream held open for hours it would
otherwise cover an unbounded window.
...
authenticates with [`apiKey`](/docs/reference/plugins/httpplugin) in query
mode:
...
A query string is not a private channel. It lands in access logs, proxy logs
and browser history, and it is visible to anything that sees the URL. Issue
credentials for this channel short-lived and narrowly scoped, and rely on the
expiry behaviour above to close the stream when they lapse so the client
...
**That return is delivered late for a producer parked on an inner `await`.**
...
The interval bounds how long a disconnected client's run survives it, so pick it
for the reap you are avoiding rather than making it as long as an intermediary
allows. The [ops event tail](/docs/reference/plugins/opsplugin#tailing-events) is
built this way. Neither shipped consumer is exposed: an agent stream aborts its
run from inside the adapter, and the tail's keep-alive settles its own await.
...
- **There is no `header` to set.** The specification fixes all three names (`webhook-signature`, `webhook-id`, `webhook-timestamp`), so this scheme takes a secret and nothing else, and the option type refuses a header on it. `header` stays required for the other three schemes, which fix no names of their own.
- **The secret is base64.** Pass it as the provider shows it, `whsec_` prefix and all; the prefix is stripped and the rest decoded to the signing key. Padding is optional. A secret that is not base64 fails at the `http({...})` call site rather than on the first delivery.
- **Several signatures may arrive at once**, space separated, which is what key rotation looks like. Any `v1` entry that matches admits the delivery.
- **`toleranceSec`** works as it does for Stripe, defaulting to 300 seconds, and a delivery outside it rejects `signature expired`. A delivery missing `webhook-id` or `webhook-timestamp` rejects `missing signature header`: nothing was presented to verify.
- **A passing signature authenticates the sending system, not a person.** The gate admits the request and attaches no principal. Whatever the payload says about an end user is the sender's claim, so treat it as identification and not as authentication.
- **Redeliveries inside the tolerance window are yours to deduplicate.** The window bounds replay by age, and the specification's answer to the rest is `webhook-id`, which is stable across retries. Read it from `routecraft.http.rawHeaders['webhook-id']` when a duplicate delivery would do damage; the raw headers are on every HTTP exchange and need no opt-in, so deduplicating costs you nothing extra to retain.
- **`prefix` is ignored**, as it is for `stripe-timestamped`.
...
A webhook sender is not waiting for your pipeline. Bird treats anything past a few seconds as a failed delivery and redelivers under the same `webhook-id` for about 27 hours; Stripe and Svix behave the same way; the [Standard Webhooks](https://www.standardwebhooks.com/) specification says outright to acknowledge before processing. A route that answers with its result is therefore wrong for a webhook whose work takes tens of seconds, such as one that leads to a model turn.

`respond: "accepted"` is the rule in one option: acknowledge, then process.

```typescript
export const birdAckHook = craft()
  .id('bird-hook')
...
Every gate still runs before the acknowledgement: path and method matching, the body size limit, body parsing, signature verification, and the mount's credential check where it has one. Only once all of them pass does the caller get `202` with an empty body, and only then does the pipeline start. Rejections are unchanged, so a bad signature is still a `401` and the sender still sees it.

What changes is where a failure goes. The response is gone by the time the pipeline runs, so a failure reaches the route's `.error()` handler and the ordinary error events (`route:error`, `context:error`, `route:exchange:failed`), and nowhere else. Give a route that acknowledges early an `.error()` handler; without one the failure is visible only to an event listener and the log.

The pipeline still runs in full and the exchange is untouched, but its body is never read for the response. `/openapi.json` reflects that: an accepted route advertises `202` rather than `200` and `204`, while keeping the rejection codes its gates can still produce. A route that streams is not a candidate for this option, since the stream would have no reader.

A graceful shutdown still waits for a detached run. The run is the route's in-flight work from the moment it starts, and the context drains that work before any listener closes, bounded by the same shutdown deadline as everything else. Once shutdown has begun the route stops acknowledging: a request that arrives, or that was still in its body read, answers `503` with `retry-after` rather than a `202` this process cannot promise to finish. That is what a webhook sender's redelivery is for, and it is why the option refuses to be combined with `.batch()`: a batched message waits in the buffer rather than counting as in-flight work, so the acknowledgement would outlive the delivery.
...
Bound admission instead: give `.concurrency()` a `maxQueue`, or `mode: "reject"`. Past the bound the exchange fails with `RC5026` on the route's `.error()` channel, which is a delivery you have already acknowledged, so treat that channel as the place a shed delivery is recorded and rely on the sender's redelivery to bring it back.

**A refusal only reaches the sender if it happens before the acknowledgement.** Every transport gate does: path and method, body size, body parse, signature, and the mount's credential check. The pre-from chain positions that run inside the pipeline do not, `.authorize()` among them, so a scope denial on an accepted route stops the work but the caller has already been told `202` and cannot tell a denial from acceptance. Put a route that must be able to refuse a caller on `respond: "result"`.

**Response hints are inert.** `routecraft.http.response.status` and its siblings are read when the dispatcher builds the response, which on this option has already happened, so a step that sets them changes nothing.

**What this does not do.** An acknowledged delivery lost to a crash is not retried, replayed, or persisted; that is the sender's redelivery to make, which is what deduplicating on `webhook-id` in the route is for.
