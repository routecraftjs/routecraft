...
The mount, not the route, decides authentication. `http.mounts` declares named path-scoped surfaces; each is a wall or public, and routes belong to exactly one:
...
| Public (`auth: false`) | served; credentials never inspected, no auth events | verification forced via the server validator: absent or invalid is 401, then roles/scopes |
