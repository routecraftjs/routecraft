...
export const birdHook = craft()
  ...
  // detached pipeline rather than the next step alone.
  ...
  .process(async (ex) => handleDelivery(ex))
  .to(noop())
```
...
A graceful shutdown still waits for a detached run. The run is the route's in-flight work from the moment it starts, and the context drains that work before any listener closes, bounded by the same shutdown deadline as everything else.

**Bounding the work.** One detached pipeline per delivery is unbounded by construction, and a sender that redelivers a backlog will hand you all of it at once. Nothing new bounds this, because the route already can: `.throttle()` and `.concurrency()` sit in the [pre-from filter chain](/docs/advanced/filter-chain) and apply to the detached run exactly as they do to any other. Put one of them on any route that acknowledges early.
