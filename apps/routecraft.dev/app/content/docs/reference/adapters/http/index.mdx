...
```typescript
.enrich(
  http({ url: (ex) => ex.body.mediaUrl, responseBody: 'bytes' }),
  only((r) => r.body, 'media'),
)
```

The body is a `Uint8Array` of exactly what arrived, with no JSON parsing, and the result type says `Uint8Array` without the call site repeating it. `maxBodySize` and `timeout` apply unchanged, counting bytes as they arrive, and an oversized body still fails with [`RC5061`](/docs/reference/errors#rc-5061). With `throwOnHttpError` the failure message names the body's size and content type rather than quoting it, since decoding it for the message would reintroduce the corruption this mode exists to avoid.
