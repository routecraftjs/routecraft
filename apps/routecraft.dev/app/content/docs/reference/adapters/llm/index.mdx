...
- **Raw bytes cannot cross a suspension boundary.** An agent that parks (a suspending tool, a human approval) persists its thread as JSON data, so a `Uint8Array` in a part is refused at park time with a message naming the exact part. Pass base64 or a URL on any route that can park. See [suspension](/docs/reference/plugins).
