...
A level a provider cannot express maps to the nearest one it supports rather than throwing, because an option that refuses on some providers is not portable and portability is the point. Three rows are lossy, and it is worth knowing which:

- **Gemini has no way to turn thinking off.** `none` reaches it as `minimal`, its lowest level. A Gemini model still thinks a little.
...
Anthropic's `effort` is sent as an `output_config` field, so it needs a model new enough to accept one; older models will refuse it at the API rather than at the call site.
