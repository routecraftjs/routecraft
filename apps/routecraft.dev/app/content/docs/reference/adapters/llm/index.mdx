...
- **Neither raw bytes nor a `URL` object can cross a suspension boundary.** An agent that parks (a suspending tool, a human approval) persists its thread as JSON data, so a `Uint8Array` or a `URL` instance in a part is refused at park time with a message naming the exact part. On any route that can park, pass a base64 string, or the URL as a plain string: the SDK reads a URL-shaped string as a URL, and a string survives the store. See [suspension](/docs/reference/plugins).
