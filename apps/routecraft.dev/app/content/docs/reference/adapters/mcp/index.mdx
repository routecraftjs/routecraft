...
One exception to "advertises that schema": a route that can park at a durable deferral advertises `oneOf: [Output, Deferred]` instead, because a call that parks returns the framework's `Deferred` acknowledgment rather than the declared output. See [Deferrable tools](/docs/advanced/expose-as-mcp#deferrable-tools).
