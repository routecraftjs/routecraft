...
One exception to "advertises that schema": a route that can park at a durable suspension advertises `oneOf: [Output, Suspended]` instead, because execution one may answer with the framework's `Suspended` acknowledgment rather than the declared output. See [Suspendable tools](/docs/advanced/expose-as-mcp#suspendable-tools).
