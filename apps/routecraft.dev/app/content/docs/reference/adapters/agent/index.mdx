...
| `session` | `string \| (exchange) => string` | -- | No | The conversation this message continues. The id is the conversation's identity, so naming the same one twice continues the same conversation, exactly as `headers["routecraft.id"]` names an exchange. Every message for one id shares one transcript, kept in the context's [session store](/docs/reference/configuration#sessions). See [Sessions](#sessions-an-agent-that-remembers) |
...
| `models` | `readonly LlmModelId[]` | -- | No | The models a person talking to this agent may choose from. `model` stays what it runs on. A list of fewer than two advertises nothing. See [Offering a choice](/docs/reference/plugins/agentplugin#offering-a-choice-of-model-or-thinking-level) |
| `reasoningLevels` | `readonly LlmReasoningEffort[]` | -- | No | The thinking levels a person may choose from, on the same terms as `models` |
...
```ts skip="fragment: readFileSync, DeploymentStatus, github are illustrative"
...
```ts skip="fragment: the session option on its own, without its route"
agent('max', { session: (ex) => `${ex.principal?.subject}:${ex.body.session}` })
```

The id is a retrieval handle, so it has to be known to be retrieved, the same way a suspension is resumed by its token. A hardcoded id (`session: 'support'`) is therefore a deliberate choice rather than a mistake: every invocation of that route continues one conversation, on one transcript that grows for as long as the route runs. Derive the id from the message when each caller, thread or ticket should get its own.
...
Every session is listable on the management API at [`GET /ops/agent-sessions`](/docs/reference/plugins/opsplugin#contributed-resources), with its turn state and inbox depth, and an editor speaking the Agent Client Protocol is a client on this: see [Talk from your editor](/docs/advanced/talk-from-your-editor).
...
```typescript skip="fragment: ChatInput, aria are illustrative"
craft()
  .id('chat-stream')
