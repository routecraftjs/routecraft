...
**Who a queued message runs as.** A session is keyed by its id alone. A queued message is consumed by whichever turn runs next, under that turn's exchange, principal and tools, not under those of the caller that queued it: the model sees the queued text as user content, and a tool guard reading `ctx.principal` sees the consuming turn's caller. Where more than one principal can reach the route, derive the session id from the principal so no two callers share one:

```ts
agent('max', { session: (ex) => `${ex.principal?.sub}:${ex.body.session}` })
```
