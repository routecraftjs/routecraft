...
`stream: true` replaces that result with an `AgentStream`, an async iterable of `AgentDelta` handed over as soon as the model starts writing. The [http source frames an async iterable as Server-Sent Events](/docs/reference/adapters/http#streaming-responses-sse), so a token-by-token endpoint is one declarative route:
...
The queue and the abort that closes it live inside the adapter. Iterating the stream is what drives the run, and abandoning it aborts one, so a client that disconnects mid-answer stops the model. Custom SSE event names are one ordinary `.transform()` step away; a queue and a wake loop written by hand in a transform are not the way there.

The trade is what a stream cannot carry. `text`, `output`, `usage` and `toolCalls` are consolidated when a run finishes and a stream is handed over before that, so a route needing both streams for its reader and reads the coarse `route:agent:*` events for the record. Under `.enrich()` the stream merges like any other enrichment value. See [the agent plugin's observability channels](/docs/reference/plugins/agentplugin#observability-three-channels) for the full comparison with `onDelta`.
