...
The last seven are the same sampling block `llm()` takes, and they mean the same thing here: one agent can run a narrow classification at `reasoning: 'none'` while another names the same model and asks for `'high'`. An agent that sets none of them behaves exactly as it did before they existed. They can be set once for every agent in a context via `agentPlugin({ defaultOptions })`, where a per-agent value wins per key.
