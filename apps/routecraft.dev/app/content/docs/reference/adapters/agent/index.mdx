...
They apply to every model call in a run: the first turn, each tool-calling turn, and each `validate` retry. A run that defers and resumes resolves them again from the context it resumes in rather than replaying what was resolved before it parked, so a changed `defaultOptions` takes effect on the resumed turns.
