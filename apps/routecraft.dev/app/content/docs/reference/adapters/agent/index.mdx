...
The trade is what a stream cannot carry. `text`, `output`, `usage` and `toolCalls` are consolidated when a run finishes, and a stream is handed over before that. A route needing both streams to its reader and reads the coarse `route:agent:*` events for the record. Under `.enrich()` the stream merges like any other enrichment value. See [the agent plugin's observability channels](/docs/reference/plugins/agentplugin#observability-three-channels) for the full comparison with `onDelta`.
