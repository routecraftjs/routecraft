...
- `**` (globstar) matches zero or more segments at any depth: `route:**` matches every route event; `route:operation:**` matches every operation event.
...
A route is not delivered the events its own exchanges emit (its `route:step:*`, `route:exchange:*` and `route:operation:*` events), because every step it runs emits step events and it would otherwise feed on itself. Its own lifecycle events, such as `route:started`, are delivered. Two `event()` routes that each watch step or exchange events still feed each other, so point an observability route at the routes it observes rather than at another observer.
