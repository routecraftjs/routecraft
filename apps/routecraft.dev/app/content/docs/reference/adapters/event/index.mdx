...
A route is not delivered the events its own exchanges cause: their step, exchange, operation and error events, including `context:error`, and those of any route they call through `direct()`, which shares their correlation id. Every step it runs emits step events, so it would otherwise feed on itself. Its own lifecycle events, such as `route:started`, are delivered. Two `event()` routes that each watch step or exchange events still feed each other, so point an observability route at the routes it observes rather than at another observer.
