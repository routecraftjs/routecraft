...
```ts skip="fragment: writeCsv is illustrative"
craft()
  .from(cron('0 2 * * *'))
  .enrich(carddav())
...
```ts skip="fragment: isStale, myUrlFor are illustrative"
craft()
  .from(carddav())
...
```ts skip="fragment: rawVCardString is illustrative"
