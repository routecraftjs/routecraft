...
Requires the optional peer `croner`: `bun add croner`. A missing peer raises `RC5017` with an install hint when the route starts.
...
| `maxJitter` | `Duration` | `0` | No | Upper bound on a random delay added to each fire, drawn uniformly from `[0, maxJitter)` |
