...
```ts expect-error="file() is not generic in the route body, unlike http(), so the path callback receives Exchange<unknown>. Leaving it inferred does not compile and annotating it is not assignable; only a cast works, and a cast is not what we want users to write."
