...
craft()
  ...
  .transform(async (body, ex) => {
    ...
    // Only the path that got here withdraws and releases. A cancel leaves
    // both to the framework, and reaching for the surface on that path is
    // the refusal this whole section is about.
    release()
