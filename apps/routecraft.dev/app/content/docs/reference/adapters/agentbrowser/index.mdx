...
Requires `@routecraft/os` and the optional peer `agent-browser` at 0.17: `bun add @routecraft/os agent-browser@^0.17`. A missing peer raises `RC5017` with an install hint.
...
craft()
  ...
  .to(agentBrowser<{ url: string }>('open', { url: (ex) => ex.body.url }))
