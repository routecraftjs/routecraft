...
```ts skip="fragment: source, processData, businessLogic are illustrative"
...
- `internal` - Close the route's external doors (default: `false`). An internal route keeps its in-process endpoint, so `direct('id')` enrichers and `forward()` compose against it unchanged, but it skips the capability registry: it is not dispatchable through the [ops management API](/docs/reference/plugins/opsplugin#management-api) (the listing shows `dispatchable: false`, a dispatch refuses with [`RC5060`](/docs/reference/errors#rc-5060) naming the declaration) and a `directTool` naming it fails `context.start()` with the remedy. Declare it on a trusting subroutine whose boundary route carries `.input()`, `.description()` and `.authorize()` for it:

```ts skip="fragment: resolveOrder is illustrative"
// The boundary owns validation and authorization; the subroutine trusts it.
craft()
  .id('resolve-order')
  .from(direct({ internal: true }))
  .process(resolveOrder)

craft()
  .id('orders')
  .description('Resolve an order')
  .input({ body: z.object({ orderId: z.string() }) })
  .authorize({ scopes: ['orders:resolve'] })
  .from(direct())
  .to(direct('resolve-order'))
```
...
```ts skip="fragment: processUser is illustrative"
craft()
  .id('user-processor')
...
```ts skip="fragment: handleRequest are illustrative"
...
craft()
  ...
  .from(direct())
  .process((ex) => {
    // ex.body.createdAt is Date, not string
    console.log(ex.body.createdAt.getFullYear())
    return ex
  })
```
