...
Emitted once per context by [`agentPlugin`](/docs/reference/plugins/agentplugin),
after `context:started`, so a console or a catalogue can list what this
context can do without reaching into plugin internals. They describe the
registry, not a run: a dispatch reports itself through the
[agent operations](#agent-operations) above.

| Event | When it fires | Details |
| --- | --- | --- |
| `agent:registered` | A named agent was registered with the context | `{ agentId, description?, model?, source }` |
| `agent:tool:registered` | A named fn was registered as an agent tool | `{ toolName, description?, tags?, source }` |

A deferred fn (`directTool`, resolved at dispatch rather than eagerly)
registers with its name only: it has no description or tags to publish until
a call resolves it, and those surface on the tool-invocation events instead.
