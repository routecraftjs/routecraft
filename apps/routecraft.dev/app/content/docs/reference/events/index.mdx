...
Emitted on every auth attempt by every auth-enabled surface: HTTP routes, the MCP HTTP transport, and any custom mount that resolves credentials through a named server's validator. The `source` field identifies which surface emitted the event (`"http"`, `"mcp"`, or a custom mount id).
