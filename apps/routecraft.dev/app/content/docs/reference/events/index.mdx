...
[`shutdown.timeoutMs`](/docs/reference/configuration#shutdown) and in-flight
...
The one exception is a [forced shutdown](/docs/reference/configuration#shutdown): when `shutdown.timeoutMs` elapses, in-flight exchanges are abandoned mid-step and emit no terminal event at all. A consumer reconciling starts against terminals should treat a forced shutdown as an expected gap rather than a defect; the warn line the forced stage writes names the routes that were still working.
...
`route:agent:tool:refused` is its call-time counterpart: the tool WAS offered, the model called it, and the tool's guard rejected that particular call. Keeping the two apart matters for what you can count. `denied` says an agent may not have a tool; `refused` says an agent asked for something its grant does not cover, which is the signal that an agent is probing the edges of its allowlist. The payload is bounded to identity plus `rc`, the error code the guard threw (`OS2001` for a shell command outside its granted patterns). The refused input is deliberately absent even under snapshot capture: it is the input least worth trusting, and a command line can carry a token someone passed as an argument. The refusal also surfaces to the model as an ordinary tool error, so it can retry with a permitted form.
