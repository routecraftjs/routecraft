...
An [`event()`](/docs/reference/adapters/event) route is not delivered the events its own exchanges cause, so an observability route cannot feed on itself.
...
**Lifecycle guarantee:** every `route:exchange:started` is eventually followed by exactly one of `completed`, `failed`, `dropped`, or `deferred`.
...
- `'drop'` → `route:exchange:dropped` with `reason: "parse-failed"` (no `route:step:failed` fires; the parse step catches and drops cleanly).
