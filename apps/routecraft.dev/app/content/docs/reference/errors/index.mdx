...
The route's `.input()` schema is [RC5065](#rc-5065) instead, because a caller's bad payload is the caller's fault and this code's cases are the instance's. A transport that answers a request maps the two to different statuses.
...
**Suggestion**  
Cancellation is terminal: re-dispatch the work as a new exchange if it should run again. A run that deferred at a deferral BEFORE the abort survives independently; a run whose deferral raced the abort is refused or denied with [RC5054](#rc-5054), so no stale resume link is left behind.
...
- Anything that must reach the editor after a cancel, a terminal to kill and release, is registered beforehand with [`surface.onCancel()`](/docs/reference/adapters/surface#when-the-turn-is-cancelled); the framework sends it after the turn has answered.
...
**Why it happens**  
The [`.resume({ authorize })`](/docs/reference/operations/resume#securing-resume) hook on the ingress route refused this principal. Who may resume a deferred run is your application's policy, not the framework's: the hook receives the live principal, the deferred principal snapshot, the raw submitted payload, and the record's context, and decides.
...
This is the caller's fault rather than the instance's, which is the whole reason it is not [RC5002](#rc-5002). RC5002 also covers `.output()` validation, a mid-pipeline `validate()` step and an empty aggregation, none of which the caller can do anything about. Splitting them lets a transport answer the two differently: the [ops dispatch mount](/docs/reference/plugins/opsplugin) returns 400 with the message for this code, where a route failure is a 500 carrying only the code.
...
## RC5067
Deferral notification failed

**Why it happens**  
An error handler answered with [`recovery.defer({ notify })`](/docs/reference/operations/error#recovery-defer-request), the exchange parked, and then the `notify` hook threw or did not settle before the deferring route's abort signal fired.

Nobody was successfully told the work is waiting, so the deferral was denied claim-first and its resume link is dead: a token that did reach a recipient reads [RC5050](#rc-5050) rather than reviving work whose caller was told the run failed. The hook's own failure is carried as the cause.

The hook is bounded by the deferring route's abort signal, widened by an enclosing `.timeout()`, because it is awaited inside the executor: an unsettled hook holds the step, which holds `drain()`, and its latency sits in front of the caller's acknowledgment.

**Suggestion**  
Notify by forwarding to a route your application owns rather than calling a mailer inline. The send is then a route like any other: retryable, observable, and testable without the handler, and it runs under the parked exchange's principal because `forward` carries identity by reference. A hook that legitimately takes longer than the route allows needs the route's `.timeout()` widened, not the bound removed.
