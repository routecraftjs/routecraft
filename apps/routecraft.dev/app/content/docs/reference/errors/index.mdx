...
**Why it happens**  
The [`.resume({ authorize })`](/docs/reference/operations/resume#securing-resume) hook on the ingress route refused this answerer. Who may answer a parked question is your application's policy, not the framework's: the hook receives the answerer's live principal, the parked principal snapshot, and the record's metadata, and decides.
...
A door that declares no hook is bearer by design: any holder of a valid token may answer. If you did not intend that, the fix is a hook, not a config flag.
