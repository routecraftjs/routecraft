...
**Why it happens**  
A conversation asked to run on a model or a thinking level its agent does not list. Whoever writes the persona decides what may be changed about it, and the default is that nothing can be: an agent file naming one `model:` offers no choice, and one listing several offers exactly those.
