...
**Why it happens**  
The `ops` config does not describe a surface the plugin can serve. The common causes, each with its own message: an indicator bound to a `route` id no route declares (the message lists the known ids), two indicators sharing a name, an indicator pushed through `up()` or `down()` before any context registered it, or an invalid `port` or `health.details` value.

**Suggestion**  
For a bound route, check the id against the route's `.id(...)`; the binding is by id, not by variable name. For a duplicate, rename one indicator: names are the keys of the health report, so a duplicate would shadow one dependency with another. For a push that has nowhere to land, add the handle to `ops.indicators`, which is what gives it a ledger. A silently dropped report is the failure this refuses to allow, because a dependency nobody is watching looks healthy.
