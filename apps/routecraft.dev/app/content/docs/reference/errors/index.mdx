...
**Suggestion**  
The cause error carries a machine-readable `missing.scopes` array. For a `scopes` (AND) refusal it lists exactly the required scopes that are absent, and every one is needed. For an `anyScope` (OR) refusal it lists the whole accepted set, of which any ONE would have opened the door, so a consent flow can offer the caller the choice. Feed it to your consent flow, or grant at the IdP, then retry.
