...
Four shapes are refused: an empty thread, a tool call with no result (or a result with no call), a tool-call id used twice, and a thread that no longer contains the suspended call the approver's answer lands on. The first three are what a summariser breaks by dropping a message it judged uninteresting; every provider rejects the unpaired thread that comes out. The fourth would fail the revival with [AI1007](#ai-1007) instead, after the approval had already been spent.

The check runs before the store is touched, so a refused replacement leaves the parked record exactly as it was and the run resumes uncompacted.
...
The usual case is `isolation: "none"` with egress left denied. `network` defaults to denied and that tier contains nothing, so it cannot withhold the network; granting it silently would leave the call believing in a containment it never had.

**Suggestion**  
- Say what you mean at the call site: `network: true` beside `isolation: "none"` accepts egress out loud.
- Or choose a tier that can deny it, which on Linux is `unshare`.
