...
**Why it happens**  
The framework-owned `routecraft.suspension.sequence` header carries a value the park counter cannot use: a non-numeric or negative value, or one so large its successor cannot be represented. Headers are a writable bag, so a step (or an external system the headers were round-tripped through) can mangle this one like any other.
