...
**Suggestion**  
- Deferral from a sessionless agent, where `ctx.defer()` defers the dispatch itself.
...
**Suggestion**  
Return one of the supported outcomes from your step: `continue`, `complete`, `drop`, `branch`, or `fanOut`. To deferral an exchange, use `.defer()` rather than hand-rolling the outcome: the executor needs the site the builder assigns to work out what would run on resume.
...
**Why it happens**  
A `.defer()` was declared where the framework cannot durably deferral and revive the exchange: inside a `.split()` that no `.aggregate()` balances, or inside a `.multicast()` path or `.dispatch()` target.
