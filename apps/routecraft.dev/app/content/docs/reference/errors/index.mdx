...
**Why it happens**  
An error handler answered with [`recovery.defer({ notify })`](/docs/reference/operations#recoverydefer), the exchange parked, and then the `notify` hook threw or did not settle before the deferring route's abort signal fired.
