...
Each server accepts `port` (required), `host` (default `127.0.0.1`), `kind` (currently only `http`), an optional validator `auth`, and `shutdownGraceMs` (default `30000`) bounding the graceful drain on stop. A surface inherits server auth unless it supplies its own `auth`, or sets `auth: false` to opt out. Every named server must receive at least one mount, and two names cannot claim the same non-ephemeral `host:port`. Full listener behaviour lives on the [serversPlugin reference](/docs/reference/plugins/serversplugin).

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `server` | `string` | `default` | Named entry under `servers`. |
| `auth` | `ValidatorAuthOptions \| ApiKeyAuthOptions \| false` | inherited | Single-mount sugar: wall for a lone `default` mount at `/`. `false` makes it public. Mutually exclusive with `mounts`. |
| `mounts` | `Record<string, { path, auth? }>` | one `default` mount at `/` | Named path-scoped surfaces with per-mount walls; routes pick one via `http({ mount })`. See the [adapter reference](/docs/reference/adapters/http#mounts-and-authentication). |
...
| `health` | `{ details?: 'always' \| 'when-authenticated' \| 'never' }` | `{ details: 'when-authenticated' }` | How much per-component detail to expose. Statuses are always served. |
| `indicators` | `Indicator[]` | `[]` | Indicators to register, declared with [`defineIndicator`](/docs/reference/plugins/opsplugin#defineindicator). |
