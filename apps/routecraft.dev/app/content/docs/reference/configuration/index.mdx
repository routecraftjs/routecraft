...
| `deferral` | `DeferralConfig` | Only when a route can reach `.defer()` or `.resume()` | -- | Where parked exchanges are stored and how resume tokens are signed. A context with a deferrable route and no `deferral` block refuses to start with `RC5052`; the fields inside it are individually optional ([details](#deferral)) |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `store` | `string \| { path: string } \| "memory" \| DeferralStore` | `.routecraft/deferrals.db` | Where parked exchanges live. A path opens the SQLite backend. `"memory"` opts into the in-process backend, accepting that parked exchanges die with the process. A `DeferralStore` instance plugs in a backend of your own. |
...
| `retention` | `Duration \| "never"` | `90d` | How long settled deferrals (resumed, expired, denied) are kept before the sweeper purges them, measured from the moment the record settled, not from when it parked: an approval that waited 89 days and resolved yesterday still gets its full window. `"never"` keeps everything for audit deployments, and the store then grows with every exchange that ever deferred. |
...
- **Nothing is settled or denied on the way down.** A parked deferral
