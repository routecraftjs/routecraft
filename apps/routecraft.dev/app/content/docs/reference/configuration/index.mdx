...
Configures the HTTP server that backs the [`http()` source](/docs/reference/adapters#adapter-http). Setting this key starts a listener when the context starts (Bun.serve on Bun, `node:http` on Node 22+). See [httpPlugin](/docs/reference/plugins#plugin-httpplugin) for the full behaviour.
