...
Each server accepts `port` (required), `host` (default `127.0.0.1`), `kind` (currently only `http`), and an optional validator `auth`. A surface inherits server auth unless it supplies its own `auth`, or sets `auth: false` to opt out. Every named server must receive at least one mount, and two names cannot claim the same non-ephemeral `host:port`.
