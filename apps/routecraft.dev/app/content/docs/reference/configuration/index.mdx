...
**Every store needs its own file.** A SQLite database carries one `PRAGMA user_version`, and each store versions its own schema through it, so a path given to both `sessions` and `suspension` can satisfy neither. Configuring two stores onto one path is refused at startup, naming both settings. A file that already belongs to another store is refused when it is opened, naming what the file actually is.
</Callout>
