...
| `authorizeTimeout` | `Duration` | `5s` | How long a [`.suspend({ authorize })`](/docs/reference/operations/suspend#who-may-answer) predicate may run before it is refused. It sits in the window where the suspension's own deadline can elapse and the sweeper can take an expiry claim, so it is bounded rather than left to a callback. The common predicate is a pure comparison over claims the ingress already resolved; needing longer usually means a network call that should not be there. |

The `store` and `secret` settings are also readable from the environment, which is how a container deployment supplies them. `defaultTtl`, `sweepInterval`, `expiryLease`, `retention` and `authorizeTimeout` are config-only:
