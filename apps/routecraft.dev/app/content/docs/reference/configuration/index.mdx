...
| `errorHandler` | `ContextErrorHandler \| { handler, mayDefer? }` | No | -- | The application's own error handler for every route in this context ([details](#error-handlers)) |
...
A context error handler is the outermost of three error rings. A step's `.error()` wrapper runs first, then the route's own [`.error()`](/docs/reference/operations/error), and this chain is reached only where those gave up: a route with no handler, or one whose handler rethrew or threw. A route that handles its own failures is never overridden.
...
`registerErrorHandler` returns an unregister function, the same shape `ctx.on()` returns.
...
Declare `mayDefer` on a handler that may answer `recovery.defer()`. It makes every route in the context deferrable, so the context refuses to start without a deferral runtime ([`RC5052`](/docs/reference/errors#rc-5052)) rather than failing on the first park, and transports that advertise deferability (the MCP source's `Deferred` arm) advertise it for every route while such a handler is registered.
