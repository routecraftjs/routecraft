...
| `handlers` | `Partial<Record<HandlerPoint, Handler \| Handler[]>>` | No | -- | The application's own handlers, keyed by lifecycle point ([details](#handlers)) |
...
| `deferral` | `DeferralConfig` | Only when a route can reach `.defer()` or `.resume()` | -- | Where deferred exchanges are stored and how resume tokens are signed. A context with a deferrable route and no `deferral` block refuses to start with `RC5052`; the fields inside it are individually optional ([details](#deferral)) |
...
| `acp` | `AcpPluginOptions` | `acpPlugin(options)`; see [acp](#acp) |
...
```ts skip="third-party: my-kafka-adapter is not a dependency of the docs site"
...
Configures where deferred exchanges are persisted, and the secret used to sign the resume tokens an approver is handed. Required as soon as any route can reach a [`.defer()`](/docs/reference/operations/defer): such a context refuses to start without it, with [`RC5052`](/docs/reference/errors#rc-5052). A context that never defers does not need the key.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `store` | `string \| { path: string } \| "memory" \| DeferralStore` | `.routecraft/deferrals.db` | Where deferred exchanges live. A path opens the SQLite backend. `"memory"` opts into the in-process backend, accepting that deferred exchanges die with the process. A `DeferralStore` instance plugs in a backend of your own. |
...
| `defaultTtl` | `Duration \| "never"` | `72h` | How long a deferral stays resumable when [`.defer()`](/docs/reference/operations/defer) names no `ttl` of its own. `"never"` opts the context out of default expiry entirely, so an unanswered deferral is kept until something else retires it. |
| `sweepInterval` | `Duration` | `60s` | How often the sweeper looks for overdue deferrals. TTLs are measured in hours, so sub-minute precision buys nothing; the knob is for tests and for deployments that want expiry noticed sooner. |
...
| `retention` | `Duration \| "never"` | `90d` | How long settled deferrals (resumed, expired, denied) are kept before the sweeper purges them, measured from the moment the record settled, not from when it deferred: an approval that waited 89 days and resolved yesterday still gets its full window. `"never"` keeps everything for audit deployments, and the store then grows with every exchange that ever deferred. |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `store` | `string \| { path: string } \| "memory" \| SessionStore` | `.routecraft/sessions.db` | Where session records live. A path opens the SQLite backend. `"memory"` opts into the in-process backend, accepting that every conversation dies with the process. A `SessionStore` instance plugs in a backend of your own: five operations (`get`, `create`, `replace` under a version compare-and-swap, `keys`, `remove`) plus `close`, the contract both shipped backends pass the same test suite against. |

| Variable | Purpose |
|----------|---------|
| `ROUTECRAFT_SESSION_STORE` | Store location: a file path, or the literal `memory`. An explicit config value wins. |

The backend is chosen per runtime exactly as the deferral store's is: `bun:sqlite` under Bun, `better-sqlite3` under Node. An explicitly configured path that cannot be opened fails at startup with [`AI1012`](/docs/reference/errors#ai-1012); the unconfigured default falls back to memory with a warning when no driver is available. A value that names neither a location nor a backend is refused at startup with [`RC5003`](/docs/reference/errors#rc-5003). A store instance you supply is never closed by the context.
...
Added by `@routecraft/ai`. Serves the [Agent Client Protocol](/docs/advanced/talk-from-your-editor) so a person can talk to this instance's agents from their editor, the way `mcp` serves MCP. Takes exactly the options [`acpPlugin()`](/docs/reference/plugins/acpplugin) takes, and none of them is required: an empty object serves every agent the context holds at `/acp` on the `default` server, behind that server's wall.
...
| `server` | `string` | `'default'` | Named entry under `servers` to mount on |
| `auth` | `HttpAuth \| false` | inherits `servers.<name>.auth` | Auth for the mount; `false` removes the wall |
| `agent` | `string` | the only registered agent | The agent a harness serves when it names none |
| `cors` | `HttpCorsOptions \| false` | loopback-only | CORS for the mount |
| `browserOrigins` | `readonly string[]` | `[]` | Exact HTTP(S) origins allowed to send browser requests, independently of CORS. Clients without Origin are unaffected. |
...
The agents it serves are the ones `craft start` discovered under `agents/` or the ones written under [`agent`](#ecosystem-keys-added-by); the key applies after `agent` whichever is written first. Agents registered only through `plugins: [agentPlugin()]` are not visible to it, because `plugins` apply after every key: use the `plugins: [agentPlugin(), acpPlugin()]` form for that, in that order. A context holding no agents when the key applies fails the build with [`RC5003`](/docs/reference/errors#rc-5003) rather than serving a protocol with nothing behind it.

**The signing secret is required, at least 32 bytes, and never generated for you.** A context whose routes can reach a durable defer fails at startup with [`RC5040`](/docs/reference/errors#rc-5040) when no secret is configured. The secret is deliberately not stored alongside the deferrals: a store compromise must not also yield forgeable resume tokens. `testContext()`, `NODE_ENV=development` and `NODE_ENV=test` mint an ephemeral in-memory key so tests and local iteration need no setup; that key is regenerated on every start, so tokens minted before a restart stop verifying, and the framework warns when it is in use.
...
- **A sweep the deferral sweeper had already begun does not finish its
...
- **Nothing is settled or denied on the way down.** A deferred exchange
  survives a forced shutdown exactly as it survives a graceful one. Only a
  cancelled run denies its own deferral, and shutdown is not that.
...
A **handler** is registered on the context, by someone who did not write the route, and reaches routes that never mention it. Registration site and authorship are the whole definition, and they are what separate a handler from an [operation](/docs/reference/operations) (which the route author declares, in the chain, at the position it runs) and from an [event](/docs/reference/events) (which cannot change what happens next).

Handlers are keyed by a POINT on the route lifecycle. One point exists today, `error`; `admission`, `entry` and `exit` are specified in [#816](https://github.com/routecraftjs/routecraft/issues/816) and land separately. `HandlerPoint` is a declaration-merged interface, so a package outside core contributes a point without core changing, and two packages claiming one name with different signatures is a compile error.

### The `error` point \{#the-error-point}

The outermost of three error rings. A step's `.error()` wrapper runs first, then the route's own [`.error()`](/docs/reference/operations/error), and this chain is reached only where those gave up: a route with no handler, or one whose handler rethrew or threw. A route that handles its own failures is never overridden.

It exists so that something OUTSIDE a route can decide a failure is recoverable. A plugin can recognise an authorization refusal, park the exchange with [`recovery.defer()`](/docs/reference/operations/error#recovery-defer-request), have a human lend the missing scope, and let the continuation finish, without any route knowing it happened.

There are two ways in, landing in one chain. The config field is registered while the context is built, so it is consulted ahead of anything a plugin registers in its `apply()`.

```ts skip="fragment: stepUpPlugin and the handler bodies are illustrative"
export default defineConfig({
  handlers: {
    error: (error, exchange, forward, ctx) => undefined,
  },
  plugins: [stepUpPlugin()],
  deferral: {},
})
```

```ts skip="fragment: isMine and the recovery body are illustrative"
const stepUpPlugin = (): CraftPlugin => ({
  name: 'step-up',
  apply(ctx) {
    ctx.registerHandler(
      'error',
      (error, exchange, forward, ctx) =>
        isMine(error) && ctx.execution === 1
          ? recovery.defer({ ttl: '4h' })
          : undefined,
      { tags: ['gated'], mayDefer: true },
    )
  },
})
```

`registerHandler` returns an unregister function, the same shape `ctx.on()` returns.

**The fourth argument is a context object, not a bare route.** `ErrorContext` carries `route`, the route the failing exchange belongs to, and `execution`, which is `1` on the exchange's first run and `2` once it is a resumed continuation. A handler that parks needs `execution`: without it, a failure the resume itself causes parks the same exchange again and the human is asked the same question twice. An exchange that parks a second time and resumes again stays `2`, because the distinction is original against continuation and not a park counter, which `ex.deferral.sequence` already is. A three-parameter route `ErrorHandler` stays assignable, so the same function can serve a route and the context.

**A selector scopes a registration to some routes**, by id or by tag, rather than an `if` at the top of the handler: that `if` is a switch statement that grows forever and runs on every exchange of every route. `routes` and `tags` are two ways of naming routes, so a selector carrying both applies to a route matching either; carrying neither applies to every route. A config-declared handler always applies to every route, because a selector is a registration-time argument; scope one by wrapping it in a one-line plugin.

**Handlers are consulted in registration order and the first to return anything other than `undefined` decides.** `undefined` passes to the next one, which is what lets two plugins each own a slice of the failures without either knowing about the other. The return vocabulary is a route handler's, unchanged: a body recovers, `recovery.drop()` discards, `recovery.rethrow()` propagates, and `recovery.defer()` parks.

A handler that throws is reported as `route:error-handler:failed` with `scope: "context"` and a `handlerIndex`, and **the chain continues to the next handler**. Its throw never replaces the error that reaches the failure path. A chain whose first member could take the whole context's error reporting down with it would be worse than no chain.

Declare `mayDefer` on a handler that may answer `recovery.defer()`. It lives on the selector because it is a property of the registration, not of the point. It makes every route in the context deferrable, so the context refuses to start without a deferral runtime ([`RC5052`](/docs/reference/errors#rc-5052)) rather than failing on the first park, and transports that advertise deferability (the MCP source's `Deferred` arm) advertise it for every route while such a handler is registered.

<Callout type="warning" title="A decided failure no longer reaches context:error">

`context:error` and `route:exchange:failed` fire only when nothing decided. A failure a handler resolves reaches neither, which is a change from before this chain existed: a subscriber counting `context:error` as "every failure" will under-count once a handler starts deciding.
...
`.env` is read first, then `.env.<profile>` when a [settings profile](/docs/reference/cli#profiles) is selected, then `.env.local`, each overriding the one before it. A profile can also name one exact file or carry its values inline, and `--env <path>` overrides all of it; the [CLI reference](/docs/reference/cli#the-environment-a-profile-selects) owns that chain in full.

The environment is in place before `craft.config.ts` is imported, which is what lets the config file read `process.env` at module scope.
