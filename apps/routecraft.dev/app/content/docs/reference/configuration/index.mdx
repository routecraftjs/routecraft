...
| `servers` | `Record<string, HttpServerDefinition>` | Only when a surface mounts HTTP | -- | Named HTTP listeners that HTTP, MCP, and custom plugins mount onto ([details](#servers-and-http)) |
