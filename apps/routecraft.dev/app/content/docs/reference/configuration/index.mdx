...
| `shutdown` | `{ timeoutMs?: number }` | No | `{ timeoutMs: 30000 }` | How long a graceful shutdown may drain before it is forced ([details](#shutdown)) |
...
`shutdown.timeoutMs` is how long stage one may drain before stage two
...
released, and the forced stage never runs. A `timeoutMs` under that timer
...
**So `timeoutMs` is not the whole shutdown budget.** Teardown runs after the
deadline, and a plugin that drains on its own clock adds to the total: a
named server drains up to its
[`shutdownGraceMs`](/docs/reference/plugins/serversplugin), which also
defaults to 30 seconds. A context with an HTTP mount and both defaults can
therefore take up to 60 seconds to exit. When you size `timeoutMs` against a
