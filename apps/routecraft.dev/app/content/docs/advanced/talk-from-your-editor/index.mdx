...
| --- | --- | --- |
| `agent` | Which persona answers | The context holds more than one agent, and nothing has been said yet |
...
The persona can be changed only before the first turn. A persona carries its own system prompt and its own tools, so changing it mid-conversation would hand the model a transcript another persona wrote and answers produced with tools it does not have: talking to a different persona is a new conversation rather than a setting. The picker is withdrawn once anything is in the transcript rather than left there to snap back, and a client acting on a list it already held is refused with the unchanged list. An early change is one field on one record, since the conversation keeps its id, and it clears the model and thinking level chosen under the old persona rather than carrying them into an agent that may not offer them. The model and the thinking level themselves stay changeable for the life of the conversation.
