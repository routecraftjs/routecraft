...
- **It outlives the connection.** Close the editor, restart it, connect from another machine, and the same conversation is there by its id. Loading one replays the transcript into the editor so the history is on screen rather than merely on the server.
...
The persona can be changed only before the first turn. The conversation is stored under the agent it belongs to, so an early change re-keys an empty record and a late one would have to rewrite a transcript into a voice that did not say it. The mount refuses the late change with the unchanged list rather than pretending.
