...
`craft start` discovers `agents/` for you, so the plugin takes no list. Building a context by hand instead, pass one: `agentPlugin({ agents: await agents('./agents') })`. That form cannot resolve `skills:` frontmatter refs, which need the house and bundle folders only the project runtime has.
