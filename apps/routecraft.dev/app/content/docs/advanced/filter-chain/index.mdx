...
```ts skip="fragment: loadEmployees are illustrative"
craft()
  .id('list-employees')
  .authorize({ roles: ['hr'] })
  .input(schema)
...
| 4 | `input` | does not run | `.input()` validates the shape arriving at the route, and execution two starts mid-pipeline with a body earlier steps already transformed. The ANSWER is validated instead, against the deferring step's `expect` |
...
| 6 | `circuitBreaker` | does not run | By the time a continuation runs, the resume has already claimed the deferral, so fast-failing here would record a failed continuation result and spend the approval. Put the breaker on the resume ingress route, whose chain wraps `.resume()` and so refuses subsequent answers before they are claimed |
| 7 | `retry` | **runs** | Attempts happen before any continuation result is recorded, so a retried continuation never spends the approval it was answering |
| 8 | `timeout` | **runs** | Bounds execution two. Distinct from `ttl`, which is a store-side expiry on the deferral rather than a per-attempt deadline in this process. It wraps the bulkhead below it, so on a route that also declares `.concurrency()` the deadline covers the wait for a slot as well as the work. Route-scope `.retry()` mitigates this in practice, because it re-attempts before any continuation result is recorded; closing it properly means moving admission above the claim ([#598](https://github.com/routecraftjs/routecraft/issues/598)) |
...
| 9 / 10 | `cache` | refused at build | Both filters wrap the user pipeline, which a deferral exits and a resume re-enters partway down, so neither would ever run (`RC5003`) |
| - | **your continuation** | runs | Steps after the defer, with their own step-scope wrappers |
...
A route-scope `.retry()` applies to execution two, so the steps after a `.defer()` are at-least-once on failure, the same as the steps before it. If your continuation does something a downstream cannot absorb twice (issuing a payment, sending a message), make it idempotent or move it behind a step-scope wrapper you control. This is the position that changed most when continuations started honouring the chain.
...
- **`cacheStore` just below the pipeline.** Runs only on
  ...
  already computed); they emit `route:cache:failed phase:"set"` for
