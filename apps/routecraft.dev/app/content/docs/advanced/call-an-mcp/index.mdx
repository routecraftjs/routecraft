...
export default craft()
  ...
  .authorize({ roles: ['maintainer'] }) // per-call principal check, before the source
  .from(direct())
...
<QuickLink title="Running an MCP server" icon="plugins" href="/docs/advanced/expose-as-mcp" description="Run your own capabilities as MCP tools for AI clients." />
