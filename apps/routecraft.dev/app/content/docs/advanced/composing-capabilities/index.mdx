...
```ts skip="fragment: fetchNewOrders is illustrative"
...
```ts skip="fragment: fulfillOrder is illustrative"
...
export default craft()
  .id('jobs.route')
  .input({ body: z.object({ priority: z.string() }) })
  .from(http({ path: '/jobs', method: 'POST' }))
  // explicit body type at .to() until #726
  .to(direct<{ priority: string }, unknown>((exchange) => `jobs.${exchange.body.priority}`))
```

```ts skip="fragment: processUrgent is illustrative"
...
```ts skip="fragment: processNormal is illustrative"
...
```ts skip="fragment: fetchSnippets is illustrative"
export default craft()
...
<QuickLink title="Capabilities" icon="plugins" href="/docs/introduction/capabilities" description="Author small, focused capabilities using the DSL." />
