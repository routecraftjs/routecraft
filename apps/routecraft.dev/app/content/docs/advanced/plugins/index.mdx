...
export default function loggerPlugin(context: CraftContext) {
  ...
  context.on('context:error', ({ details: { error } }) => {
    context.logger.error(error, 'Context error')
  })
}
```

Or as an object, which is what you need when the plugin has more than one
lifecycle phase. The phases are `apply`, `start` and `teardown`:

```ts
// plugins/metrics.ts
import { type CraftContext, type CraftPlugin } from '@routecraft/routecraft'

// Declaring the key types for what goes in and what comes back out, so
// readers do not have to cast.
declare module '@routecraft/routecraft' {
  interface StoreRegistry {
    'metrics.counters': { started: number; errors: number }
  }
}
...
export default {
  ...
  async apply(context: CraftContext) {
    ...
    context.on('route:started', () => {
      const counters = context.getStore('metrics.counters')
      if (counters) counters.started += 1
    })
  },
...
```ts skip="fragment: KafkaChannel is illustrative"
// craft.config.ts
export const craftConfig: CraftConfig = {
...
Plugins subscribe to events using `context.on(eventName, handler)`. Common events include `route:started`, `route:stopped`, `context:started`, `context:stopped`, and `context:error`. See the [Events reference](/docs/reference/events) for the full list.
...
<QuickLink title="Monitoring" icon="theming" href="/docs/introduction/monitoring" description="Observability patterns built on plugins and events." />
