...
For a catch-all, subscribe to `context:error`. It fires for unhandled errors and for handler failures, but only when nothing DECIDED the failure. A step `.error()` wrapper, a route [`.error()`](/docs/reference/operations/error) and a [context error handler](/docs/reference/configuration#the-error-point) all resolve the exchange when they recover, drop or park it, and neither `context:error` nor `route:exchange:failed` fires for a failure any of them resolved. Count on this as "every failure" only where none of the three is in play.
