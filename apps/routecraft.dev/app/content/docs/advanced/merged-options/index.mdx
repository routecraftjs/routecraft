...
Now every `cron()` source inherits `timezone: 'UTC'` and `jitter: 2000` unless overridden:

```ts
// Inherits timezone: 'UTC' and jitter: 2000 from config
.from(cron('@daily'))

// Overrides timezone but keeps jitter: 2000
