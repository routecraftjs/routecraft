...
Use the same server name for HTTP and other web surfaces to share one port. Use a separate server name when a surface needs an isolated listener. MCP inherits authentication from that server, accepts its own `auth` override, and supports `auth: false` for an explicitly public mount.
