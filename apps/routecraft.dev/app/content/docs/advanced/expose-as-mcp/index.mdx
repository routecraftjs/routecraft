...
| --- | --- | --- |
| `icons` | Routecraft logo (PNG and SVG, light and dark variants) | `icons: []` |
...
Proxied tools appear in `tools/list` under their original name (or the `name` override) with the remote input/output schema, description, title, annotations, and icons passed through. Calls dispatch over the client's registered transport and auth, and the remote result (content, `structuredContent`, `isError`) is returned unchanged, save for the protocol-revision projection every result gets: when the remote and the calling client sit on different revisions, the `{ result: ... }` envelope is added or omitted so the result still matches the schema this server advertised for it. Selection is live: wildcard entries follow tool refresh and stdio restarts.
...
```ts skip="fragment: one property of the mcpPlugin options"
proxy: [
  {
    ref: 'billing:search',
    guard: (_input, ctx) => {
...
A tool whose route can park at a durable [`.suspend()`](/docs/reference/operations/suspend), or whose pipeline dispatches an [agent](/docs/advanced/durable-agents) (its tool handlers may park at runtime), answers a parked run with the framework's `Suspended` acknowledgment instead of its declared output. MCP has no out-of-band status channel the way HTTP has `202`, so the union is real and is published:

- **Advertisement.** When the route declares `.output()`, `tools/list` advertises `outputSchema` as the derived union `oneOf: [Output, Suspended]`. You declare only the output arm; the acknowledgment arm is the framework's. A tool with no `.output()` advertises nothing, parkable or not.
- **The wire result.** A park is an ordinary `isError: false` result whose `structuredContent` is the acknowledgment: `{ status: "suspended", suspensionId, token, schema?, expiresAt? }`, with the same JSON mirrored in the text block. That is the whole shape: anything the suspend site attached as `meta` stays on the record, because this value crosses to the caller. On the 2025 protocol revision both the advertisement and the acknowledgment arrive inside the [SEP-2106](https://modelcontextprotocol.io/specification/2026-07-28) `{ result: ... }` envelope, because a `oneOf` root is not an object root and that revision requires one; the 2026-07-28 revision carries both bare. The envelope is applied to the wire, never to the body your route carries. The caller (a human client, or an agent that can escalate to its own caller) holds the resume token; the payload arrives through any [`.resume()`](/docs/reference/operations/resume) ingress, and the route's real output flows to its destinations on execution two.
- **Events.** A park emits `plugin:mcp:tool:suspended` with the suspension id, never `completed` (a parked run reported as finished is a false receipt) and never `failed` or `declined`.

The advertised union over-approximates for agent-bearing routes: the framework cannot rule suspension out statically, so a client of such a tool must handle the acknowledgment even if a given deployment never parks. Note that `oneOf` is exact-match: a declared output schema loose enough to also accept the acknowledgment shape (an open passthrough object) makes a strict client's validation of a park ambiguous, so keep `.output()` closed.
