...
A tool whose route can defer at a durable [`.defer()`](/docs/reference/operations/defer), or whose pipeline dispatches an [agent](/docs/advanced/durable-agents) (its tool handlers may defer at runtime), answers a deferred run with the framework's `Deferred` acknowledgment instead of its declared output. MCP has no out-of-band status channel the way HTTP has `202`, so the union is real and is published:
