...
- **The wire result.** A park is an ordinary `isError: false` result whose `structuredContent` is the acknowledgment: `{ status: "suspended", suspensionId, token, expect?, expiresAt?, question?, reason? }`, with the same JSON mirrored in the text block. The caller (a human client, or an agent that can escalate to its own caller) holds the resume token; the answer arrives through any [`.resume()`](/docs/reference/operations/resume) ingress, and the route's real output flows to its destinations on execution two.
