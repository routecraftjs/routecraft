...
```ts expect-error="the 0.5 skills option on agent, replaced by blocks in 0.6"
agent({
...
```ts expect-error="the 0.5 skills API on agentPlugin, removed in 0.6"
...
```ts skip="fragment: loadMemoryFor is illustrative"
...
```ts expect-error="the 0.5 tag-selector object; 0.6 selects tagged tools through the catalog callback"
agent({
...
```ts skip="fragment: shows only the option under discussion"
agent({
...
```ts skip="fragment: MyBody is illustrative"
...
```ts expect-error="the 0.5 MailBody shape: subject and text moved in 0.6"
craft()
...
```ts skip="fragment: ctx, sink are illustrative"
...
```ts skip="fragment: fn, source are illustrative"
...
```ts skip="fragment: client, Req, Res are illustrative"
// Before
...
```ts skip="fragment: introspect is illustrative"
// Before (0.5.x)
mcpPlugin({
  transport: 'http',
  auth: oauth({
...
```ts skip="fragment: jwksUrl, issuer and audience are illustrative"
auth: oauth({
...
- New error codes (`RC5018`, `RC5019` for HTTP; `AI1001`-`AI1003` for agent blocks, see [section 8](#8-error-codes-namespace); `RC1003` for error-code registration).
