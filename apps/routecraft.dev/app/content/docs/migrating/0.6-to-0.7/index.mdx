...
3. **Deferral.** `.defer({ expect })` is `.defer({ schema })`, the acknowledgment drops `question` and `reason`, defers expire by default, and a custom store gains members. [Section 3](#3-deferral).
...
Routes built only from the DSL with framework adapters typically need section 1 (the rename is mechanical, and the build tells you where), section 2 if they serve HTTP or MCP, and section 3 if they defer. The rest is for adapter authors, store authors and advanced integrations, plus two notes for anyone who ran a 0.7.0 canary ([section 11](#11-canary-only-notes)). [Section 12](#12-what-is-new-in-070) lists what is new, for context.
...
## 3. Deferral \{#3-deferral}

### 3.1 `expect` is `schema`, and it is optional

**Before (0.6.x):**

```ts expect-error="defer took expect in 0.6; 0.7 takes schema"
...
craft()
  .id("payout")
  .from(direct())
  .defer({ expect: z.object({ approved: z.boolean() }) })
  .to(noop());
```
...
The rename carries through `ctx.defer()` in an agent tool, `DeferError`, `testFn`'s structural defer, the `Deferred` acknowledgment's wire field, the stored record, and `describeExpect`, which is `describeSchema`. A site that declares no `schema` defers with no contract, validates nothing at the resume ingress, and types `ex.deferral.result` as `unknown`. The SQLite store migrates itself.

### 3.2 The acknowledgment carries the contract and nothing else

`Deferred` no longer carries `question` or `reason`, and neither defer surface accepts them. The acknowledgment is `{ status, deferralId, token, schema?, expiresAt? }`: it crosses the wire to whoever called the route, so it carries the contract, while everything policy-shaped lives on the record where only the resume hook and an operator can read it. Put what you carried there into `.defer({ meta })`, which is persisted verbatim and handed to `.resume({ authorize })`, or send it through the notification step that already runs before the deferral.

### 3.3 Defers expire by default

Omitting `ttl` used to mean no expiry. It now means the context's `defaultTtl`, which is `72h`; an overdue deferral is retired by a sweeper that emits `route:exchange:expired` and re-enters the route's error channel with `RC5047`, and scans at startup before the context reports ready. Settled records are purged after `retention` (default `90d`). A deployment relying on deferrals that live indefinitely sets both explicitly:

```ts
import { defineConfig } from "@routecraft/routecraft";

export default defineConfig({
  deferral: { defaultTtl: "never", retention: "never" },
});
```

### 3.4 A custom `DeferralStore` gains members
...
- `claimExpiry(id, at)` and `releaseExpiring(before)`: expiry is claim, notify, finalize, so a crash mid-delivery is redelivered. `DeferralStatus` gains `"expiring"` and records gain `claimedAt`; `markExpired` and `markDenied` now finalize from `expiring`.
- `resumedWithoutTerminal(limit?)`: diagnostic-only, required.
- `replaceStepState(id, fingerprint, next)`: a compare-and-swap on the opaque `stepState` slot of a record that is still deferred, which is what agent compaction uses.
...
A resumed continuation ran under a definition with empty filter arrays, so `.concurrency()`, `.retry()` and `.timeout()` did not apply to execution two. They do now. **A route that declares route-scope `.retry()` alongside `.defer()` gains at-least-once execution of the steps after the deferral**, with no opt-in, so make those steps idempotent or move a side effect a downstream cannot absorb twice behind a step-scope wrapper. `authorize`, `parse`, `input`, `throttle`, `circuitBreaker` and `cache` stay off on a resume, each for a reason recorded on the [filter chain page](/docs/advanced/filter-chain).

### 3.6 Securing resume

Not a break, but the thing most deferring routes want next: `.resume({ authorize })` is one hook receiving `{ principal, deferred, payload, record }`, refusing before the single-use link is spent and before the record's state is disclosed. With no hook the door stays bearer, exactly as in 0.6. Per-call credentials for a parallel tool batch come from `ex.deferral.tokenFor(call)`. See the [resume reference](/docs/reference/operations/resume).
...
- **The deferral deferral counter refuses corruption** with `RC5057` instead of resetting to zero, which could have re-derived an id an earlier deferral already used.
...
- **Durable defer and resume.** [`.defer()`](/docs/reference/operations/defer), [`.resume()`](/docs/reference/operations/resume), the deferral store with signed tokens, the sweeper, `.resume({ authorize })`, `meta`, `tokenFor`, and re-entrant defer sites for adapters that deferral mid-step.
