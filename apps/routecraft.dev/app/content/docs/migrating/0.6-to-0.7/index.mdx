...
10. **Smaller breaks.** `.input()` validation throws `RC5065` rather than `RC5002`, `timer({ exactTime })` and `timePattern` removed, `ops.auth: false` meaningful, thenable and callable schemas validate, ArkType deferral digests change, `.retry()` reaches continuations. [Section 10](#10-smaller-breaks).
...
- **MCP and ACP mounts check `Host` and `Origin` before CORS and auth, and answer 403.** Two things that worked on 0.6 now need configuration:
  ...
  A request the Host or browser-origin check refuses emits `server:request:rejected` with the server, the mount and the reason (`host` or `origin`); a CORS refusal answers `403` without it. See [Securing capabilities](/docs/advanced/securing-capabilities#cors) for the full rules.
