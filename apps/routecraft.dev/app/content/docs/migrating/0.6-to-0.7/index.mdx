...
- **MCP and ACP mounts check `Host` and `Origin` before CORS and auth, and answer 403.** Two things that worked on 0.6 now need configuration:
  ...
  A refused request emits `server:request:rejected` with the server, the mount and the reason (`host` or `origin`). See [Securing capabilities](/docs/advanced/securing-capabilities#cors) for the full rules.
