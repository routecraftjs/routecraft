...
3. **Deferral.** `.defer({ expect })` is `.defer({ schema })`, the acknowledgment drops `question` and `reason`, parks expire by default, and a custom store gains members. [Section 3](#3-deferral).
...
`checkpointId` is renamed `deferralId`, and `defer` is a required member, so a hand-built handler context must provide one. `makeFnHandlerContext` and `testFn` both do, and `@routecraft/testing`'s `TestFnHandlerContext` includes a structural `defer` that returns the sentinel shape without parking anything.
...
- **Callable schemas (ArkType) are accepted** wherever the framework asks "is this a Standard Schema", and a deferral parked under one now hashes its rendered JSON Schema. **The digest changes for ArkType only**: an ArkType park from before the upgrade resumes into `RC5048` (continuation changed), is denied, and re-asks its approver. Settle or drain those before upgrading if a re-ask is disruptive. Zod digests are byte-identical.
- **The deferral park counter refuses corruption** with `RC5057` instead of resetting to zero, which could have re-derived an id an earlier park already used.
...
- **Durable defer and resume.** [`.defer()`](/docs/reference/operations/defer), [`.resume()`](/docs/reference/operations/resume), the deferral store with signed tokens, the sweeper, `.resume({ authorize })`, `meta`, `tokenFor`, and re-entrant defer sites for adapters that park mid-step.
- **Durable agents and sessions.** `ctx.defer()` inside a tool parks the loop; `agent(name, { session })` keeps a transcript with an inbox, one turn at a time, `interrupt: true`, and `directTool(id, { background: true })` for work that outlasts a turn. See [durable agents](/docs/advanced/durable-agents) and the [agent adapter](/docs/reference/adapters/agent).
