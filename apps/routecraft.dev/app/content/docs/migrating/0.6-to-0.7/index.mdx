...
- **A custom store gained members** during the canary: `findExpired(now, limit, after?)`, `claimExpiry(id, at)`, `releaseExpiring(before)`, `resumedWithoutTerminal(limit?)`, `replaceStepState(id, fingerprint, next)`, and `settledAt` on every terminal transition, which is what retention measures from.
