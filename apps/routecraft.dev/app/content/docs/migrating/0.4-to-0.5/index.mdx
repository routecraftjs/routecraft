...
- Forward-compat hooks landed for durable agents (0.6.0): `SuspendError`, `FnHandlerContext.checkpointId` (renamed `suspensionId` in 0.7, when the durable surface landed for real), `AgentSession`. These are forward-compat surfaces; the whole 0.x API is unstable (see the API stability policy).
