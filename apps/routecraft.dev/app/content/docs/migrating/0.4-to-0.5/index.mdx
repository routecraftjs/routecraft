...
```bash
craft run --log-file ./logs.txt server.js
```

The flag belongs before the file. Anything after `run <file>` is pass-through and is handed to the route's own CLI adapter, so a `--log-file` written there never reaches the logger.
...
```bash
craft run --log-file ./mcp.log mcp-server.js
# or
craft run --log-level silent mcp-server.js
```
...
```ts skip="fragment: message are illustrative"
...
```ts expect-error="the 0.4 option names: modelId, systemPrompt, userPrompt, allowedRoutes and allowedMcpServers were all replaced in 0.5"
agent({
...
```ts expect-error="the 0.4 shape: model took an inline provider object, and 0.5 takes a provider-prefixed id string"
agent({
...
```ts skip="fragment: shows only the options under discussion"
...
```ts skip="fragment: ResultSchema is illustrative"
...
```ts expect-error="the 0.4 call: embedding no longer accepts an empty options object, using is required in 0.5"
...
```ts expect-error="the 0.4 import: these symbols moved to @routecraft/routecraft in 0.5, and AuthPrincipal became Principal"
...
- Forward-compat hooks landed for durable agents (0.6.0): `DeferError`, `FnHandlerContext.checkpointId` (renamed `deferralId` in 0.7, when the durable surface landed for real), `AgentSession`. These are forward-compat surfaces; the whole 0.x API is unstable (see the API stability policy).

### Choice operation

```ts skip="fragment: prepUrgent, prepHighValue are illustrative"
craft()
  .id("dispatch")
  .from(direct())
  .choice((c) =>
...
```ts skip="fragment: context is illustrative"
...
```ts skip="fragment: attachEmailPrincipal, yourDestination are illustrative"
