...
```ts skip="fragment: source, auditLog, destination are illustrative"
craft()
  .id('order-pipeline')
  .from(source)
  .tap((exchange) => {
...
<QuickLink title="Operations" icon="presets" href="/docs/introduction/operations" description="What each operation does with the exchange body and headers." />
