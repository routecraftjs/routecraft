...
```ts skip="fragment: enrichOrder is illustrative"
...
```bash
craft start ./my-project --log-level debug
craft run --log-level debug ./capabilities/orders.ts
```
...
```ts skip="fragment: alerts is illustrative"
// craft.config.ts
...
```ts skip="fragment: metrics, alerts are illustrative"
...
```ts skip="third-party: @opentelemetry/sdk-trace-base is not a dependency of the docs site"
import { telemetry } from '@routecraft/routecraft'
...
```ts skip="fragment: tracerProvider is illustrative"
