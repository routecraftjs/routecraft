...
An exchange error never changes a status. A refused caller or a validation failure is a healthy route behaving correctly, so escalating it would turn every scope gap into an incident. Repeated failure reaches health through a circuit breaker, or through an [indicator](/docs/reference/plugins/opsplugin#define-indicator) your own code pushes.
