...
| Check a validation boundary honours an async schema | `thenableSchema({ issues })` / `thenableSchema({ value })` |
...
```ts skip="fragment: mailMock are illustrative"
...
```ts skip="fragment: vi is illustrative"
const httpMock = mockAdapter(http, {
  send: vi.fn()
...
```ts skip="fragment: t is illustrative"
...
```ts skip="fragment: expect are illustrative"
import { testContext, spy } from "@routecraft/testing";
...
```ts skip="fragment: timerRoute is illustrative"
...
```ts skip="fragment: expect, spyAdapter are illustrative"
// Basic assertions
...
```ts skip="fragment: expect is illustrative"
...
```ts skip="fragment: exchange, expect are illustrative"
...
<QuickLink title="Errors reference" icon="warning" href="/docs/reference/errors" description="RC error codes -- useful when asserting on t.errors in tests." />
