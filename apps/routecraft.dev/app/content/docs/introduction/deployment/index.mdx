...
- **No secrets in the build context.** `.dockerignore` keeps `.env` files, `.npmrc`, `node_modules` and local state out. A private registry token belongs in a build secret, never in the context.
...
- **Listen on every interface.** A server binds `127.0.0.1` unless its `host` says otherwise, which nothing outside the container can reach. Set `host: '0.0.0.0'` on the servers you publish (see [servers and http](/docs/reference/configuration#servers-and-http)).
- **Keep state on a volume.** Deferred exchanges, agent sessions and telemetry are stored under `.routecraft/`, which a redeploy replaces. Mount a named volume there (`docker run -v my-app-state:/app/.routecraft my-app`) so pending work survives; a named volume keeps the `nonroot` ownership.
- **No host binaries.** `shell()` and `agentBrowser()` from `@routecraft/os` run programs the distroless runtime does not contain. A project that uses them needs a runtime stage that ships those programs, and is scanned as such.
