...
1) Repository
- Push your app to GitHub/GitLab including `craft.config.ts` and the `Dockerfile` `create-routecraft` scaffolds (see the [Deployment guide](/docs/introduction/deployment#docker)).

2) App creation
- Create App → connect repo → pick root directory.
- App Platform auto-detects the Dockerfile. No build/run command needed; the image's `CMD` runs `craft start`.
