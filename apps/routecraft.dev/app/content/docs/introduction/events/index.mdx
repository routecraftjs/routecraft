...
export const craftConfig: CraftConfig = {
  on: {
    ...
    'context:error': ({ details: { error, route } }) => {
...
Call `context.on(event, handler)` with an event name or pattern. The handler receives `{ ts, contextId, details }`.
...
export default function loggerPlugin(ctx: CraftContext) {
  ...
  ctx.on('context:error', ({ details: { error } }) => {
    ctx.logger.error(error, 'Context error')
  })
}
```
...
```ts skip="fragment: ctx, handler are illustrative"
...
```ts skip="fragment: ctx, audit, alerts are illustrative"
...
// Declaring the event is what makes it emittable and gives subscribers
// its details type.
declare module '@routecraft/routecraft' {
  interface EventDetailsMap {
    'plugin:auth:capability:secured': { capabilityId: string }
  }
}
...
```ts skip="fragment: ctx, alerts are illustrative"
...
<QuickLink title="Events reference" icon="presets" href="/docs/reference/events" description="Full event catalog with all payload shapes and filtering patterns." />
