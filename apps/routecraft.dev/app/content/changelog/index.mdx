...
- **`.defer({ expect })` is `.defer({ schema })`, and the option is optional** -- the `Deferred` acknowledgment drops `question` and `reason` (carry them in `meta`, or in the notification step that runs before the deferral), and a `DeferralStore` of your own gains required members. Deferrals now expire by default after `72h`; set `deferral: { defaultTtl: 'never' }` to keep the previous behaviour. See the [migration guide](/docs/migrating/0.6-to-0.7#3-deferral).
