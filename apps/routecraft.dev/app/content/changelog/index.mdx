...
- **An unwritable log file stops the command** -- `craft` exits `2` naming the path instead of running on with its logs diverted to the temporary directory. See [basic usage](/docs/next/reference/cli#basic-usage).
...
- **A missing or plain-http `resource.url` fails with `RC5003`** -- the refusal was a bare `TypeError`; it now carries a code, a docs link, and the fix (`NODE_ENV=development` for local work, an HTTPS URL behind a proxy).
