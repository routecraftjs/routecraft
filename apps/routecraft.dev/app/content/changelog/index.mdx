...
- **Context error handlers** -- [`ctx.registerErrorHandler(handler)`](/docs/next/reference/configuration#error-handlers) and the `errorHandler` config key add a third error ring outside the route, so a plugin can decide that a failure is recoverable without any route knowing. Handlers run in registration order and the first non-`undefined` return decides; a route that handles its own failures is never overridden. <Badge color="red">Breaking</Badge> **`context:error` and `route:exchange:failed` now fire only when nothing decided**, so a failure a handler resolves reaches neither: a subscriber counting `context:error` as "every failure" will under-count once a handler starts deciding.
