...
- **An error can become a deferral** -- `.error()` handlers and the new context error chain may answer with [`recovery.defer(request)`](/docs/next/reference/operations/error#recovery-defer-request), which parks the failing exchange durably instead of recovering, dropping or rethrowing. The handler names no position: the framework parks at the failing step re-entrantly, or at position 0 as an admission when the failure came from the pre-from chain, and refuses positions a deferral cannot be revived from with `RC5051`. The directive carries `notify`, which the executor awaits AFTER the record is written and BEFORE `route:exchange:deferred` fires: after the write so a park the framework refuses sends nothing external and nobody is handed a token for a record that will never exist, and before the event so a notification that fails does not leave an announced park contradicting the failure that follows. It is bounded by the deferring route's own abort signal, and a throw or an abort denies the record claim-first (`RC5067`), so no live link survives a notification that did not go out and the exchange still reports exactly one terminal event.
- **Context handlers, and the first point on the route lifecycle** -- [`ctx.registerHandler('error', handler, selector?)`](/docs/next/reference/configuration#handlers) and the `handlers` config key add a third error ring outside the route, so a plugin can decide that a failure is recoverable without any route knowing. Handlers run in registration order and the first non-`undefined` return decides; a route that handles its own failures is never overridden, and nothing is consulted inside a `.retry()`, `.timeout()`, `.circuitBreaker()` or `.concurrency()` segment, so a park lands after the attempts are exhausted rather than on the first one. A selector scopes a registration by route id or tag, and the handler's fourth argument carries the route and whether it is looking at the first run or a resumed continuation. `error` is the first of four points ([#816](https://github.com/routecraftjs/routecraft/issues/816)); the registry and the config key are shaped for the rest from the start, so nothing is renamed when they land. <Badge color="red">Breaking</Badge> **`context:error` and `route:exchange:failed` now fire only when nothing decided**, so a failure a handler resolves reaches neither: a subscriber counting `context:error` as "every failure" will under-count once a handler starts deciding.
- **A resume door may elevate within the same identity** -- [`.resume(mapper, { elevate })`](/docs/next/reference/operations/resume#elevating-within-the-same-identity) re-mints the parked principal so the continuation carries more authority than the exchange parked with, which is what makes a generic step-up possible: refused for a missing scope, park, a human lends the scope, resume, succeed, with no route knowing. The framework enforces the rest: the principal must be live-branded, it must be the same two parties (everything but `scopes` is compared structurally), and a lend is capped by the scopes the refusal recorded on the deferral. Anything else is `RC5056`, non-destructive. A door with no `elevate` behaves exactly as before.
- **`InsufficientAuthority.missing` carries `effective`** -- whether the gate that refused read the effective ring, so a consent flow can decline to ask a human for a scope that could never open that door. `insufficientAuthorityOf(error)` is exported for reading the refusal detail off a thrown `RC5038`.
- **Durable defer and resume** -- [`.defer()`](/docs/next/reference/operations/defer) defers an exchange in a store (SQLite by default, `memory` for tests, or a `DeferralStore` of your own) and answers the caller with a `Deferred` acknowledgment; [`.resume()`](/docs/next/reference/operations/resume) revives it by signed token from any transport at the same position, with `.resume({ authorize })` as the one policy point, `meta` travelling with the deferral, per-call credentials through `tokenFor`, and expiry, crash-safe delivery and retention handled by a sweeper that scans at boot. Re-entrant defer sites let an adapter defer the step it is executing, which is what durable agents build on.
- **Named servers and mounts** -- one listener carries http routes, MCP, ACP and the ops surface on distinct paths, or each gets its own port; mounts claim paths at bind time and own their authentication. See the [servers plugin](/docs/next/reference/plugins/serversplugin) and the [http plugin](/docs/next/reference/plugins/httpplugin).
- **The ops plugin** -- `/health` with liveness and readiness, `defineIndicator` for dependencies the framework cannot see, the management API (`GET /ops/routes`, `POST /ops/routes/{id}/exchanges`, contributed resources) behind scope-gated tiers that answer 404 until named, and `GET /ops/events` tailing the event bus as Server-Sent Events. See the [ops plugin](/docs/next/reference/plugins/opsplugin).
...
- **Remotes** -- `defineConfig({ remotes })` makes another instance's dispatchable routes direct endpoints here, named git-style (`hello` for the default remote, `lab:hello` for any other), with a local route winning a name clash loudly. See the [remotes plugin](/docs/next/reference/plugins/remotesplugin).
- **`.enabled()`** -- a route declares whether it runs at all; a disabled route is known, not started and not offered to an agent, with its reason on the health report, and `context.reevaluateEnablement()` brings it up without a restart. See [enabled](/docs/next/reference/operations/enabled).
...
- **`shell()` in `@routecraft/os`** -- runs a command without ever invoking a shell, isolated by default (`unshare` on Linux, or `docker` as a throwaway container per command), with a granted rather than inherited environment and `untrusted()` marking values that came from outside. See the [shell reference](/docs/next/reference/adapters/shell).
- **`surface()` in `@routecraft/ai`** -- a capability reaches the person's editor over the Agent Client Protocol: `surface(method, params)` asks and waits, `surface.notify()` tells. See the [surface reference](/docs/next/reference/adapters/surface).
- **`timer()` and `cron()` take `maxJitter`**, and `.throttle({ per })` accepts a `Duration`.

### AI & MCP <Badge color="red">Breaking</Badge>

- **`currentTime()` and `randomUuid()` are removed** -- declare the fn inline under the same tool name; every `tools([...])` reference keeps working. See the [migration guide](/docs/next/migrating/0.6-to-0.7#5-agents-and-tools).
...
- **A downstream park never hands the model a resume token** -- a tool whose own route parks resolves with the framework's `Deferred` acknowledgment, which carries a live token; under the default bearer door that would let a model resume its own parked act. The bridge records the neutral `DEFERRED_TOOL_PLACEHOLDER` instead, so the token reaches neither the model's context, the session thread nor the telemetry snapshot. The agent's own run does not park on a downstream park: the turn continues.
- **The agent events are declared by this package** -- every `agent:*` and `route:agent:*` entry moves out of core's `EventDetailsMap` and into `@routecraft/ai` through declaration merging, the shape the MCP events already used. No event name, payload or emit site changes, and subscribing is unchanged.
- **Durable agents** -- `ctx.defer()` inside a tool defers the whole tool loop through the core store; the loop survives a restart and resumes mid-conversation with the answer swapped into the tool result; MCP advertises `oneOf: [Output, Deferred]`; a cancelled run fails with `AI1005` instead of returning a partial result. See [durable agents](/docs/next/advanced/durable-agents).
- **Sessions** -- `agent(name, { session })` keeps a durable transcript with an inbox and one turn at a time, `interrupt: true` cancels the running turn, a `sessions` store (SQLite by default) holds the records, and `GET /ops/agent-sessions` lists them. Background tools (`directTool(id, { background: true })`) hand long work to a route and post the result to the session's inbox when it finishes. See the [agent adapter](/docs/next/reference/adapters/agent).
- **Talk to an agent from your editor** -- `acp:` on `defineConfig` (or `acpPlugin()`) serves the Agent Client Protocol beside MCP; a conversation belongs to the person who started it and outlives the connection, the editor picks the model and the thinking level from lists the agent file advertises, a message sent mid-turn is answered under that message, and tool calls stream as they run. See [talk from your editor](/docs/next/advanced/talk-from-your-editor) and the [acp plugin](/docs/next/reference/plugins/acpplugin).
- **`craft start` and the project runtime** -- a project boots from its folder layout (`craft.config.ts`, `plugins/`, `agents/`, `skills/`, `capabilities/`), Claude Code agent files load unchanged, and a package claims a convention folder through `registerProjectDiscoverer`. See [project structure](/docs/next/introduction/project-structure).
...
- **`craft start`**, **`craft exec <route>`** and **`craft ops health | ready | routes | deferrals | indicators`** boot a project and drive a running instance; **`craft acp`** is the bridge an editor runs, and it reconnects on its own when the instance behind it restarts. A personal settings file (`.routecraft/settings.yaml`) with **profiles** (`url`, `token`, `agent`, `env`) points every command at a laptop or a company instance, `.env.<profile>` joins the env cascade, a refusal names who issues acceptable tokens and which scope is missing, a blank flag no longer overrides the file, and a failed dispatch shows the framework's error code. See the [CLI reference](/docs/next/reference/cli).
...
- **[0.6.x to 0.7.0 migration guide](/docs/next/migrating/0.6-to-0.7)** -- every breaking change above with its before and after.
- **New pages** -- [durable agents](/docs/next/advanced/durable-agents), [talk from your editor](/docs/next/advanced/talk-from-your-editor), [project structure](/docs/next/introduction/project-structure), and reference pages for the [ops](/docs/next/reference/plugins/opsplugin), [servers](/docs/next/reference/plugins/serversplugin), [acp](/docs/next/reference/plugins/acpplugin) and [remotes](/docs/next/reference/plugins/remotesplugin) plugins and the [shell](/docs/next/reference/adapters/shell) and [surface](/docs/next/reference/adapters/surface) adapters.
