...
## [v0.7.1](https://github.com/routecraftjs/routecraft/releases/tag/v0.7.1) <Badge color="yellow">Pre-release</Badge>

*September 2026*

### CLI

- **`--log-level` and `--log-file` take effect again** -- in 0.7.0 both flags were ignored and every log line went to stdout, which corrupts an MCP server run over stdio. See [basic usage](/docs/reference/cli#basic-usage).
...
- **`LOG_LEVEL` and `LOG_FILE` work from env files** -- `.env.<profile>`, a profile's `env:` and `--env <file>` were loaded after the logger was built, so log settings there were ignored; a flag still beats them. See [the environment a profile selects](/docs/reference/cli#the-environment-a-profile-selects).
- **An unwritable log file stops the command** -- `craft` exits `2` naming the path and where it came from (a flag, `LOG_FILE`, or `craft.log.js` / `.cjs`) instead of running on with its logs diverted to the temporary directory. See [basic usage](/docs/reference/cli#basic-usage).
- **A forced `--once` shutdown names the right option** -- its hint said to raise `shutdown.timeoutMs`, which 0.7 refuses; it now says `shutdown.timeout`. See [shutdown](/docs/reference/configuration#shutdown).
- **A named env file that cannot be read stops the command** -- `--env <path>` or a profile's `env: "<file>"` pointing at nothing was skipped with a note hidden at the default log level, and the command ran against an environment nobody chose; it now exits `2` naming the path. A `.env` cascade file that exists but cannot be read stops the command the same way. See [the environment a profile selects](/docs/reference/cli#the-environment-a-profile-selects).
...
- **The 0.6 to 0.7 migration guide covers the Host and Origin checks** -- MCP and ACP mounts refuse a browser `Origin` or an unknown `Host` with 403 until `browserOrigins` or `allowedHostnames` lists them, which the guide did not say. See [servers and mounts](/docs/migrating/0.6-to-0.7#2-servers-and-mounts).
...
- **Event names match what the framework emits** -- reference pages, the events namespace map and JSDoc still used bare `exchange:*`, `step:*` and `error:caught` names and route ids inside names, none of which a subscription matches since 0.6. See [events](/docs/reference/events).
- **mcpPlugin documents `resource`** -- the options table had no row for the protected-resource URL that production requires. See [mcpPlugin](/docs/reference/plugins/mcpplugin).
