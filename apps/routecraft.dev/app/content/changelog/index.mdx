...
- **`--log-level` and `--log-file` take effect again** -- in 0.7.0 both flags were ignored and every log line went to stdout, which corrupts an MCP server run over stdio. Setting `LOG_LEVEL` and `LOG_FILE` in the environment was the workaround. See [basic usage](/docs/next/reference/cli#basic-usage).
