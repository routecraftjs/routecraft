...
- **An unwritable log file stops the command** -- `craft` exits `2` naming the path and where it came from (a flag, `LOG_FILE`, or `craft.log.js`) instead of running on with its logs diverted to the temporary directory. See [basic usage](/docs/next/reference/cli#basic-usage).
...
- **`defineConfig` rejects keys `CraftConfig` does not declare** -- in 0.7.0 a config still written for 0.6 (`http: { host, port, auth }`, `shutdown.timeoutMs`) or a misspelled key compiled and failed only at boot; it is now a compile error at any depth, the same one a `CraftConfig` annotation reports. The helper returns `CraftConfig` rather than the literal type of its argument. See [configuration](/docs/next/reference/configuration#craft-config) and the [migration guide](/docs/next/migrating/0.6-to-0.7#2-servers-and-mounts).
...
- **A missing, plain-http or unparseable `resource.url` fails with `RC5003`** -- the refusal was a bare `TypeError`; it now carries a code, a docs link, and the fix (`NODE_ENV=development` for local work, an HTTPS URL behind a proxy).
