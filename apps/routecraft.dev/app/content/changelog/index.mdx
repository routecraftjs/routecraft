...
- **`defineConfig` rejects keys `CraftConfig` does not declare** -- in 0.7.0 a config still written for 0.6 (`http: { host, port, auth }`, `shutdown.timeoutMs`) or a misspelled key compiled and failed only at boot; it is now a compile error at any depth, the same one a `CraftConfig` annotation reports. The helper returns `CraftConfig` rather than the literal type of its argument. See [configuration](/docs/next/reference/configuration#craftconfig) and the [migration guide](/docs/next/migrating/0.6-to-0.7#2-servers-and-mounts).
