...
> These are the in-development docs for the next Routecraft release, built from the main branch. They describe API that has not shipped yet. For the released documentation, see <https://routecraft.dev/llms-full.txt>.
...
The CLI is Bun-only. See the [Runtime reference](/docs/next/reference/runtime) for the rationale and the Node embedding path.
...
For all flags and options, see [CLI -- create](/docs/next/reference/cli#create).
...
>
> The `start` script invokes the local `craft` bin, which runs on Bun (>=1.1.0) regardless of which package manager runs the script. Install Bun on the production host, or follow the [Node embedding path](/docs/next/advanced/programmatic-invocation) instead.
...
You get full programmatic control: load specific capability files, run a single capability for a batch job, or integrate Routecraft into a larger Express, Next.js, or Fastify server. See the [Programmatic Invocation guide](/docs/next/advanced/programmatic-invocation) for the full pattern.

---

## Related

- [CLI reference](/docs/next/reference/cli) -- All CLI commands and options.
- [Project structure](/docs/next/introduction/project-structure) -- Understand the layout of a Routecraft project.
...
- [Installation](/docs/next/introduction/installation) -- Install via CLI or manually add packages.
- [Project structure](/docs/next/introduction/project-structure) -- Nuxt-style folder layout and auto-discovery.
- [Capabilities](/docs/next/introduction/capabilities) -- Author small, focused capabilities using the DSL.
...
[`craft start`](/docs/next/reference/cli#start) boots the whole project from this layout, so an
...
use [`direct()`](/docs/next/advanced/composing-capabilities) with the callee's id, and import its
...
| `adapters/` | Custom adapters that connect to external systems, one folder per adapter. `index.ts` exposes the single factory; `source.ts`, `destination.ts`, and friends hold the operation implementations (`subscribe`, `send`, `process`). See the [custom adapters guide](/docs/next/advanced/custom-adapters). |
...
> **Warning: Config blocks replace, they do not merge**
>
...
- [Composing Capabilities](/docs/next/advanced/composing-capabilities) -- Reuse capabilities with direct() and exported contract types.
- [Configuration reference](/docs/next/reference/configuration) -- craft.config.ts options and context settings.
...
- [Operations reference](/docs/next/reference/operations) -- Full API: all operations with signatures and examples.
...
Returning the same `exchange` unchanged is still a valid no-op pass-through. For the full rationale and the drop-signalling helpers that moved off headers (`markDropped` / `isDropped`), see the [0.4 to 0.5 migration guide](/docs/next/migrating/0.4-to-0.5).
...
- [Operations](/docs/next/introduction/operations) -- What each operation does with the exchange body and headers.
...
`.suspend()` is the one flow-control operation that stops an exchange without ending it. It parks the exchange durably and answers the caller straight away with a `Suspended` value, because the answer it waits for (a human approval, an out-of-band decision) arrives in hours or days and no transport can be held open that long. `.resume()` is the other half: it addresses that parked exchange by its signed token, from any route on any transport, and re-enters the pipeline at the step after the suspend with the answer on `ex.suspension.result`. A route with a reachable suspend therefore has two shapes of output, its own and `Suspended`, and each source renders the second one its own way (`202` on HTTP, the value itself on `direct()`, an ack on a queue). See [suspend](/docs/next/reference/operations/suspend) and [resume](/docs/next/reference/operations/resume).

### Wrappers

Wrappers modify the behaviour of the **next operation only**. They do not stand alone -- they must be followed by the operation they wrap, placed immediately before it. Most are dual-mode: the same method called BEFORE `.from()` applies to the whole pipeline instead (see the [filter chain](/docs/next/advanced/filter-chain)).
...
- [Operations reference](/docs/next/reference/operations) -- Full API: all operations with signatures, options, and examples.
...
Options passed directly to the adapter always take precedence over config defaults. See the [Merged Options guide](/docs/next/advanced/merged-options) for the full pattern and a list of adapters that support it.

---

## Related

- [Adapters reference](/docs/next/reference/adapters) -- Full catalog with all options and signatures.
- [Creating adapters](/docs/next/advanced/custom-adapters) -- Build your own source, destination, enricher, or processor adapter.
...
- [Events reference](/docs/next/reference/events) -- Full event catalog with all payload shapes and filtering patterns.
...
- [Errors reference](/docs/next/reference/errors) -- RC error codes -- useful when asserting on t.errors in tests.
...
The two paths can be mixed within the same project. See the [Runtime reference](/docs/next/reference/runtime) for the rationale.
...
See the [Programmatic Invocation guide](/docs/next/advanced/programmatic-invocation) for the embedding API and runnable examples.

---

## Related

- [Runtime reference](/docs/next/reference/runtime) -- Bun-only CLI, Node embedding, version floors.
- [CLI reference](/docs/next/reference/cli) -- All craft CLI commands including run.
- [Programmatic Invocation](/docs/next/advanced/programmatic-invocation) -- Embed Routecraft inside Node, Express, or Next.js.
...
For the full event catalog see the [Events reference](/docs/next/reference/events).
...
See the [Terminal UI guide](/docs/next/introduction/tui) for navigation and options.

---

## Related

- [Events reference](/docs/next/reference/events) -- Full event catalog with payload shapes and identity fields.
- [Plugins](/docs/next/advanced/plugins) -- How to write and register plugins.
- [Terminal UI](/docs/next/introduction/tui) -- Browse routes, exchanges, and live events from the terminal.
...
See [Monitoring](/docs/next/introduction/monitoring#telemetry-plugin) for full plugin options.
...
- [Monitoring](/docs/next/introduction/monitoring) -- Logging, events, and the telemetry plugin.
- [CLI reference](/docs/next/reference/cli) -- All craft commands and options.
...
For the full pattern -- how merged options work, which adapters support them, and how to add support to a custom adapter -- see the [Merged Options guide](/docs/next/advanced/merged-options).
...
Plugins subscribe to events using `context.on(eventName, handler)`. Common events include `route:started`, `route:stopped`, `context:started`, `context:stopped`, and `error`. See the [Events reference](/docs/next/reference/events) for the full list.
...
- [Plugins reference](/docs/next/reference/plugins) -- Full API for plugin interfaces and context methods.
- [Monitoring](/docs/next/introduction/monitoring) -- Observability patterns built on plugins and events.
...
- [Composing Capabilities](/docs/next/advanced/composing-capabilities) -- Build modular systems with direct() and reusable capability chains.
- [Events](/docs/next/introduction/events) -- Subscribe to error and exchange lifecycle events.
...
> **Warning: Route-scope `.retry()` now re-runs continuation steps**
>
...
Plugins that manage additional concerns (like `llmPlugin` which also registers provider credentials) wrap `defaultOptions` inside a larger configuration object. See the [Plugins reference](/docs/next/reference/plugins) for the full options of each plugin.

The `direct` adapter also supports a context-level `channelType` to swap all endpoints from in-memory to a distributed implementation. See [Configuration](/docs/next/reference/configuration#direct).
...
- [Configuration](/docs/next/reference/configuration) -- Full CraftConfig reference including cron and direct fields.
- [Creating adapters](/docs/next/advanced/custom-adapters) -- Build your own source, destination, enricher, or processor adapter.
- [Plugins reference](/docs/next/reference/plugins) -- Full API for built-in plugin options.
...
- [direct adapter](/docs/next/reference/adapters/direct) -- The direct adapter whose endpoints DirectEndpointRegistry constrains.
- [Composing Capabilities](/docs/next/advanced/composing-capabilities) -- Build modular systems with direct() and reusable capability chains.
...
See the [testing guide](/docs/next/introduction/testing#mocking-external-adapters) for the consumer-side API.
...
See the [Merged Options guide](/docs/next/advanced/merged-options) for the full walkthrough and design rationale.

## Sharing state between adapters

Adapters can use the context store to share state, read global configuration set by plugins, or maintain connections across exchanges. See [Plugins](/docs/next/advanced/plugins) for how to populate the context store at startup.

---

## Related

- [Adapters](/docs/next/introduction/adapters) -- How adapters work and how to configure them.
- [Adapters reference](/docs/next/reference/adapters) -- Full catalog with all options and signatures.
...
The `craft` CLI is Bun-only (see the [Runtime reference](/docs/next/reference/runtime)). If your application must run on Node, embed `@routecraft/routecraft` directly: import the builder, define your routes, and run them inside your existing Node process. No CLI, no Bun.
...
- [direct adapter](/docs/next/reference/adapters/direct) -- The direct() adapter that powers programmatic dispatch.
- [CLI reference](/docs/next/reference/cli) -- craft run and other CLI commands.
- [Configuration](/docs/next/reference/configuration) -- ContextBuilder options and craftConfig.
...
A capability becomes an MCP tool when you use `mcp()` as its source: the tool name is the route's `.id()`, and the `.description()` and `.input()` schema live on the route builder so Routecraft can validate every call before any business logic runs. See the [MCP example](/docs/next/examples/mcp) for a complete, copyable capability, and the [`mcp()` adapter reference](/docs/next/reference/adapters/mcp) for the full option surface.
...
Start the server with `craft run`, then point your AI client at it. Anything reachable over the network must be authenticated: see [Securing capabilities](/docs/next/advanced/securing-capabilities) for every auth mode (`jwt()`, `jwks()`, custom validators, and `oauth()` as a resource-server gate), identity enrichment, RFC 9728 discovery metadata, and CORS.
...
- [Securing capabilities](/docs/next/advanced/securing-capabilities) -- Authenticate HTTP endpoints, enrich identity, RFC 9728, CORS.
- [MCP tool](/docs/next/examples/mcp) -- A copyable capability exposed as an MCP tool.
- [Calling an MCP](/docs/next/advanced/call-an-mcp) -- Call external MCP servers from within a capability.
- [mcp() adapter reference](/docs/next/reference/adapters/mcp) -- Full MCP adapter API and options.
...
Tiers 1 and 2 are covered on the [agent plugin reference](/docs/next/reference/plugins/agentplugin). For tier 3, put a route in front of the tool: its entry is a `direct()` endpoint, its exit is the `.to(mcp(...))` call you have already seen, and the guardrails live on the steps between.
...
Hand the agent the governed route instead of the raw tool. The same underlying tool can be exposed both ways: wrap the ones that need policy (one route per tool), leave harmless read-only tools raw. The same tiering applies when re-exposing client tools through your own MCP server: a plain [`mcpPlugin({ proxy })`](/docs/next/advanced/expose-as-mcp#proxying-tools-from-configured-clients) entry is the raw tier, a proxy entry with a `guard` is the guarded tier, and a `.from(mcp())` route is the governed tier.
...
Why a route and not a richer guard? A guard runs once and holds no state. Caching, timeouts, throttling, retries, and fallbacks each need something wrapped around the call with its own state and lifecycle, which is exactly what a route step is. Today a wrapped route gives you [`authorize()`](/docs/next/reference/operations/authorize), [`error()`](/docs/next/reference/operations/error) fallbacks, [`cache()`](/docs/next/reference/operations/cache), [`timeout()`](/docs/next/reference/operations/timeout), [`retry()`](/docs/next/reference/operations/retry), and `.tap(log())` for an audit trail. [`throttle()`](/docs/next/reference/operations/throttle) is planned; when it ships it drops onto the same route with no change to how the agent consumes the tool. The route is the only place that behaviour can ever live.

---

## Related

- [Running an MCP server](/docs/next/advanced/expose-as-mcp) -- Run your own capabilities as MCP tools for AI clients.
- [MCP tool](/docs/next/examples/mcp) -- A copyable capability exposed as an MCP tool.
- [mcp() adapter reference](/docs/next/reference/adapters/mcp) -- Full MCP adapter API and options.
...
When the agent should get the chance to correct itself before the dispatch resolves, move the same judge into the agent's [`validate`](/docs/next/reference/plugins/agentplugin) hook. Returning a string from `validate` sends the agent back for another turn with that string as a corrective message, conversation intact, sharing the `maxTurns` budget:
...
For wiring the server itself and pointing clients at it, see [Running an MCP server](/docs/next/advanced/expose-as-mcp). For a concrete, copyable capability, see the [MCP example](/docs/next/examples/mcp).
...
See the [mcpPlugin reference](/docs/next/reference/plugins/mcpplugin) for the full `Principal` field list.
...
- **Authorize per route** -- gate sensitive capabilities with [`authorize()`](/docs/next/reference/operations/authorize) against the verified principal's roles or scopes
- **Principle of least privilege** -- only expose capabilities the AI actually needs
- **Govern agent tool access** -- hand an agent a wrapped `Direct(...)` route instead of a raw `MCP(...)` tool when it needs authorization, caching, or timeouts; see [Calling an MCP](/docs/next/advanced/call-an-mcp#guardrails-raw-guarded-or-wrapped)
...
- [Running an MCP server](/docs/next/advanced/expose-as-mcp) -- Transports, client wiring, and server identity.
- [MCP tool](/docs/next/examples/mcp) -- A copyable capability exposed as an MCP tool.
- [mcpPlugin reference](/docs/next/reference/plugins/mcpplugin) -- Full plugin options and the Principal field list.
...
- [Linting reference](/docs/next/reference/linting) -- Full rule catalog with defaults and descriptions.
...
- [Linting](/docs/next/advanced/linting) -- Enforce Routecraft authoring best practices with ESLint.

- [Operations reference](/docs/next/reference/operations) -- The choice, split, and aggregate operations the plugin formats.
...
- [Operations](/docs/next/reference/operations) -- Verbs that act on the exchange between source and destination.
- [Configuration](/docs/next/reference/configuration) -- Project-level options and craft.config.ts.
- [Errors](/docs/next/reference/errors) -- Error codes, lifecycle, and recovery via .error().
...
{% operations-index channel="next" /%}

## Related

- [Adapters](/docs/next/reference/adapters) -- Sources, destinations, and transformers that connect operations to the outside world.
- [Events](/docs/next/reference/events) -- The lifecycle events emitted around every operation.
- [Errors](/docs/next/reference/errors) -- Error codes raised by operations and how to recover them.

# Events

Full catalog of lifecycle and runtime events emitted by the Routecraft context.

{% event-namespaces channel="next" /%}
...
| `route:exchange:suspended` | Exchange parked at a [`.suspend()`](/docs/next/reference/operations/suspend); execution one ends here | `{ routeId, exchangeId, correlationId, suspensionId, position, expiresAt? }` |
...
| `route:agent:tool:denied` | [`toolPolicy`](/docs/next/reference/plugins/agentplugin#tool-policy) refused a tool admission, so it was never offered to the model | `{ routeId, exchangeId, correlationId, agentName?, toolName, toolKind, reason }` |
...
- [Events](/docs/next/introduction/events) -- How to subscribe, filter by payload identity, emit custom events, and common patterns.
- [Configuration](/docs/next/reference/configuration) -- Subscribe to events via craft.config.ts.
...
Core adapters have dedicated config fields so you can set context-wide defaults without importing a plugin. See [Merged Options](/docs/next/advanced/merged-options) for how the merge hierarchy works.
...
Configures the HTTP server that backs the [`http()` source](/docs/next/reference/adapters#http). Setting this key starts a listener when the context starts (Bun.serve on Bun, `node:http` on Node 22+). See [httpPlugin](/docs/next/reference/plugins#httpplugin) for the full behaviour.
...
| `builtins` | `{ health?, ready?, openapi?: { enabled?: boolean; requireAuth?: boolean } }` | see [adapter reference](/docs/next/reference/adapters/http#configuring-built-ins) | Per-endpoint config for `/health`, `/ready`, `/openapi.json`. Uniform `{ enabled, requireAuth }` shape per built-in (inspired by Spring Boot Actuator). |

### suspension

Configures where parked exchanges are persisted, and the secret used to sign the resume tokens an approver is handed. Required as soon as any route can reach a [`.suspend()`](/docs/next/reference/operations/suspend): such a context refuses to start without it, with [`RC5052`](/docs/next/reference/errors#rc-5052). A context that never suspends does not need the key.
...
- [Events reference](/docs/next/reference/events) -- All lifecycle and runtime events available in the on field.
- [Plugins reference](/docs/next/reference/plugins) -- Full API for plugin interfaces and context methods.
- [Adapters reference](/docs/next/reference/adapters) -- All adapters, options, and signatures.
...
>
> The `craft` CLI runs on Bun (>=1.1.0). Node users should embed `@routecraft/routecraft` programmatically instead -- see [Programmatic Invocation](/docs/next/advanced/programmatic-invocation) and the [Runtime reference](/docs/next/reference/runtime).
...
The file must export a capability (or array of capabilities) as its default export, and optionally a `craftConfig` named export. See the [Configuration reference](/docs/next/reference/configuration) for the config export format.
...
Boot a whole project from its [folder convention](/docs/next/introduction/project-structure)
...
See [Programmatic Invocation](/docs/next/advanced/programmatic-invocation) for the full embedding guide, including Express, Next.js, and Commander integrations.
...
- [CLI reference](/docs/next/reference/cli) -- craft commands and options.
- [Programmatic Invocation](/docs/next/advanced/programmatic-invocation) -- Embed Routecraft inside Node, Express, or Next.js.
- [Installation](/docs/next/introduction/installation) -- System requirements and project setup.
...
{% plugin-index channel="next" /%}

{% callout %}
Core adapter defaults (`cron`, `direct`) are set via dedicated fields on `CraftConfig`, not via plugins. See [Configuration](/docs/next/reference/configuration) and [Merged Options](/docs/next/advanced/merged-options).
{% /callout %}
...
- [Configuration](/docs/next/reference/configuration) -- craft.config.ts and the merged options resolution order.
- [Adapters](/docs/next/reference/adapters) -- The connectors that plugins configure defaults for.
- [AI capabilities](/docs/next/advanced/composing-capabilities) -- Build the agent or expose capabilities to one.
...
explicitly. See [Capability boundaries](/docs/next/advanced/linting#capability-boundaries-opt-in).

---

## Related

- [Linting](/docs/next/advanced/linting) -- Install, configure, and customize rule severity.
...
The `Retry` column shows whether the [`retry`](/docs/next/reference/operations/retry) wrapper will retry this error by default. Codes marked `No` typically represent permanent failures (bad input, configuration errors) that won't succeed on retry.

{% error-table channel="next" /%}
...
- For in-route failures: configure `auth:` on the source (e.g. `mcp({ auth: jwt(...) })`) so the source emits a principal, or attach a custom principal in a `.process()` step before the `authorize()` validator runs. See [`.authorize()`](/docs/next/reference/operations/authorize).
...
- For in-route denials: grant the principal the missing role(s) at your IdP, or relax the `.authorize()` requirement. The error message lists the missing roles. See [`.authorize()`](/docs/next/reference/operations/authorize).
...
See [Running an MCP server](/docs/next/advanced/expose-as-mcp) for transports and client wiring,
and [Securing capabilities](/docs/next/advanced/securing-capabilities) when you serve it over HTTP.
...
See [Calling an MCP](/docs/next/advanced/call-an-mcp) for custom argument mapping and inline-URL
calls, and the [`mcp()` adapter reference](/docs/next/reference/adapters/mcp) for the full option
...
`{ name, guard, description }` adds a per-tool [guard](/docs/next/advanced/securing-capabilities) or
...
- [agent() adapter reference](/docs/next/reference/adapters/agent) -- Model, system, tools, blocks, and loop options.
- [Securing capabilities](/docs/next/advanced/securing-capabilities) -- Guards, principals, and authorizing what an agent can reach.
- [MCP tool](/docs/next/examples/mcp) -- Expose a capability as a tool an external agent can call.
...
See [Composing Capabilities](/docs/next/advanced/composing-capabilities) for fan-out, dynamic routing, and schema validation at the channel boundary.
...
For the complete list with options and signatures, see the [Adapters reference](/docs/next/reference/adapters).
...
See [Creating adapters](/docs/next/advanced/custom-adapters) for full examples including factory functions and multi-role adapters.

## How do I expose a capability as an MCP tool?

Use `mcp()` from `@routecraft/ai` as the source adapter and run the file with `craft run`. See the [MCP example](/docs/next/examples/mcp) and [Running an MCP server](/docs/next/advanced/expose-as-mcp).
...
1) Repository
- Push your app to GitHub/GitLab including `craft.config.ts` and the Bun Dockerfile from the [Deployment guide](/docs/next/introduction/deployment).
...
See [Programmatic Invocation](/docs/next/advanced/programmatic-invocation) for the embedding API.
...
Three behavioural notes that are not API changes: context store seeding for `cron`/`direct`/`mail` config now happens in `initPlugins()` (called automatically by `start()`) instead of the `CraftContext` constructor; plugin teardown plus `registerTeardown` callbacks now unwind in reverse (LIFO) order; and `.input()` validation now runs inside the [filter chain](/docs/next/advanced/filter-chain) (position #4) instead of eagerly in the consumer handler, so an invalid message is routable through the route-scope `.error()` handler and an unrecovered failure emits `route:exchange:failed` (previously `route:exchange:dropped`) while still rejecting the sender.
...
> The key returns in 0.7.0 with different semantics: it declares **where skills come from** (local paths and `npm:` package refs) rather than naming blocks, and the project runtime resolves it. See [`agents()`](/docs/next/reference/plugins/agentplugin#loading-agents-from-markdown-agents).
...
A `directTool` alias registered under a fn id reports `kind: "direct"`, not `"fn"`. It reaches the same route under a different name, so classifying it as a fn would make aliasing a way around [`toolPolicy`](/docs/next/reference/plugins/agentplugin#tool-policy).
...
If you never imported `HttpOptions` by name (the common case, since `http({...})` infers its argument type), no change is needed. See the [`http()` adapter reference](/docs/next/reference/adapters/http) for the new source surface.
...
- **HTTP source adapter.** `http({ path, method? })` exposes a route over HTTP, configured via `defineConfig({ http: { port, host, auth } })`. Bun runtimes bind via `Bun.serve`; Node 22+ uses a `node:http` shim. Global auth (`jwt()` / `jwks()` bearer or `apiKey({...})`), per-route `.authorize()`, built-in `/health`, `/ready`, and `/openapi.json` endpoints. See the [`httpPlugin`](/docs/next/reference/plugins/httpplugin) reference.
- **`multicast` operation.** `.multicast(...paths)` fans the exchange out to multiple independent paths in parallel (each on a deep clone), waits for all to settle, then continues the original unchanged. See the [`multicast`](/docs/next/reference/operations/multicast) reference.
...
- **`agentPlugin({ toolPolicy })`**: repository-wide admission rules for the agent tool surface, keyed by tool kind (`fn` / `direct` / `mcp`), each `true`, `false`, or a predicate. Omitting it admits everything, so existing contexts are unaffected; supplying it makes the surface an allowlist in which every kind must be decided explicitly (a partial policy is a compile error, because an omitted key would mean denial). Enforced at the single point every agent form converges on, so no agent can opt out. Denials emit `route:agent:tool:denied`. See [tool policy](/docs/next/reference/plugins/agentplugin#tool-policy).
...
[← All adapters](/docs/next/reference/adapters)
...
- **By name** (`agent("summariser")`) -- resolves a registered agent from the context. Register agents via `agentPlugin({ agents: { name: {...} } })` ([`agentPlugin` reference](/docs/next/reference/plugins/agentplugin)).
...
Provider credentials are configured once in `llmPlugin()` and shared across all `agent()` calls. See [`llmPlugin` reference](/docs/next/reference/plugins/llmplugin).
...
Only the loggable identity fields (`name`, `email`, `subject`) and `roles` are surfaced; fields that are absent on the principal are omitted, and interpolated values have newlines collapsed so a subject-controlled field (a self-service display name, say) cannot forge prompt structure. Scopes, `claims`, `userinfoClaims`, and the bearer token are never injected. The block is informational context only: authorization is still enforced by [`.authorize()`](/docs/next/reference/operations/authorize) and tool guards, never by the model.
...
| `onParseError` | `'fail' \| 'abort' \| 'drop'` | `'fail'` | How to handle a row parse failure (source role only). See [parse error handling](/docs/next/reference/adapters#parse-error-handling). |
...
See [Type Registries](/docs/next/advanced/type-registries) for how to populate `DirectEndpointRegistry`.
...
The direct registry stores only the direct adapter's own metadata. Other adapters that expose routes externally (such as [`mcp()`](/docs/next/reference/adapters/mcp) or a future inbound `http()`) maintain their own parallel registries; they are never written to or read from the direct registry.
...
Scan a directory and list its entries. The directory adapter is the "find the files" half of working with a directory; the [`file`](/docs/next/reference/adapters/file) adapter reads or writes a single file. Compose the two to process every file in a directory.
...
By default the source emits a single exchange whose body is the full `DirectoryEntry[]` listing, the same collection-in-one-exchange shape as the non-chunked [`csv`](/docs/next/reference/adapters/csv) and [`jsonl`](/docs/next/reference/adapters/jsonl) adapters. Pass `chunked: true` to emit one exchange per entry instead. Filtering is intentionally not built in either way: list the entries, then decide which ones.
...
**List mid-route:** the adapter's `fetch` returns the sorted `DirectoryEntry[]` listing, so you can list a directory partway through a route with `.enrich()` or `.to()`, the same way [`file`](/docs/next/reference/adapters/file)'s fetch pulls in a file's content. A listing is a read, so it belongs in the pull-in role. This is what makes the adapter usable inside a [`direct()`](/docs/next/reference/adapters/direct) capability, where there is no `.from()` slot to hang the listing on. The source role needs a static string path; the enricher also accepts a function, resolved against the exchange when the scan runs.
...
An enclosing [`.timeout()`](/docs/next/reference/operations/timeout) that expires mid-scan makes the fetch **throw** rather than return the entries it had collected: a truncated listing is indistinguishable from a complete directory, so it is never handed to the route as a body. The source is unaffected (an abort there simply stops emission).
...
Provider credentials are configured once in `embeddingPlugin()` and shared across all `embedding()` calls. See [Plugins reference](/docs/next/reference/plugins).
...
| `encoding` | `BufferEncoding` | `'utf-8'` | Text encoding |
| `createDirs` | `boolean` | `false` | Create parent directories (send role only) |
| `onParseError` | `'fail' \| 'abort' \| 'drop'` | `'fail'` | How to handle an extraction failure (source role only). See [parse error handling](/docs/next/reference/adapters#parse-error-handling). |
...
The server, port, host, and global auth live on [`defineConfig({ http })`](/docs/next/reference/configuration#http), not on the source. Routes only declare which request they want.
...
RC5039 applies only to the built-in `signature` gate; it is documented on the [errors reference](/docs/next/reference/errors#rc-5039).
...
See [HTTP plugin events](/docs/next/reference/events#http-plugin-events) on the events reference.
...
| `onParseError` | `'fail' \| 'abort' \| 'drop'` | `'fail'` | How to handle a parse failure (source role only). See [parse error handling](/docs/next/reference/adapters#parse-error-handling). |
...
| `onParseError` | `'fail' \| 'abort' \| 'drop'` | `'fail'` | How to handle a line parse failure (source role only). See [parse error handling](/docs/next/reference/adapters#parse-error-handling). |
...
Provider credentials are configured once in `llmPlugin()` and shared across all `llm()` calls. See [Plugins reference](/docs/next/reference/plugins).
...
| `onParseError` | `'fail' \| 'abort' \| 'drop'` | `'fail'` | How to handle a per-message MIME parse failure. See [parse error handling](/docs/next/reference/adapters#parse-error-handling). All three modes mark the malformed message Seen so it does not refetch forever. `'fail'` routes the failure through the route's `.error()` handler (or `exchange:failed` if no handler is set). `'drop'` does NOT invoke `.error()`; it emits `exchange:dropped` with `reason: 'parse-failed'` so subscribers can count parse drops as a structured event without scraping logs. Pre-#187 behaviour was equivalent to a silent `'drop'` (logged at debug, no event); set `onParseError: 'drop'` to keep lossy-ingest semantics with structured observability. |
...
Two checks stand behind that promise. The route's own output validation runs first and covers every result it completes, reporting a violation as [RC5002](/docs/next/reference/errors#rc-5002). The server then checks anything that reached it without passing through that validation, and reports those as [AI2001](/docs/next/reference/errors#ai-2001). A body the route already validated is not checked twice: validation replaces the body with the schema's output, so a schema that transforms (`z.string().transform(...)`, `.pipe()`) would reject the value it just produced.

A tool whose route declares no `.output()` advertises no schema, so nothing is checked and its result passes through as-is.

**Declined calls:**

A route that drops the exchange (a `.filter()` rejecting, a `.choice()` matching no branch, an error handler returning `recovery.drop()`) has produced no result. The call comes back as `isError: true` saying the tool declined the request (`AI2002`), which mirrors [RC5031](/docs/next/reference/errors#rc-5031) on the direct and forward surfaces. This applies to every tool, declared output or not.
...
See [Running an MCP server](/docs/next/advanced/expose-as-mcp), [Calling an MCP](/docs/next/advanced/call-an-mcp), and the [MCP example](/docs/next/examples/mcp).
...
See [Testing](/docs/next/introduction/testing) for full usage patterns.
...
| `onParseError` | `'fail' \| 'abort' \| 'drop'` | `'fail'` | No | How to handle a parse failure (source role only). See [parse error handling](/docs/next/reference/adapters#parse-error-handling). |
...
[← All operations](/docs/next/reference/operations)
...
This is the explicit way to establish identity from a source the framework cannot verify on its own. `authorize()` trusts only principals minted this way (or attached by a source verifier such as `jwt()` / `jwks()` / `oauth()`); a plain object written via `.header('routecraft.auth.principal', ...)` or `.process()` is rejected with [`RC5023`](/docs/next/reference/errors#rc-5023). Sugar over the `authenticate()` helper, which you can call directly in tests, custom source adapters, or a `.choice()` branch.

Only `subject` is required; `kind` defaults to `"custom"` and `scheme` to `"custom"`.

`.authenticate()` establishes **identification**: who the caller is. It answers nothing about delegation. When an agent should act on the authenticated caller's behalf, follow it with [`.delegate()`](/docs/next/reference/operations/delegate), which marks the actor and narrows scopes under a consent record; minting a principal from a channel identifier alone and handing it to an agent gives the agent the caller's full authority with no consent trail.
...
- **No principal on the exchange:** throws [`RC5012`](/docs/next/reference/errors#rc-5012). The source did not authenticate (no `auth:` configured) and no `.authenticate()` step ran before the check.
- **Principal not authentic (self-asserted object):** throws [`RC5023`](/docs/next/reference/errors#rc-5023).
- **Actor present but not admitted (or required but absent):** throws [`RC5034`](/docs/next/reference/errors#rc-5034).
- **Subject constraint failed:** throws [`RC5035`](/docs/next/reference/errors#rc-5035).
- **Delegation chain deeper than `maxDelegationDepth`:** throws [`RC5036`](/docs/next/reference/errors#rc-5036).
- **Missing role or failed predicate:** throws [`RC5015`](/docs/next/reference/errors#rc-5015). Permanent: no ceremony changes who the subject is.
- **Missing scope:** throws [`RC5038`](/docs/next/reference/errors#rc-5038). Recoverable: the cause carries `missing.scopes` so a consent flow can request exactly what is absent.
...
>
> The `actor` default is `'none'`. Routes written before delegation existed keep exactly their old behavior for direct callers, but a principal carrying an actor (minted by [`.delegate()`](/docs/next/reference/operations/delegate) or parsed from a token's `act` claim) is rejected with RC5034 until the route declares its permitted actor(s). This is deliberate: a capability is not agent-reachable unless it says so.
> 
...
>
> Use the ESLint rule `@routecraft/routecraft/batch-before-from` to ensure `batch()` is placed **before** `.from()`. See [Linting Rules](/docs/next/reference/linting#batch-before-from).
...
The breaker emits the `route:circuitBreaker:*` family. See the [events reference](/docs/next/reference/events) for payload shapes. `scope` is `"route"` when declared before `.from()` and `"step"` for the wrapper after it.
...
The bulkhead emits the `route:concurrency:*` family. See the [events reference](/docs/next/reference/events) for payload shapes. `scope` is `"route"` when declared before `.from()` and `"step"` for the wrapper after it.
...
**Events:** `route:delay:started` when the wait begins; `route:delay:stopped` when it ends (with `elapsed` and the `cancelled` flag). See the [events reference](/docs/next/reference/events).
...
The distinction this operation establishes is the core of the delegation model (RFC 8693): `subject` stays the party the action is taken **on behalf of**, `actor` becomes the party **performing** it. A route can then distinguish the three cases with one [`authorize()`](/docs/next/reference/operations/authorize) grammar: a person acting directly (no actor), an agent acting for a person (subject person, actor agent), and an agent acting under its own standing authority (subject agent, no actor).
...
- **Resolver returns `undefined` (no consent record):** by default the subject's direct principal is STRIPPED and the exchange continues anonymous, so downstream [`authorize()`](/docs/next/reference/operations/authorize) refuses with [`RC5012`](/docs/next/reference/errors#rc-5012). Drop is the default because the step marks the boundary where a request starts acting THROUGH someone else: passing the principal through would hand the continuation the caller's full direct authority precisely when consent is absent, which is the fail-open confused-deputy shape this operation exists to prevent. The strip is narrow: anonymous exchanges, already-delegated principals, and autonomous agent subjects (`subjectProfile: 'ai_agent'`) pass through untouched. Pass `{ otherwise: 'keep' }` when the continuation serves the caller directly (delegation was an optional enhancement, not the authority boundary) and an ungranted caller should keep acting as themselves.
- **Resolver returns a directive but the exchange is anonymous:** [`RC5012`](/docs/next/reference/errors#rc-5012). Delegation transforms an existing identity; it never creates one.
- **Subject principal is not authentic:** [`RC5023`](/docs/next/reference/errors#rc-5023). A chain cannot be built on a self-asserted object.
- **Subject's `mayAct` does not permit this actor:** [`RC5037`](/docs/next/reference/errors#rc-5037). Matching uses the `(issuer, subject)` pair. `mayAct` travels with the subject, so it gates every hop: re-delegation to a second agent is checked against the same consent list, which makes delegation non-transitive by default.

Delegation state can only be established here. [`authenticate()`](/docs/next/reference/operations/authenticate) rejects `actor` and `grantId` claims with [`RC5024`](/docs/next/reference/errors#rc-5024), so spreading a delegated principal back through a mint cannot fabricate a chain while skipping the `mayAct` check and the scope intersection. (`mayAct` itself is accepted at mint: it describes the subject, like roles, and is legitimately established when identity is resolved from a directory or grant store.)
...
For the autonomous case there is nothing to delegate: mint the agent as its own subject with [`.authenticate()`](/docs/next/reference/operations/authenticate) on an internal trigger (for example `cron()`), and gate capabilities with `authorize({ subject: { profile: 'ai_agent' } })`.
...
When the route declares `.input()` with a body schema before `.from()`, `T` defaults to the schema's inferred output type and the generic can be omitted; see [input](/docs/next/reference/operations/input). An explicit `.from<T>()` still overrides it.
...
Declare input validation for the next route. The engine validates the incoming body and headers against these schemas at [filter chain](/docs/next/advanced/filter-chain) position #4: after `authorize` and `parse`, before any resilience wrapper or user step, so the pipeline never sees an invalid message. A failure throws `RC5002`, which the route-scope `.error()` handler can observe and recover; unrecovered it takes the normal error path (`route:error`, `context:error`, `exchange:failed`) and rejects the sender (e.g. a `direct()` caller). Accepts either a bundle (`{ body, headers }`) or a bare Standard Schema as a body-only shorthand.
...
Revive an exchange parked by [`.suspend()`](/docs/next/reference/operations/suspend) and run its continuation.
...
A principal that came back from the store with the parked exchange is marked restored and refused by `authorize()` with [`RC5043`](/docs/next/reference/errors#rc-5043): it is a recorded shape with no live credential behind it. Re-verify after resume, or authorize here.
...
| Code | Cause |
|------|-------|
| [`RC5041`](/docs/next/reference/errors#rc-5041) | The token is malformed or its signature does not verify. |
| [`RC5046`](/docs/next/reference/errors#rc-5046) | The token verifies but the store holds no such suspension, or its route is not registered in this context. |
| [`RC5047`](/docs/next/reference/errors#rc-5047) | The suspension's `ttl` elapsed. |
| [`RC5048`](/docs/next/reference/errors#rc-5048) | The steps after the suspend point (or the `expect` schema) changed while the exchange was parked, so the stored approval no longer authorizes what would run. Refused before any of those steps execute. |
| [`RC5049`](/docs/next/reference/errors#rc-5049) | The answer does not satisfy `expect`. Ingress route only. The suspension stays resumable, so a corrected answer still works. |
| [`RC5050`](/docs/next/reference/errors#rc-5050) | The suspension was denied, typically because the run carrying it was cancelled. |

## Related

- [`.suspend()`](/docs/next/reference/operations/suspend) -- the other half.
- [`.authorize()`](/docs/next/reference/operations/authorize) -- guarding the ingress.
- [Configuration → suspension](/docs/next/reference/configuration#suspension) -- the store and the signing secret.
...
See the [errors reference](/docs/next/reference/errors) for which errors are retryable by default.
...
**Events:** `route:retry:started` when the guarded execution begins, `route:retry:attempt` before each backoff wait and re-attempt (with `attemptNumber`, the actual `backoffMs`, and `lastError`), `route:retry:stopped` on final success or failure. Payloads carry `scope: "route" | "step"`. See the [events reference](/docs/next/reference/events).
...
Route-scope `.retry()` sits at position 7 of the [filter chain](/docs/next/advanced/filter-chain): outside `.timeout()` (each attempt gets its own deadline) and inside `.error()` (the handler sees the final attempt's failure, not every intermediate one). Builder call order does not matter; the framework fixes the chain order.
...
Park the exchange durably and exit the pipeline. The run that reaches a `.suspend()` ends there and answers immediately; the exchange continues from the next step later, when [`.resume()`](/docs/next/reference/operations/resume) arrives with the answer.
...
| Expiry handling | `ttl` plus a route-scope `.error()`; an expired answer arrives as [`RC5047`](/docs/next/reference/errors#rc-5047) |
...
`.suspend()` is refused at `craft()` build time where the framework could not revive the exchange, with [`RC5051`](/docs/next/reference/errors#rc-5051):
...
- **Under a step-scope wrapper** (`.retry()`, `.timeout()`, `.cache()`, …), which fails with [`RC5003`](/docs/next/reference/errors#rc-5003). Parking is not a failure to re-attempt. Put `.error()` at route scope instead, where it also catches revival failures.
- **On a route with route-scope `.cache()`**, also [`RC5003`](/docs/next/reference/errors#rc-5003). The cache filters wrap the user pipeline, which a park exits and a resume re-enters partway down, so neither the check nor the store would ever run and the cache would silently do nothing. Use a step-scope `.cache()` on the expensive step instead.

Two more refusals happen outside build time, each as early as it can be known and both well before a resume:

- A context whose routes can reach a suspend (or a [`.resume()`](/docs/next/reference/operations/resume)) but which configured no [`suspension`](/docs/next/reference/configuration#suspension) block fails at **startup** with [`RC5052`](/docs/next/reference/errors#rc-5052).
- An exchange holding anything that is not plain JSON data (a function, a class instance, a `Secret`) fails at **suspend time**, with [`RC5042`](/docs/next/reference/errors#rc-5042) naming the offending path. Deliberately at the park rather than at the resume: the deploy that introduced the value is what should fail, not the approver's click days later.

> **Warning: Durable from the suspend point onward**
>
...
- [`.resume()`](/docs/next/reference/operations/resume) -- the other half.
- [Configuration → suspension](/docs/next/reference/configuration#suspension) -- where parked exchanges are stored and how tokens are signed.
- [Events](/docs/next/reference/events) -- `route:exchange:suspended`, `:resumed`, `:expired`.
...
- **Before `.from()` (route scope):** rate-limits the whole pipeline at [pre-from filter chain](/docs/next/advanced/filter-chain) position 5, outside the resilience wrappers, so a throttled request never reaches `.retry()` / `.timeout()`. The gate runs before the cache check, so a paced request does not consume a cache lookup until it is admitted.
...
**Events:** `route:timeout:started` when the guarded execution begins, `route:timeout:stopped` when it settles in time, `route:timeout:expired` when the deadline fires (followed by the `RC5011` throw). Payloads carry `scope: "route" | "step"`. See the [events reference](/docs/next/reference/events).
...
Route-scope `.timeout()` sits at position 8 of the [filter chain](/docs/next/advanced/filter-chain): inside route-scope `.retry()` (each attempt gets its own deadline) and outside the cache check (a cache hit counts as a fast success and never expires). Builder call order does not matter; the framework fixes the chain order.
...
[← All plugins](/docs/next/reference/plugins)
...
See the [`agent`](/docs/next/reference/adapters/agent) adapter for usage patterns.
...
> **Warning: A deny list needs an allow list**
>
...
When to hand an agent a raw `MCP(...)` tool versus a wrapped `Direct(...)` route -- and why a guard cannot stand in for the wrap -- is covered in [Calling an MCP](/docs/next/advanced/call-an-mcp#guardrails-raw-guarded-or-wrapped).
...
Hard-coded assertions like the one above cover mechanical requirements ("this tool must have been called"). To judge whether the agent achieved the *outcome* the request asked for, pair `toolCalls` with a second model call: see [Judging Agent Results](/docs/next/advanced/judging-agent-results).
...
See [`embedding` adapter](/docs/next/reference/adapters/embedding) for usage.
...
Serves routes over HTTP. Backs the [`http()` source](/docs/next/reference/adapters/http); routes declare `.from(http({ path, method }))` and the plugin owns the listener, the port, and the global auth check. Bun runtimes bind via `Bun.serve`; Node 22+ uses a `node:http` shim. Zero runtime dependencies.
...
| `builtins` | `{ health?, ready?, openapi?: { enabled?: boolean; requireAuth?: boolean } }` | see below | No | Per-endpoint config for `/health`, `/ready`, `/openapi.json`. Each takes the same `{ enabled, requireAuth }` shape. See [Configuring built-ins](/docs/next/reference/adapters/http#configuring-built-ins) on the adapter reference for defaults and the per-endpoint behaviour table. |

Per-route authorization uses the existing [`.authorize({ roles, scopes })`](/docs/next/reference/operations/authorize) builder. A route relaxes the global check with `http({ auth: "optional" })` (admit anonymous, attach principal when a valid token is present) or `http({ auth: "skip" })` (bypass the middleware entirely). See [Auth modes](/docs/next/reference/adapters/http#auth-modes) on the adapter reference for the full matrix. Built-in endpoints `/health`, `/ready`, and `/openapi.json` are served unless a user route claims the same path.
...
- A bind failure (`EADDRINUSE` / `EADDRNOTAVAIL`) surfaces as [`RC5019`](/docs/next/reference/errors#rc-5019). The plugin resets its store flags so a retry on the same context starts clean.

## Events

See [HTTP plugin events](/docs/next/reference/events#http-plugin-events) for the full list. The plugin also re-uses the framework's `auth:success` / `auth:rejected` events with `source: "http"`.

## Related

- [`http()` adapter](/docs/next/reference/adapters/http) -- both source and destination overloads.
- [Configuration](/docs/next/reference/configuration#http) -- the `http` first-class config key.
- [`.authorize()`](/docs/next/reference/operations/authorize) -- per-route role/scope/predicate checks.
...
See [`llm` adapter](/docs/next/reference/adapters/llm) for usage.
...
| `icons` | `McpIcon[]` | Routecraft logo | `serverInfo.icons`, inherited by tools that set none of their own; pass `[]` to omit. See [Server identity and branding](/docs/next/advanced/expose-as-mcp#server-identity-and-branding). |
...
| `cors` | `false \| McpCorsOptions` | loopback-only | CORS for the HTTP transport. Default reflects loopback `Origin` headers; set to `false` to disable or `{ origin }` to allowlist production browser clients. See [Securing capabilities -> CORS](/docs/next/advanced/securing-capabilities#cors). |
...
Proxied calls dispatch over the client's registered transport and auth, and the remote result (`content`, `structuredContent`, `isError`) passes through verbatim. The caller's authenticated principal is not forwarded, and no route pipeline runs (no `authorize()`, validation, or resilience wrappers); a per-entry `guard` covers identity and role checks. Reserve raw `proxy` entries for simple, read-only tools; put anything needing stateful guardrails behind a `.from(mcp())` route. See [Running an MCP server -> Proxying tools from configured clients](/docs/next/advanced/expose-as-mcp#proxying-tools-from-configured-clients).

See [Running an MCP server](/docs/next/advanced/expose-as-mcp), [Calling an MCP](/docs/next/advanced/call-an-mcp), and [Securing capabilities](/docs/next/advanced/securing-capabilities) for usage guides.
