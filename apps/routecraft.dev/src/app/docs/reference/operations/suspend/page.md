...
A resume runs the continuation below **most** of the [pre-from filter chain](/docs/advanced/filter-chain#a-resumed-exchange-re-enters-below-the-chain). Route-scope `.error()`, `.retry()`, `.timeout()` and `.concurrency()` apply to execution two; `authorize`, `parse`, `input`, `throttle` and `circuitBreaker` do not, because they describe an exchange arriving at the route and this one arrived once. `cache` is refused at build alongside a reachable suspend. For a position that stays off, declare it inside the continuation with a step-scope wrapper, or hand the work to a route that has its own chain with `.to(direct('...'))`.
