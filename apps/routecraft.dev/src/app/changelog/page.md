...
- **CSV appends no longer splice records together** -- repeated `.to(csv({ append: true }))` writes previously produced `a,bc,d` from `a,b` and `c,d`. Appends are also serialised per path, so concurrent writes can no longer both emit the header.
