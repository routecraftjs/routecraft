...
Until then, the concepts stand on their own, and the order of operations from capability selection applies directly: memory and the gap logger first, read-only capabilities next, and let the agent help write its own roadmap.
