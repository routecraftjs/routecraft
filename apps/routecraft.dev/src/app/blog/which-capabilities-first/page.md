...
The pattern in production at a major European bank: incident triage. Score it against the filters. Incidents arrive daily across many teams (frequency times toil: high). Diagnosis today means a ticket system, a log store, a tracing UI, API specs, the repository, and two databases (fragmented context: maximal). Every capability in the suite (ticket context, logs by trace id, trace walking, spec validation, code lookup, datastore sync checks) is a deterministic read (filters 1 and 5: clean pass). Juniors use them to learn triage, seniors to skip tabs, an agent to chain the whole diagnosis (dual-use: the full three consumers). The backing systems all spoke service accounts already (filter 6: pass).
