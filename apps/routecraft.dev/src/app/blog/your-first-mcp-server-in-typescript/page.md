...
This post sticks to stdio because that is the fastest path to a working setup. Once you are happy with the shape, the [Clerk](/blog/securing-mcp-with-clerk) and [WorkOS](/blog/securing-mcp-with-workos) posts cover putting auth in front of an HTTP version of the same server.
...
- **Go to HTTP, with auth.** When you want this reachable from anywhere, not just your laptop, follow the [Clerk MCP auth](/blog/securing-mcp-with-clerk) or [WorkOS MCP auth](/blog/securing-mcp-with-workos) posts.
