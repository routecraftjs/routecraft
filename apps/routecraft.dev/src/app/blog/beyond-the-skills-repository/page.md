...
Stage 4 is the shape we are building with a major European bank right now: read-only capabilities that gather the facts of an incident behind the corporate login, so a junior can learn the runbook from them, a senior can skip six browser tabs, and an agent can chain the lot in the first minutes. It began as one engineer's personal automation and is a funded project with multiple contributors today. The skills repo explains triage; the capabilities perform it; the same access rules govern both. And the shape is not a banking shape: the same pattern fits a logistics team tracing shipments, a law firm assembling case files, or a clinic reconciling appointments against billing. We make the architectural argument behind this layer in [AI agents are still single-player](/blog/ai-agents-are-still-single-player).
...
Stage 4 is the layer [Routecraft](/docs/introduction) exists to provide: capabilities as code, deployed once, governed by the identity layer you already have.
