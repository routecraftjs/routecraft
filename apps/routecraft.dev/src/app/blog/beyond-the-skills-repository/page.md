...
A worked example of what lives on rung 4, from production at a major European bank: a read-only incident-triage suite (ticket context, logs by trace id, distributed-trace walking, API-spec validation, code lookup, datastore sync checks) used simultaneously by juniors learning the runbook, seniors skipping six browser tabs, and an agent that chains the lot. The skills repo explains triage; the capabilities perform it; the same access rules govern both.
