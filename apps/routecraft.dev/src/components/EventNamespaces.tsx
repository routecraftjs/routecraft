...
export function EventNamespaces() {
  ...
  return (
    <div className="not-prose mt-8 border border-ink/15 bg-paper-deep/30 dark:border-paper/15 dark:bg-ink-soft/30">
      <header className="flex items-center gap-3 border-b border-ink/15 px-5 py-3 dark:border-paper/15">
      ...
      <ul role="list" className="divide-y divide-ink/10 dark:divide-paper/10">
        {groups.map((g) => (
          <li key={g.pattern} className="px-5 py-4">
            <div className="flex items-baseline gap-3">
              ...
              >
                ...
                <code className="font-mono text-[0.9rem] font-medium text-ink transition group-hover:text-cobalt-500 dark:text-paper dark:group-hover:text-cobalt-300">
                  {g.pattern}
                </code>
              </a>
              <span className="ml-auto font-mono text-[0.6rem] tracking-[0.18em] text-ink/45 uppercase tabular-nums dark:text-paper/45">
                {g.events.length}
              </span>
            </div>
            <p className="mt-1.5 pl-6 font-mono text-[0.78rem] leading-[1.7] text-ink/65 dark:text-paper/65">
              {g.events.map((e, i) => (
                <span key={e}>
                  {i > 0 && (
            ...
            {g.note && (
              <p
                className="mt-1.5 pl-6 font-editorial text-[0.85rem] text-ink/55 italic dark:text-paper/55"
