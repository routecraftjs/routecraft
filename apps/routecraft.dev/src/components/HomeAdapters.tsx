    ...
    <section>
      <div className="mx-auto max-w-7xl px-4 pt-12 pb-20 sm:px-6 lg:px-8 lg:pt-14 lg:pb-24">
        <header className="max-w-3xl">
          ...
          <p className="mt-4 max-w-2xl text-[1rem] leading-[1.7] text-ink/65">
        ...
        <div className="mt-12 grid grid-cols-1 gap-px border border-ink/15 bg-ink/15 sm:grid-cols-2 lg:grid-cols-5">
          {columns.map((col) => (
            <div key={col.label} className="flex flex-col bg-paper p-5">
              <header className="flex items-baseline justify-between gap-2 border-b border-ink/15 pb-3">
                <h3 className="font-mono text-[0.65rem] tracking-[0.22em] text-ink/65 uppercase">
                  {col.label}
                </h3>
                <span className="font-mono text-[0.6rem] tracking-[0.22em] text-ink/40 tabular-nums">
                ...
                {col.items.map((item) => (
                  <li key={`${col.label}-${item.name}`}>
                    ...
                    >
                      ...
                      <span
                        aria-hidden="true"
                        className="font-mono text-[0.8rem] text-ink/25 opacity-0 transition group-hover:translate-x-0.5 group-hover:text-cobalt-500 group-hover:opacity-100"
                      >
                        →
                      </span>
                    </Link>
        ...
        <p className="mt-6 max-w-2xl text-[0.95rem] text-ink/60 italic">
          ...
          <Link
            ...
            className="border-b border-current pb-px font-editorial text-cobalt-500 hover:text-cobalt-600"
          >
            write your own
          </Link>
