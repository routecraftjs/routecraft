...
- [ ] A fixable vulnerability the `security` gate reports is fixed, not waived, whenever the fix is ours to make (a lockfile bump or a widened peer range). A waiver in `.trivyignore.yaml` states why it cannot be fixed here, is scoped to its package with `purls`, and expires within 30 days. A Dockerfile this repository ships follows the image rules. See [`.standards/security.md` § 13](.standards/security.md#13-vulnerabilities-and-the-supply-chain)
...
- [ ] A change to the release path (freeze, generators, prerender list, Dockerfile) is exercised locally as a production build: `bun scripts/freeze-docs.ts <tag>`, `docker build`, `docker run`, then `bun scripts/freeze-docs.ts --restore`. Commit docs edits first: `--restore` checks the pinned paths out of git and discards uncommitted changes to them
