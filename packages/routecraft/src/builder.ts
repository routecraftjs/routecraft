...
import { resolveSuspendSites, resumeDoors } from "./suspension/sites.ts";
import {
  type Splitter,
...
> extends StepBuilderBase<S> {
    ...
    if (this.pendingOptions !== undefined) {
      throw rcError("RC2001", undefined, {
        message:
          ...
          `.cache / .retry / .timeout / .throttle / .circuitBreaker / .concurrency) must be ` +
  ...
  build(): RouteDefinition[] {
    ...
    for (const route of this.routes) {
      ...
      const doors = resumeDoors(route);
      if (doors.length > 0) {
        route.usesResume = true;
        route.resumeDoors = doors;
      }
   ...
   * `.throttle()`, `.circuitBreaker()`, `.concurrency()`, `.cache()`,
   * `.delay()`) was staged but the user is starting a
  ...
  private assertNoPendingWrappers(method: string): void {
    if (this.pendingStepWrappers.length > 0) {
      throw rcError("RC2001", undefined, {
        message:
          `Wrapper(s) staged via .error() / .retry() / .timeout() / .throttle() / .circuitBreaker() / .cache() / .delay() / .concurrency() but no step followed before .${method}(). ` +
