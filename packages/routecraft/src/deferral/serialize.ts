 ...
 *
 * Three rules are enforced here, at defer time, so a violation surfaces
 ...
 *    `auth/restored.ts` for why deferral is the direct route back into
 *    #355's bug class.
 *
 * ## The wire form
 *
...
function encode(
  ...
  if (
    ...
    // A Date carrying its own properties would defer fine and resume with
...
function decode(value: unknown): unknown {
  ...
  if (entries.length === 1 && entries[0]?.[0] === DATE_TAG) {
    ...
    if (Number.isNaN(revived.getTime())) {
      // Not `refuse()`: that phrases everything as a defer-time failure,
  ...
  // that worked before the defer). `defineProperty` writes the key
...
function refuse(path: string, what: string): Error {
  return rcError("RC5042", undefined, {
    message: `Cannot defer: exchange ${path} holds ${what}, which cannot be persisted to the deferral store.`,
  });
}
