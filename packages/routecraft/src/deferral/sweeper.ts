...
/**
 * How long settled deferrals are kept before the sweeper purges them,
 * when unconfigured. `"never"` opts out for audit deployments.
 */
export const DEFAULT_DEFERRAL_RETENTION = "90d";
...
/**
 * Deferrals retired per `findExpired` call.
 *
 ...
 * token for a deferral that timed out, so without something firing on a
 ...
 * compare-and-swap a late resume competes for, so a deferral resumed at
...
export class DeferralSweeper {
  ...
  /**
   * Retire every deferral already overdue at `now`.
   *
      ...
      for (const deferral of due) {
        ...
        if (deadline === undefined) {
          ...
          this.context.logger.error(
            ...
            "findExpired returned a deferral without a deadline; aborting this sweep pass.",
          );
          return retired;
        }
        cursor = { expiresAt: deadline, id: deferral.id };
        visited++;
        const route = this.context.getRouteById(deferral.routeId);
        if (!route) {
        ...
        try {
          const { cas } = await expireDeferral(
            this.context,
            this.store,
            route,
          ...
          // them.
          this.context.logger.error(
            { err, deferralId: deferral.id, routeId: deferral.routeId },
            "Failed to retire an expired deferral; continuing the sweep.",
          );
        }
      }

      if (due.length < SWEEP_BATCH) break;
      this.context.logger.info({ retired }, "Still retiring expired deferrals");
    }
  ...
  private async purgeOnCadence(now: Date): Promise<void> {
    ...
    try {
      ...
      if (purged > 0) {
        this.context.logger.info(
          { purged },
          "Purged settled deferrals past retention.",
        );
      }
    } catch (err) {
      this.context.logger.error(
        { err },
        "Failed to purge settled deferrals; the next cadence will retry.",
      );
    }
  }
  ...
  private reportMissingRoutes(missing: Map<string, number>): void {
    ...
    if (overflowRoutes > 0) {
      this.context.logger.warn(
        ...
        "Further absent routes hold expired deferrals this context left alone.",
      );
    }
  }
   ...
   * loud-degradation case the deferral config already fails on.
   *
   * @internal
   */
  private async runStartScan(): Promise<void> {
    ...
    this.context.logger.info(
      {
        ...
        ...(summary.oldest !== undefined
          ? { oldestDeferredAt: summary.oldest.toISOString() }
          : {}),
        stranded: stranded.length,
        ...(stranded[0] !== undefined
          ? { oldestStrandedAt: stranded[0].deferredAt.toISOString() }
          : {}),
      },
    ...
    if (stranded.length > 0) {
      ...
      this.context.logger.warn(
        { stranded: stranded.length },
        "Deferrals were resumed but never recorded an outcome, so a process died mid-continuation. Their approvals are spent and nothing will retry them; each needs an operator.",
      );
      this.context.logger.debug(
        { deferralIds: stranded.map((record) => record.id) },
        "Stranded deferral ids",
      );
    }
  }
