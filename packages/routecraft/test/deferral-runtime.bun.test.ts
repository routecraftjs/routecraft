  ...
  node: () =>
    Promise.reject(
      rcError("RC5017", undefined, {
        message:
          'deferral store (sqlite) requires the optional peer dependency "better-sqlite3".',
      }),
    ),
};
...
describe("deferral runtime resolution", () => {
  ...
  beforeEach(() => {
    for (const key of [DEFERRAL_STORE_ENV, DEFERRAL_SECRET_ENV]) {
  ...
  test("reads the store location from the environment", async () => {
    process.env[DEFERRAL_STORE_ENV] = join(scratch, "from-env.db");
    runtime = await createDeferralRuntime(new CraftContext(), SECRET);
    expect(runtime.backend).toBe("sqlite");
  });
  ...
  test("treats a blank store environment variable as unset", async () => {
    process.env[DEFERRAL_STORE_ENV] = "   ";
    runtime = await createDeferralRuntime(new CraftContext(), {
      ...SECRET,
   ...
   * @preconditions A DeferralStore instance passed as the store
   * @expectedResult That instance is used verbatim
   */
  test("accepts a custom store instance", async () => {
    const custom = new MemoryDeferralStore();
    runtime = await createDeferralRuntime(new CraftContext(), {
      ...SECRET,
      store: custom,
    });
   ...
   *   a route that may never defer must not be blocked by a missing peer
   */
  test("falls back to memory with a warning when no driver is available", async () => {
    ...
    runtime = await createDeferralRuntime(context, {
      ...SECRET,
      loaders: absentPeerLoaders,
    });
