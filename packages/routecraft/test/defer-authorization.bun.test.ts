...
describe("the resume authorize hook", () => {
    ...
    t = await testContext()
      ...
      .routes([
        craft()
          .id("payout")
          .from(direct())
          .defer({ schema: Approval, ttl: "30ms" })
          .to(noop()),
        craft()
          .id("answers")
          .from(direct())
  ...
  test("meta round-trips from park to hook, without the body", async () => {
    ...
    expect(view["deferredAt"]).toBeInstanceOf(Date);
    ...
    t = await testContext()
      ...
      .routes([
        craft()
          .id("payout")
          .from(direct())
          .defer({ schema: Approval, meta: { hook: () => true } })
          .to(noop()),
      ])
      .build();
    await t.startAndWaitReady();
...
describe("the parked answer schema", () => {
  ...
  test("a removed static schema takes the RC5048 re-ask", async () => {
    ...
    try {
      t2 = await testContext()
        .with(shared(store))
        .routes([
          craft().id("payout").from(direct()).defer({}).to(noop()),
          craft().id("answers").from(direct()).resume(payloadFrom),
        ])
        .build();
  ...
  test("the structural fallback and the advertised schema agree", () => {
    const advertised = DEFERRED_JSON_SCHEMA.properties as Record<
      string,
      unknown
    >;
    expect(Object.keys(advertised)).toContain("schema");
    expect(Object.keys(advertised)).not.toContain("expect");
    expect(DEFERRED_JSON_SCHEMA.additionalProperties).toBe(false);

    const base = { status: "deferred", deferralId: "s-1", token: "t-1" };
    const accepted = deferredSchema["~standard"].validate({
      ...base,
      schema: { type: "object" },
    });
    expect("value" in accepted).toBe(true);

    const refused = deferredSchema["~standard"].validate({
      ...base,
      expect: { type: "object" },
    });
   ...
   * @expectedResult RC5056 rather than RC5049, and the record is left deferred, so the refusal is decided without the validator ever seeing the value
   */
   ...
   * @expectedResult RC5049 leaves the record deferred and undenied, and the corrected submission still resumes it
   */
   ...
   * @preconditions A step reading ex.deferral.tokenFor through a loosely typed boundary and passing no binding
    ...
    t = await testContext()
      ...
      .routes([
        craft()
          ...
          .from(direct())
          .tap((ex) => {
            const loose = ex.deferral as unknown as {
