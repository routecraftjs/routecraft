...
describe("continuationTailHash", () => {
   ...
   * @preconditions Two pipelines differing in a step AFTER the defer point
  ...
  /**
   * @case Prepending steps shifts the defer position without changing the tail
  ...
  test("hashes the tail rather than the pipeline", () => {
    const short = [step("defer", (v) => v), step("pay", (v) => v)];
    const long = [
      step("log", (v) => v),
      step("enrich", (v) => v + 7),
      step("defer", (v) => v),
  ...
  /**
   * @case Deferring at the last step leaves an empty continuation
   * @preconditions Position points at the final step
   * @expectedResult A hash is still produced, and it is stable
   */
  test("handles a defer at the end of the pipeline", () => {
    const steps = [step("defer", (v) => v)];
...
describe("continuationTailHash over adapter configuration", () => {
   ...
   */
  test("a changed adapter option after the defer point invalidates it", () => {
    const toBankA = [
      step("defer", (v) => v),
  ...
  /**
   * @case An option change BEFORE the defer point still does not invalidate
   ...
   */
  test("an option change before the defer point leaves it resumable", () => {
    const before = [
...
describe("continuationTailHash over factory-built adapters", () => {
   ...
   */
  test("a changed factory argument after the defer point invalidates it", () => {
    const toA = [
      step("s", (v) => v),
