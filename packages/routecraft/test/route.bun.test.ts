...
describe("Route Behavior", () => {
  ...
  test("processes messages through pipeline", async () => {
    const transformerSpy = mock((body) => body);
    const s = spy();
  ...
  test("handles processor errors gracefully", async () => {
    ...
    expect((t.logger.error as ReturnType<typeof mock>).mock.calls[0][1]).toBe(
      "Processor error",
    );
  });
  ...
  test("handles nested split operations", async () => {
    ...
    const destSpy = spy();
    const processorSpy = mock((exchange) => exchange);
    const processorSpy2 = mock((exchange) => exchange);
    ...
    const aggSpy2 = spyOn(agg2, "aggregate");
  ...
  test("multiple .to() calls transform body", async () => {
    const dest1Spy = mock(async () => ({ result: 1 }));
    const dest2Spy = mock(async () => ({ result: 2 }));
    const dest3Spy = mock(async () => ({ result: 3 }));
    const s = spy();
