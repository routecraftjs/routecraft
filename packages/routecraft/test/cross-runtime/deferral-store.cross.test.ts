...
/**
 * Cross-runtime contract for the durable deferral store.
 *
...
const scratch = mkdtempSync(join(tmpdir(), "rc-deferral-cross-"));
...
describe("deferral store (cross-runtime)", () => {
  let store: SqliteDeferralStore | undefined;
   ...
   * @preconditions One deferred record; two markResumed calls
   ...
   *   on one database file, lives in `deferral-store.bun.test.ts`, where a
   ...
   * @preconditions One deferred record; two swaps based on the same read
   ...
   * @preconditions A deferred record holding step state, replaced with
   *   undefined
  ...
  test("replaceStepState refuses a record that already resumed", async () => {
    ...
    expect(result.deferral?.stepState).toEqual({ turns: 1 });
  });
