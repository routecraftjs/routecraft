...
describe("Direct adapter validation", () => {
  ...
  describe("Registry and discovery", () => {
    ...
    test("multiple routes register correctly", async () => {
      t = await testContext()
        .routes([
          craft().id("endpoint-a").from(direct()).to(mock()),
          craft().id("endpoint-b").from(direct()).to(mock()),
          craft().id("endpoint-c").from(direct()).to(mock()),
        ])
        .build();
    ...
    test("registry persists across route registrations", async () => {
      t = await testContext()
        .routes([
          ...
          craft().id("endpoint-2").from(direct()).to(mock()),
        ])
        .build();
