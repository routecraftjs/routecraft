...
describe("SqliteDeferralStore durability", () => {
   ...
   *   still-deferred row, then opened by this build
   ...
   *   one full post-upgrade retention window; the deferred row gains none
   */
  test("v4 migration backfills settledAt per status", async () => {
    ...
    db.exec(`CREATE TABLE deferrals (
       id                 TEXT PRIMARY KEY,
    ...
    const insert = db.prepare(
      `INSERT INTO deferrals
         (id, route_id, position, continuation_hash, action_fingerprint,
          exchange, "schema", status, deferred_at, expires_at, resumed_at)
    ...
    insert.run("still-parked", "deferred", parked, null, null);
    db.close();
