...
import { DEFERRAL_SECRET_ENV } from "../src/index.ts";
...
describe("resume tokens", () => {
  /**
   * @case A minted token verifies back to the deferral it names
   ...
   */
  test("mints a token that verifies back to its deferral", () => {
  ...
  /**
   * @case A token is minted before the defer step runs
   * @preconditions Only a deferral id is available, no stored record
   * @expectedResult Minting succeeds, which is what lets a notification step
   *   ahead of the defer send a working resume link
   */
  test("needs nothing but an id to mint", () => {
    const signer = new ResumeTokenSigner(SECRET, "config");
    const id = deferralIdFor("ex-1", 0);
   ...
   */
  test("derives a distinct id per deferral of the same exchange", () => {
    expect(deferralIdFor("ex-1", 0)).toBe("ex-1~0");
    expect(deferralIdFor("ex-1", 1)).not.toBe(deferralIdFor("ex-1", 0));
  });
  ...
  test("does not collide when an exchange id ends in a sequence suffix", () => {
    expect(deferralIdFor("ex-1~1", 0)).not.toBe(deferralIdFor("ex-1", 1));
  });

  /**
   * @case A tampered payload does not verify
   * @preconditions A token whose body is swapped for another deferral id
...
describe("signing secret resolution", () => {
  ...
  test("refuses to start without a secret", () => {
    ...
    expect(() => resolveSigningSecret({ env: {} })).toThrow(
      new RegExp(DEFERRAL_SECRET_ENV),
    );
  });
  ...
  test("refuses a secret below the strength floor", () => {
    ...
    expect(() =>
      resolveSigningSecret({ env: { [DEFERRAL_SECRET_ENV]: "changeme" } }),
    ).toThrow(/at least 32/);
  });
