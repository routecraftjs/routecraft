 ...
 * the docs site's hand-maintained ErrorTable component. Core covers its own
...
describe("AI error codes", () => {
   ...
   * @preconditions Codes registered in src/errors.ts; ErrorTable.tsx read from disk
  ...
  test("every AI code appears in the docs error table", async () => {
    ...
    const listed = new Set(
      Array.from(table.matchAll(/code: '(AI\d{4})'/g), (match) => match[1]),
    );
