...
import { Approval, MODEL, askFn } from "./helpers/defer-fixtures.ts";
...
/** Plugins every deferring agent context needs: a provider and the fns. */
...
describe("agent durable deferral (ctx.defer)", () => {
  ...
  test("ctx.defer defers the run and answers with the Deferred acknowledgment", async () => {
    ...
    t = await testContext()
      .with({ deferral: {}, plugins: plugins({ ask: askWithCapture }) })
  ...
  test("resume re-enters the loop with the answer and a duplicate is idempotent", async () => {
    ...
    t = await testContext()
      .with({ deferral: {}, plugins: plugins({ ask: askFn }) })
      .routes(assistantRoutes(sink))
      .build();
    await t.startAndWaitReady();
  ...
  test("a resumed run can defer again: two deferrals, two resumes, one completion", async () => {
    ...
    const askWithCapture = {
      ...askFn,
      handler: (input: unknown, ctx: FnHandlerContext) => {
        ids.push(ctx.deferralId!);
        tokens.push(ctx.deferral!.token);
        return ctx.defer({
          schema: Approval,
          meta: { question: (input as { question: string }).question },
        });
    ...
    // The final call's thread carries both deferred calls' answers.
  ...
  test("a deferring call flushes in-flight siblings and keeps their results", async () => {
    ...
    t = await testContext()
      .with({ deferral: {}, plugins: plugins({ ask: askFn, lookup }) })
      .routes([
        craft()
          .id("assistant")
          .from(direct())
   ...
   */
  test("a second defer in one batch converts to a retryable tool error", async () => {
    const sink = spy();
    const ask2 = {
    ...
    t = await testContext()
      .with({ deferral: {}, plugins: plugins({ ask: askFn, ask2 }) })
      .routes([
        craft()
          .id("assistant")
          .from(direct())
    ...
    expect(llm.calls.length).toBeGreaterThanOrEqual(2);
    const resumedPrompt = JSON.stringify(llm.calls[1]!.user);
    expect(resumedPrompt).toContain("already deferred the run");
    expect(resumedPrompt).toContain('"approved":true');
    expect(t.errors).toHaveLength(0);
  });
  ...
  test("ctx.defer meta round-trips to the resume hook", async () => {
    ...
    t = await testContext()
      .with({ deferral: {}, plugins: plugins({ ask: askWithMeta }) })
      .routes([
        craft()
          .id("assistant")
          .from(direct())
  ...
  test("a losing sibling's credential is refused", async () => {
    ...
    const capture = (name: string) => ({
      ...
      input: z.object({ question: z.string() }),
      handler: (input: unknown, ctx: FnHandlerContext) => {
        tokens[name] = ctx.deferral!.token;
        return ctx.defer({
          schema: Approval,
          meta: { question: (input as { question: string }).question },
        });
  ...
  test("DeferError defers the run and any JSON payload resumes it", async () => {
    ...
    const legacy = {
      ...
      input: z.object({}),
      handler: (): never => {
        throw new DeferError({ meta: { question: "legacy?" } });
      },
    };
    llm.script.push({ toolCalls: [{ toolName: "legacy", input: {} }] });

    t = await testContext()
      .with({ deferral: {}, plugins: plugins({ legacy }) })
      .routes([
        craft()
          .id("assistant")
          .from(direct())
   ...
   * @preconditions ctx.defer declared schema: Approval; the answer is a string that does not satisfy it
  ...
  /**
   * @case A deferral signal in an identity-less (synthetic) dispatch is refused with AI1006 and writes nothing
   * @preconditions The agent adapter is invoked directly on an exchange with no route binding; the tool calls ctx.defer
   * @expectedResult ctx.defer throws AI1006 at the call (surfaced as that tool call's error), the dispatch itself completes, and the deferral store holds no record
   */
  test("an identity-less dispatch refuses ctx.defer with AI1006 and writes no record", async () => {
    llm.script.push(
      { toolCalls: [{ toolName: "ask", input: { question: "ok?" } }] },
      { text: "recovered without deferring" },
    );
    ...
    expect(result.text).toBe("recovered without deferring");
   ...
   */
  test("ctx.defer rejects a malformed ttl with RC5003 at the call", () => {
    ...
    expect(ctx.defer({ ttl: "72h" }).status).toBe("defer-requested");
  });
});
