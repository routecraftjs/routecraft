...
import type { AgentDeferOptions } from "./defer.ts";
 ...
 * batch, with the deferred call's placeholder result still in place (the
 * resume path swaps the real answer in by `deferredToolCallId`). Only
...
/**
 * One deferral signal collected by the tool bridge during a batch of tool
 * calls: which call raised it, and what it asked for.
 *
 * @internal
 */
export interface AgentDeferSignalRecord {
  toolCallId: string;
  toolName: string;
...
export function parseStepState(value: unknown): AgentStepState {
  ...
  if (
    ...
    typeof state.deferredToolCallId !== "string" ||
    ...
    throw rcError("AI1007", undefined, {
      message:
        "The resumed deferral's stepState is not the { agentId, messages, deferredToolCallId, turnsUsed, usage? } record the agent runtime writes, so the tool loop cannot be re-entered.",
    });
  }
...
export function rehydrateSession(
  ...
  if (!swapped.found) {
    throw rcError("AI1007", undefined, {
      message: `The resumed deferral's persisted thread does not contain the deferred tool call "${state.deferredToolCallId}", so the answer has nowhere to land.`,
    });
  }
  return {
...
/**
 * Pick the winning defer signal of a batch: the one whose tool call the
