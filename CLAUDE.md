...
- The published `craft` CLI binary uses `#!/usr/bin/env bun` and `engines.bun >= 1.1.0`; the core library targets both Node 22.0+ and Bun. The root `bun run craft` workspace shortcut invokes the built `dist/index.js` via `bun`, matching the binary's runtime requirement (the CLI refuses to start under node).
- Every optional-peer dynamic import MUST go through `loadOptionalPeer` (`packages/routecraft/src/adapters/shared/optional-peer.ts`) and emit `RC5017` with an install hint. The migration of legacy bespoke try/catch sites is complete (see [`.standards/ci-cd.md` § 6](.standards/ci-cd.md#6-optional-peer-dependencies-provider-sdks)); no bespoke shape remains. New code is reviewed against this contract.
