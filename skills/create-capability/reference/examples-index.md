...
| HTTP enrichment (call an API mid-pipeline; the result replaces the body, or merge it with `only(...)`) | https://routecraft.dev/raw/docs/reference/operations.md | https://raw.githubusercontent.com/routecraftjs/routecraft/main/examples/src/hello-world.ts (the same file; focus on the `.enrich(http(...))` step) |
...
| Expose a capability as an MCP tool callable by AI agents | https://routecraft.dev/raw/docs/examples/mcp.md | https://raw.githubusercontent.com/routecraftjs/routecraft/main/examples/src/mcp-greet.ts |
| Call a tool on an external MCP server from within a capability | https://routecraft.dev/raw/docs/advanced/call-an-mcp.md | https://raw.githubusercontent.com/routecraftjs/routecraft/main/examples/src/mcp-greet.ts (the same file exposes a tool; the example page also shows the `mcp('server:tool')` call shape) |
| Call an LLM inside a capability (agent step) | https://routecraft.dev/raw/docs/reference/adapters/agent.md | https://raw.githubusercontent.com/routecraftjs/routecraft/main/examples/src/agent.ts |
